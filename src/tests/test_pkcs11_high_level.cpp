...
Test::Result test_module_ctor() {
   ...
   result.test_throws("Module ctor fails for non existent path", []() { Module const failing_module("/a/b/c"); });
...
Test::Result test_session_ctor_invalid_slot() {
   ...
   result.test_throws("Session ctor with invalid slot id fails", [&slot]() { Session const session(slot, true); });
...
Test::Result test_ecdsa_sign_verify_core(EC_Group_Encoding enc, const std::string& test_name) {
   ...
   for(const auto& curve : curves) {
      ...
      auto sign_and_verify = [&](const std::string& emsa, const Botan::Signature_Format format, bool check_soft) {
         ...
         bool const ecdsa_ok = token_verifier.verify_message(plaintext, signature);
         ...
         if(check_soft) {
            ...
            bool const soft_ecdsa_ok = soft_verifier.verify_message(plaintext, signature);
