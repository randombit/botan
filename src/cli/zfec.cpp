...
class FEC_Share final
   ...
   public:
      ...
      FEC_Share(size_t share, size_t k, size_t n,
                 size_t padding,
      ...
      static FEC_Share deserialize(const uint8_t bits[], size_t len,
         ...
         const bool digest_ok = Botan::same_mem(
            share_hash.data(), &bits[len - hash_len], hash_len);

         if(!digest_ok)
            throw CLI_Error("FEC share has invalid hash");

         return FEC_Share(share_id, k, n, padding,
   ...
   public:
      ...
      void go() override
         ...
         for(const auto& share_fsname: get_arg_list("shares"))
            ...
            try
               {
               auto share = FEC_Share::deserialize(share_bits.data(),
                                                   share_bits.size(),
                                                   *hash);
               ...
               {
               error_output() << "Ignoring invalid share '" << share_fsname
                              << "': " << e.what() << "\n";
               }
            }
         ...
         for(const auto& share: shares)
            ...
            if(k == 0 && n == 0 && padding == 0)
               ...
               if(share.k() != k ||
                  share.n() != n ||
                  share.padding() != padding ||
                  share.share_size() != share_size)
                  {
         ...
         if(!Botan::same_mem(decoded_digest.data(),
      ...
      void go() override
         ...
         try
            ...
            auto share = FEC_Share::deserialize(share_bits.data(),
                                                share_bits.size(),
                                                *hash);
            output() << "FEC share " << share.share_id() + 1 << "/"
                     << share.n() << " with " << share.k()
                     << " needed for recovery\n";
            }
         catch(std::exception& e)
            {
            error_output() << "Invalid share '" << share_fsname
                           << "': " << e.what() << "\n";
            }
         }
   };
