...
class Channel_Impl
   ...
   protected:
      ...
      struct Downgrade_Information
         ...
         std::optional<std::pair<Session, Session_Handle>> tls12_session;
      ...
      void request_downgrade_for_resumption(std::pair<Session, Session_Handle> session)
         {
         BOTAN_STATE_CHECK(m_downgrade_info &&
         ...
         BOTAN_ASSERT_NOMSG(session.first.version().is_pre_tls_13());
