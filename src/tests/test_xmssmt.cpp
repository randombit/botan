...
class XMSSMT_Signature_Tests final : public PK_Signature_Generation_Test {
   public:
      ...
      std::unique_ptr<Botan::Private_Key> load_private_key(const VarMap& vars) override {
         ...
         return std::make_unique<Botan::XMSSMT_PrivateKey>(Botan::AlgorithmIdentifier(), sec_key);
      }
};
...
class XMSSMT_Signature_Verify_Tests final : public PK_Signature_Verification_Test {
   public:
      ...
      std::unique_ptr<Botan::Public_Key> load_public_key(const VarMap& vars) override {
         ...
         return std::make_unique<Botan::XMSSMT_PublicKey>(Botan::AlgorithmIdentifier(), raw_key);
      }
...
class XMSSMT_Keygen_Tests final : public PK_Key_Generation_Test {
   public:
      ...
      std::unique_ptr<Botan::Public_Key> public_key_from_raw(std::string_view /* keygen_params */,
         ...
         const auto p = Botan::XMSSMT_Parameters::from_id(oid);
...
class XMSSMT_Keygen_Reference_Test final : public Text_Based_Test {
   public:
      ...
      bool skip_this_test(const std::string& /*header*/, const VarMap& vars) override {
         ...
         const auto params = Botan::XMSSMT_Parameters::from_name(param_str);
...
std::vector<Test::Result> xmssmt_statefulness() {
   ...
   // An XMSSMT-SHA2_20/4_256 private key whose unused leaf index is at 2^20-1,
   ...
   // The 3 byte unused leaf index is stored just after the raw public key
   // (3 byte OCTET STRING header, 4 byte OID, 32 byte root, 32 byte public seed)
   const size_t leaf_idx_offset = 71;
   ...
   return {CHECK("signing alters state",
           ...
           CHECK("state can become exhausted",
                 [&](auto& result) {
                    Botan::XMSSMT_PrivateKey sk(Botan::AlgorithmIdentifier(), almost_exhausted_sk_bytes);
                    ...
                    const Botan::XMSSMT_PrivateKey sk2(Botan::AlgorithmIdentifier(), sk.private_key_bits());
           ...
           CHECK("out of range leaf index is rejected on load",
                 [&](Test::Result& result) {
                    ...
                    // 2^20 + 1 as a big endian 3 byte value
                    skbytes[leaf_idx_offset] = 0x10;
                    skbytes[leaf_idx_offset + 1] = 0x00;
                    skbytes[leaf_idx_offset + 2] = 0x01;
                    result.test_throws<Botan::Decoding_Error>("no key with leaf index 2^h + 1", [&] {
                       const Botan::XMSSMT_PrivateKey sk(Botan::AlgorithmIdentifier(), skbytes);
                    });
                 }),

           CHECK("separately loaded copies share state", [&](auto& result) {
              const Botan::XMSSMT_PrivateKey sk(Botan::XMSSMT_Parameters::XMSSMT_SHA2_20_4_256, *rng);
              const auto skbytes = sk.private_key_bits();

              const Botan::XMSSMT_PrivateKey copy1(Botan::AlgorithmIdentifier(), skbytes);
              const Botan::XMSSMT_PrivateKey copy2(Botan::AlgorithmIdentifier(), skbytes);

              const auto sig1 = sign_something(copy1);
              result.test_opt_u64_eq(
                 "signing with one copy is seen by the other", copy2.remaining_operations(), 1048575);

              const auto sig2 = sign_something(copy2);

              // The first three bytes of an XMSSMT-SHA2_20/4_256 signature encode the leaf index
              auto leaf_index = [](const std::vector<uint8_t>& sig) {
                 return (static_cast<uint32_t>(sig[0]) << 16) | (static_cast<uint32_t>(sig[1]) << 8) | sig[2];
              };
              result.test_u64_eq("first signature used leaf 0", leaf_index(sig1), 0);
              result.test_u64_eq("second signature used leaf 1", leaf_index(sig2), 1);
           })};
}
