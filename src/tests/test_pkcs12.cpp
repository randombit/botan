...
#if defined(BOTAN_HAS_PKCS12) && defined(BOTAN_HAS_ECDSA)
...
class PKCS12_Tests final : public Test {
   public:
      std::vector<Test::Result> run() override {
         ...
         // Targeted exercises of the public PKCS12 / PKCS12_Export_Options surface.
         ...
         // Additional MAC digests (SHA-256 is covered by test_mac_sha256)
         ...
         // Openssl empty password
         results.push_back(test_parse_file_openssl_empty_password("name-1-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("name-2-3-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("name-2-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-cert-no-name-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-name-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("name-3-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-cert-name-all-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("name-all-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("name-unicode-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-cert-name-2-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-cert-name-3-no-pwd.p12"));
         results.push_back(test_parse_file_openssl_empty_password("no-cert-name-unicode-no-pwd.p12"));
         // Openssl empty password (RC2)
         results.push_back(test_parse_file_openssl_empty_password("no-password.p12"));
         // Wrong password
   ...
   private:
      ...
      Test::Result test_parse_file_openssl_empty_password(const std::string& filename) {
         Test::Result result("PKCS12 parse file openssl empty password: " + filename);

         try {
            const std::vector<uint8_t> pfx_data = Test::read_binary_data_file("pkcs12/" + filename);
            result.test_throws<Botan::Invalid_Authentication_Tag>("openssl empty password throws",
                                                                  [&]() { Botan::PKCS12(pfx_data, ""); });
      ...
      Test::Result test_nesting_depth_exceeded() {
         ...
         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         const Botan::OID safe_contents_bag_oid = Botan::OID::from_string("PKCS12.SafeContentsBag");

         // Build SafeContents with nesting depth > PKCS12_MAX_NESTING (10)
         // Each level: SafeBag { SafeContentsBag, [0] { SafeContents { ... } } }
         std::vector<uint8_t> safe_contents;
         {
            // Start from innermost empty SafeContents and wrap 11 times
            std::vector<uint8_t> inner;
            {
               Botan::DER_Encoder enc(inner);
               enc.start_sequence();
               enc.end_cons();
            }
            ...
            for(size_t i = 0; i < 11; ++i) {
               ...
               Botan::DER_Encoder enc(outer);
               enc.start_sequence();
               enc.start_sequence();
               enc.encode(safe_contents_bag_oid);
         ...
         std::vector<uint8_t> auth_safe;
         {
            Botan::DER_Encoder as(auth_safe);
            as.start_sequence();
            as.start_sequence();
            as.encode(pkcs7_data_oid);
            as.start_context_specific(0);
            as.encode(safe_contents, Botan::ASN1_Type::OctetString);
            as.end_cons();
            as.end_cons();
            as.end_cons();
         }
         ...
         {
            Botan::DER_Encoder pfx(pfx_data);
            pfx.start_sequence();
            pfx.encode(size_t(3));
            pfx.start_sequence();
            pfx.encode(pkcs7_data_oid);
            pfx.start_context_specific(0);
            pfx.encode(auth_safe, Botan::ASN1_Type::OctetString);
            pfx.end_cons();
            pfx.end_cons();
            pfx.end_cons();
         }

         result.test_throws<Botan::Decoding_Error>("nesting too deep throws", [&]() { Botan::PKCS12(pfx_data, ""); });
      ...
      Test::Result test_key_bag() {
         ...
         auto rng = Test::new_rng("PKCS12_key_bag");
         auto creds = generate_credentials(*rng);

         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         const Botan::OID key_bag_oid = Botan::OID::from_string("PKCS12.KeyBag");
         const Botan::OID cert_bag_oid = Botan::OID::from_string("PKCS12.CertBag");
         const Botan::OID x509_cert_oid = Botan::OID::from_string("PKCS9.X509Certificate");

         // Build SafeContents with a KeyBag and a CertBag
         std::vector<uint8_t> safe_contents;
         {
            Botan::secure_vector<uint8_t> pkcs8_key = Botan::PKCS8::BER_encode(*creds.key);

            Botan::DER_Encoder sc(safe_contents);
            sc.start_sequence();

            // KeyBag
            sc.start_sequence();
            sc.encode(key_bag_oid);
            sc.start_context_specific(0);
            sc.raw_bytes(pkcs8_key);
            sc.end_cons();
            sc.end_cons();

            // CertBag
            sc.start_sequence();
            sc.encode(cert_bag_oid);
            sc.start_context_specific(0);
            sc.start_sequence();
            sc.encode(x509_cert_oid);
            sc.start_context_specific(0);
            sc.encode(creds.cert.BER_encode(), Botan::ASN1_Type::OctetString);
            sc.end_cons();
            sc.end_cons();
            sc.end_cons();
            sc.end_cons();
         ...
         // Build AuthenticatedSafe (single unencrypted ContentInfo)
         std::vector<uint8_t> auth_safe;
         {
            Botan::DER_Encoder as(auth_safe);
            as.start_sequence();
         ...
         const auto parsed = Botan::PKCS12(pfx_data, "");
         result.test_is_true("Has private key", !parsed.private_keys().empty());
         result.test_is_true("Has certificate", parsed.end_entity_certificate().has_value());

         if(!parsed.private_keys().empty()) {
            const auto parsed_bits = parsed.private_keys().front()->private_key_bits();
            const auto orig_bits = creds.key->private_key_bits();
            result.test_is_true("Key matches", parsed_bits == orig_bits);
         }
      ...
      // Build a PFX (no MAC) with a SafeContentsBag wrapping a CertBag
      Test::Result test_safe_contents_bag() {
         Test::Result result("PKCS12 SafeContentsBag (nested)");

         auto rng = Test::new_rng("PKCS12_sc_bag");
         auto creds = generate_credentials(*rng);
         ...
         // Build SafeContents with a SafeContentsBag that itself contains a CertBag
         std::vector<uint8_t> safe_contents;
         {
            Botan::DER_Encoder sc(safe_contents);
            sc.start_sequence();
            ...
            // SafeContentsBag
            sc.start_sequence();
            sc.encode(safe_contents_bag_oid);
            sc.start_context_specific(0);

            // Nested SafeContents (SEQUENCE of SafeBag)
            sc.start_sequence();
            // Inner CertBag
            sc.start_sequence();
            sc.encode(cert_bag_oid);
         ...
         if(!parsed.certificates().empty()) {
            result.test_is_true("Certificate matches",
                                parsed.certificates().front().BER_encode() == creds.cert.BER_encode());
         }
      ...
      Test::Result test_end_entity_without_match() {
         ...
         auto rng = Test::new_rng("PKCS12_no_match");
         auto creds = generate_credentials(*rng, "Holder");
         auto other = generate_credentials(*rng, "Unrelated CA");

         // Build a PFX manually: one key (creds.key), one cert (other.cert)
         // whose SPKI does NOT match the key. Bypass PKCS12::export_to
         // because it intentionally rejects this configuration.
         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         const Botan::OID cert_bag_oid = Botan::OID::from_string("PKCS12.CertBag");
         const Botan::OID x509_cert_oid = Botan::OID::from_string("PKCS9.X509Certificate");
         const Botan::OID key_bag_oid = Botan::OID::from_string("PKCS12.KeyBag");

         // SafeContents: CertBag(other.cert) || KeyBag(creds.key) -- using
         // an unencrypted KeyBag avoids needing the internal pkcs12_pbe_encrypt.
         std::vector<uint8_t> safe_contents;
         {
            Botan::DER_Encoder sc(safe_contents);
            sc.start_sequence();
            ...
            sc.encode(x509_cert_oid);
            sc.start_context_specific(0);
            sc.encode(other.cert.BER_encode(), Botan::ASN1_Type::OctetString);
            sc.end_cons();
            sc.end_cons();
            sc.end_cons();
            sc.end_cons();

            Botan::secure_vector<uint8_t> pkcs8 = Botan::PKCS8::BER_encode(*creds.key);
            sc.start_sequence();
            sc.encode(key_bag_oid);
            sc.start_context_specific(0);
            sc.raw_bytes(pkcs8);
         ...
         // AuthenticatedSafe + PFX (no MAC for simplicity)
         std::vector<uint8_t> auth_safe;
         {
            Botan::DER_Encoder as(auth_safe);
            as.start_sequence();
         ...
         const Botan::PKCS12 parsed(pfx_data, "");
         result.test_is_true("Key parsed", !parsed.private_keys().empty());
         ...
         // end-entity; with a single mismatched cert it must therefore be
         // empty.
      ...
      Test::Result test_unknown_bag_types() {
         ...
         auto rng = Test::new_rng("PKCS12_unknown_bag");
         auto creds = generate_credentials(*rng);

         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         const Botan::OID cert_bag_oid = Botan::OID::from_string("PKCS12.CertBag");
         const Botan::OID x509_cert_oid = Botan::OID::from_string("PKCS9.X509Certificate");
         // SecretBag is part of RFC 7292 but not implemented here.
         const Botan::OID secret_bag_oid = Botan::OID::from_string("PKCS12.SecretBag");
         ...
         std::vector<uint8_t> safe_contents;
         {
            ...
            // An unknown SecretBag with arbitrary bytes as its value.
            sc.start_sequence();
            sc.encode(secret_bag_oid);
            sc.start_context_specific(0);
            const std::vector<uint8_t> dummy = {0x01, 0x02, 0x03};
            sc.encode(dummy, Botan::ASN1_Type::OctetString);
            sc.end_cons();
            sc.end_cons();

            // Followed by a real CertBag so the file still looks reasonable.
            sc.start_sequence();
            sc.encode(cert_bag_oid);
            sc.start_context_specific(0);
            sc.start_sequence();
         ...
         if(!parsed.unknown_bag_types().empty()) {
            result.test_str_eq("Unknown bag is SecretBag",
                               ...
                               secret_bag_oid.to_formatted_string());
         }
         // The cert should still be parsed even though a sibling bag was skipped.
      ...
      // Build a PFX with version=2 instead of version=3 and verify rejection.
      Test::Result test_pfx_invalid_version() {
         Test::Result result("PKCS12 unsupported version rejected");

         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");

         // Minimal AuthenticatedSafe: empty SEQUENCE OF ContentInfo.
         std::vector<uint8_t> empty_auth_safe;
         {
            Botan::DER_Encoder e(empty_auth_safe);
            e.start_sequence();
            e.end_cons();
         }
         ...
         std::vector<uint8_t> pfx_data;
         {
            Botan::DER_Encoder pfx(pfx_data);
            pfx.start_sequence();
            pfx.encode(size_t(2));  // unsupported version
            pfx.start_sequence();
            pfx.encode(pkcs7_data_oid);
            pfx.start_context_specific(0);
            pfx.encode(empty_auth_safe, Botan::ASN1_Type::OctetString);
            pfx.end_cons();
            pfx.end_cons();
            pfx.end_cons();
         }

         result.test_throws<Botan::Decoding_Error>("version != 3 throws", [&]() { Botan::PKCS12(pfx_data, ""); });
      ...
      Test::Result test_envelopeddata_rejected() {
         ...
         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         // PKCS7 EnvelopedData has no registered name in Botan's OID table; use raw OID.
         const Botan::OID pkcs7_enveloped_oid{1, 2, 840, 113549, 1, 7, 3};

         // AuthenticatedSafe = SEQUENCE { ContentInfo { EnvelopedData, [0] ... } }
         std::vector<uint8_t> auth_safe;
         {
            Botan::DER_Encoder as(auth_safe);
            as.start_sequence();
            as.start_sequence();
            as.encode(pkcs7_enveloped_oid);
            as.start_context_specific(0);
            // Dummy payload (empty SEQUENCE) — parsing must fail before it's looked at.
            as.start_sequence();
            as.end_cons();
            as.end_cons();
            as.end_cons();
         ...
         result.test_throws<Botan::Decoding_Error>("EnvelopedData throws", [&]() { Botan::PKCS12(pfx_data, ""); });

         return result;
      }

      // A PFX with a CrlBag (RFC 7292 §4.2.4) — not implemented here — must be
      ...
      Test::Result test_crl_bag_unknown() {
         ...
         const Botan::OID pkcs7_data_oid = Botan::OID::from_string("PKCS7.Data");
         const Botan::OID crl_bag_oid = Botan::OID::from_string("PKCS12.CRLBag");
         ...
         std::vector<uint8_t> safe_contents;
         {
            ...
            sc.start_sequence();
            sc.start_sequence();
            sc.encode(crl_bag_oid);
            sc.start_context_specific(0);
            const std::vector<uint8_t> dummy = {0x30, 0x00};  // empty SEQUENCE
            sc.raw_bytes(dummy);
            sc.end_cons();
            sc.end_cons();
            sc.end_cons();
         ...
         if(!parsed.unknown_bag_types().empty()) {
            result.test_str_eq("Reported OID is CrlBag",
                               parsed.unknown_bag_types().front().to_formatted_string(),
                               crl_bag_oid.to_formatted_string());
         }
