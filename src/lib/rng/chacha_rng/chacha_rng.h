...
class BOTAN_PUBLIC_API(2, 3) ChaCha_RNG final : public Stateful_RNG {
   ...
   public:
      enum class KeyErasure : uint8_t { NoErasure, WithKeyErasure };
      ...
      *
      * @param key_erasure overwrite state after each operation
      * for backtracking resistance, costs performance depending
      * on request size mix, deactivated by default
      */
      explicit ChaCha_RNG(KeyErasure key_erasure = KeyErasure::NoErasure);
      ...
      * on request size mix, deactivated by default
      */
      BOTAN_FUTURE_EXPLICIT ChaCha_RNG(std::span<const uint8_t> seed, KeyErasure key_erasure = KeyErasure::NoErasure);
      ...
      */
      BOTAN_FUTURE_EXPLICIT ChaCha_RNG(RandomNumberGenerator& underlying_rng,
                                       ...
                                       KeyErasure key_erasure = KeyErasure::NoErasure);
      ...
      */
      ...
      KeyErasure key_erasure() const { return m_key_erasure; }
