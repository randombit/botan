...
def create_cert_and_key(botan_dir_path):
    ...
    cert_path = os.path.join(
        Config.key_and_cert_storage_path, Config.tmp_cert_file_name
    )

    with open(key_path, "w", encoding="utf-8") as keyfile:
        subprocess.run(
            [botan_dir_path, "keygen", "--algo=RSA", "--params=2048"],
            stdout=keyfile,
            check=True,
        )
    ...
    with open(cert_path, "w", encoding="utf-8") as certfile:
        subprocess.run(
            [botan_dir_path, "gen_self_signed", key_path, "localhost"],
            stdout=certfile,
            check=True,
        )
...
def build_botan(botan_dir: str, parallel_jobs: int) -> str:
    ...
    """
    group_output("Configure Botan", subprocess.run)(
        ["python3", "./configure.py"] + Config.botan_config_args,
        check=True,
        cwd=botan_dir,
    )
    group_output("Build Botan with Make", subprocess.run)(
        ["make", f"-j{parallel_jobs}"], check=True, cwd=botan_dir
    )
...
def server_test(botan_dir_path: str, parallel: int):
    ...
    group_output("Pull TLS-Anvil image", subprocess.run)(
        ["docker", "pull", docker_img], check=True
    )
    ...
    tls_anvil_cmd = [
        ...
        "host",
        "-v",
        f"{Config.test_suite_results_dest}:/output",
        docker_img,
        "-strength",
        "1",
        "-parallelHandshakes",
        ...
        "-outputFolder",
        os.path.join(
            Config.test_suite_results_dest, Config.test_suite_results_dir_name
        ),
        "-connectionTimeout",
        "5000",
        "server",
        "-connect",
        f"{Config.server_dest_ip}:{Config.server_dest_port}",
    ]
    ...
    with open(Config.botan_server_log, "w", encoding="utf-8") as server_log_file:
        botan_server_process = subprocess.Popen(
            botan_server_cmd, stdout=server_log_file, stderr=server_log_file
        )
...
def main(args=None):
    ...
    parser.add_argument(
        "--test-target",
        help="The TLS side to test",
        choices=["client", "server"],
        required=True,
    )
    parser.add_argument(
        "--parallel", help="The number of parallel handshakes", type=int, default=1
    )
