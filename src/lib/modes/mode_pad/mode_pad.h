...
class BOTAN_TEST_API BlockCipherModePaddingMethod {
   public:
      ...
      */
      virtual size_t unpad(std::span<const uint8_t> last_block) const;
   ...
   protected:
      ...
      * @p padding_start_pos via side channels.
      */
      virtual void apply_padding(std::span<uint8_t> last_block, size_t padding_start_pos) const {
         BOTAN_UNUSED(last_block, padding_start_pos);
