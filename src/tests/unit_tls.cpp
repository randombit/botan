...
class TLS_Unit_Tests final : public Test {
   private:
      ...
      void test_custom_kdf_provider(std::vector<Test::Result>& results,
         ...
         test.results().confirm("custom KDF was used on client side", client_custom_kdf_called);
         test.results().confirm("custom KDF was used on server side", server_custom_kdf_called);
         results.push_back(test.results());
      }
