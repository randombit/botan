...
def run_fuzzer_gdb(args):
    ...
    gdb_proc = subprocess.Popen(
        ["gdb", "--quiet", "--return-child-result", fuzzer_bin],
...
def main(args=None):
    ...
    parser.add_option(
        "--gdb", action="store_true", help="Run under GDB and capture backtraces"
    )
    ...
    if options.one_at_a_time:
        ...
        for fuzzer in sorted(list(fuzzers_with_corpus)):
            ...
            corpus_files = [
                os.path.join(corpus_subdir, fsname)
                for fsname in sorted(list(os.listdir(corpus_subdir)))
            ]
            ...
            for result in pool.map(run_fuzzer_func, map_args, chunk_size):
                ...
                if retcode != 0:
                    print(
                        "Fuzzer %s crashed with input %s returncode %d"
                        % (fuzzer, corpus_file, retcode)
                    )
                    crash_count += 1

                if stdout:
                    print(
                        "Fuzzer %s produced stdout on input %s:\n%s"
                        % (fuzzer, corpus_file, stdout)
                    )
                    stdout_count += 1

                if stderr:
                    print(
                        "Fuzzer %s produced stderr on input %s:\n%s"
                        % (fuzzer, corpus_file, stderr)
                    )
                    stderr_count += 1

            duration = time.time() - start
            print(
                "Tested fuzzer %s with %d test cases, %d crashes in %.02f seconds"
    ...
    if crash_count > 0 or stderr_count > 0 or stdout_count > 0:
        print(
            "Ran fuzzer tests, %d crashes %d stdout %d stderr"
            % (crash_count, stdout_count, stderr_count)
        )
        return 2
    return 0
