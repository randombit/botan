...
def parse_command_line(args):
    ...
    install_group.add_option(
        "--umask", metavar="MASK", default="022", help="Umask to set (default %default)"
    )
...
def prepend_destdir(path):
    ...
    if destdir:
        ...
        if os.path.sep != "/":
            raise PrependDestdirError(
                "Only UNIX-like file systems using forward slash "
                "separator supported when DESTDIR is set."
            )
        if not os.path.isabs(path):
            raise PrependDestdirError(
                "--prefix must be an absolute path when DESTDIR is set."
            )
        ...
        if not is_subdir(destdir, path):
            raise PrependDestdirError(
                "path escapes DESTDIR (path='%s', destdir='%s')" % (path, destdir)
            )
...
def main(args):
    ...
    for header in cfg["public_headers"]:
        ...
        copy_file(
            full_header_path, prepend_destdir(os.path.join(target_include_dir, header))
        )
    ...
    if build_static_lib:
        ...
        copy_file(
            os.path.join(out_dir, static_lib),
    ...
    if build_shared_lib:
        if target_os == "windows":
            ...
            copy_executable(
                os.path.join(out_dir, soname_patch),
    ...
    if build_cli:
        copy_executable(
            cfg["cli_exe"], prepend_destdir(os.path.join(bin_dir, cfg["cli_exe_name"]))
        )
    ...
    if "botan_cmake_config" in cfg and "botan_cmake_version_config" in cfg:
        ...
        copy_file(
            cfg["botan_cmake_config"],
            prepend_destdir(
                os.path.join(cmake_dir, os.path.basename(cfg["botan_cmake_config"]))
            ),
        )
        copy_file(
            cfg["botan_cmake_version_config"],
            prepend_destdir(
                os.path.join(
                    cmake_dir, os.path.basename(cfg["botan_cmake_version_config"])
                )
            ),
        )
    ...
    if (
        "ffi" in cfg["mod_list"]
        and cfg["build_shared_lib"] is True
        and cfg["install_python_module"] is True
    ):
        for ver in cfg["python_version"].split(","):
            ...
            copy_file(
                os.path.join(py_dir, "botan3.py"),
                prepend_destdir(os.path.join(py_lib_path, "botan3.py")),
            )

    if cfg["with_documentation"]:
        target_doc_dir = os.path.join(
            prefix, cfg["docdir"], "botan-%d.%d.%d" % (ver_major, ver_minor, ver_patch)
        )
        ...
        copy_file(
            os.path.join(cfg["base_dir"], "license.txt"),
            prepend_destdir(os.path.join(target_doc_dir, "license.txt")),
        )
        copy_file(
            os.path.join(cfg["base_dir"], "news.rst"),
            prepend_destdir(os.path.join(target_doc_dir, "news.txt")),
        )
        for f in [f for f in os.listdir(cfg["doc_dir"]) if f.endswith(".txt")]:
            copy_file(
                os.path.join(cfg["doc_dir"], f),
                prepend_destdir(os.path.join(target_doc_dir, f)),
            )

        if cfg["with_rst2man"]:
            man1_dir = prepend_destdir(
                os.path.join(prefix, os.path.join(cfg["mandir"], "man1"))
            )
            makedirs(man1_dir)
