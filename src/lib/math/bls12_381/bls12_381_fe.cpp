...
// (p-1)/2, for the lexicographic comparison
constexpr auto FE_P_HALF = [] {
   auto x = FE_P;
   shift_right<1>(x);
   return x;
}();

// (p+1)/4; since p == 3 (mod 4) this is (p >> 2) + 1, and the increment
// cannot overflow
constexpr auto FE_SQRT_EXP = [] {
   auto x = FE_P;
   shift_right<2>(x);
   x[0] += 1;
   return x;
...
std::array<word, FieldElement::N> fe_redc(const std::array<word, 2 * FieldElement::N>& v) {
   std::array<word, FieldElement::N> r{};
   std::array<word, FieldElement::N> ws{};
   bigint_monty_redc(r.data(), v.data(), FE_P.data(), FE_P.size(), FE_P_DASH, ws.data(), ws.size());
   return r;
}
...
/*
The input may be as large as 2^768 - 1, which exceeds R*p, so the first
reduction's output is not fully reduced; it is bounded by 2^384 and
congruent to v/R. That in turn bounds the second reduction's input by
R*p, which guarantees a fully reduced result.
*/
...
FieldElement FieldElement::add(const FieldElement& other) const {
   ...
   word carry = 0;
   for(size_t i = 0; i != FieldElement::N; ++i) {
      t[i] = word_add(m_val[i], other.m_val[i], &carry);
   }
...
FieldElement FieldElement::sub(const FieldElement& other) const {
   std::array<word, FieldElement::N> r;  // NOLINT(*-member-init)
   word borrow = 0;
   for(size_t i = 0; i != FieldElement::N; ++i) {
      r[i] = word_sub(m_val[i], other.m_val[i], &borrow);
   }

   const word mask = CT::Mask<word>::expand(borrow).value();
   word carry = 0;
   for(size_t i = 0; i != FieldElement::N; ++i) {
      r[i] = word_add(r[i], FE_P[i] & mask, &carry);
   }
...
CT::Choice FieldElement::is_zero() const {
   return CT::all_zeros(this->data(), N).as_choice();
}

CT::Choice FieldElement::_is_lexicographically_largest() const {
   const auto v = fe_from_rep(m_val);
   return !bigint_ct_is_lt(v.data(), N, FE_P_HALF.data(), N, true).as_choice();
}

CT::Choice FieldElement::operator==(const FieldElement& other) const {
   // The Montgomery representation of a reduced element is unique
   return CT::is_equal(this->data(), other.data(), N).as_choice();
}

void FieldElement::conditional_assign(CT::Choice cnd, const FieldElement& other) {
   CT::conditional_assign_mem(cnd, m_val.data(), other.m_val.data(), N);
}

CT::Option<FieldElement> FieldElement::sqrt() const {
   // Since p == 3 (mod 4) any square root must be x^((p+1)/4)
   const auto r = pow_msb(*this, FE_SQRT_EXP);
   return CT::Option<FieldElement>(r, r.square() == (*this));
}
