...
def main(args=None):
    ...
    parser.add_option("--verbose", action="store_true", help="Be noisy")
    ...
    java_tls_attacker = [
        ...
        tls_attacker_jar,
        "-loglevel",
        "DEBUG" if options.verbose else "ERROR",
    ]
    ...
    if test_type == "tests":
        try:
            server_process = spawn_server(
                botan_server_cmd + ["--policy=%s" % (lax_policy_txt)]
            )
            time.sleep(1)
            run_subprocess(
                java_tls_attacker
        ...
        try:
            server_process = spawn_server(
                botan_server_cmd + ["--policy=%s" % (bsi_policy_txt)]
            )
            time.sleep(1)
            run_subprocess(
                java_tls_attacker
