...
class RFC6979_KAT_Tests final : public Text_Based_Test {
   public:
      ...
      Test::Result run_one_test(const std::string& hash, const VarMap& vars) override {
         ...
         result.test_bn_eq("vector matches", Botan::generate_rfc6979_nonce(X, Q, H, hash), K);

         Botan::RFC6979_Nonce_Generator gen(hash, Q.bits(), X);

         result.test_bn_eq("vector matches", gen.nonce_for(Q, H), K);
         result.test_bn_ne("different output for H+1", gen.nonce_for(Q, H + 1), K);
         result.test_bn_eq("vector matches when run again", gen.nonce_for(Q, H), K);
