...
_DEFAULT_REPO_CONFIG_LOCATION = os.path.realpath(
    os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "..", "configs", "repo_config.env"
    )
)
...
class RepoConfig(dict):
    ...
    def __init__(self, env_file: str = _DEFAULT_REPO_CONFIG_LOCATION):
        ...
        parser = re.compile(
            r"^(?P<var>\w+)\s*=\s*((?P<val>[^\"]\S+)|\"(?P<quoted_val>\S+)\")\s*$"
        )
...
def main():
    ...
    def get_var(cfg: RepoConfig, args):
        if args.var not in cfg:
            print(
                f'Variable "{args.var}" not found in the configuration file.',
                file=sys.stderr,
            )
            raise KeyError()
        print(cfg[args.var])
    ...
    parser_all = subparsers.add_parser(
        "all", help="Print all variables and their values."
    )
