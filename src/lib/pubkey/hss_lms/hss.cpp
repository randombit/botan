...
std::shared_ptr<HSS_LMS_PrivateKeyInternal> HSS_LMS_PrivateKeyInternal::from_bytes_or_throw(
   ...
   if(!std::all_of(params.begin(), params.end(), [&hash_name](HSS_LMS_Params::LMS_LMOTS_Params_Pair& lms_lmots_params) {
         bool valid_lmots_hash = lms_lmots_params.lmots_params().hash_name() == hash_name;
         bool valid_lms_hash = lms_lmots_params.lms_params().hash_name() == hash_name;
         return valid_lmots_hash && valid_lms_hash;
      })) {
...
secure_vector<uint8_t> HSS_LMS_PrivateKeyInternal::sign(std::span<const uint8_t> msg) {
   ...
   for(int32_t layer_it = hss_params().L().get() - 1; layer_it >= 0; --layer_it) {
      ...
      if(layer == hss_params().L() - 1) {
         ...
         std::copy(current_pk.begin(), current_pk.end(), out_child_pk_buffer_at_layer.at(layer.get()).begin());
         current_pk =
            lms_key_at_layer.at(layer.get())
...
std::vector<uint8_t> HSS_LMS_PublicKeyInternal::to_bytes() const {
   std::vector<uint8_t> lms_pub_key_bytes = m_top_lms_pub_key.to_bytes();
   std::vector<uint8_t> bytes(sizeof(m_L) + lms_pub_key_bytes.size());
   BufferStuffer stuffer(bytes);

   stuffer.append(store_be(m_L));
   stuffer.append(lms_pub_key_bytes);
   BOTAN_ASSERT_NOMSG(stuffer.full());
