...
void pkcs12_kdf_with_hash(uint8_t out[],
   ...
   if(S_len > 0) {
      copy_repeat(std::span{I}.first(S_len), {salt, salt_len});
   }
   if(P_len > 0) {
      copy_repeat(std::span{I}.subspan(S_len, P_len), pwd_bytes);
   }
   ...
   while(out_offset < out_len) {
      ...
      copy_mem(std::span{out + out_offset, to_copy}, std::span{A}.first(to_copy));
      out_offset += to_copy;
...
std::unique_ptr<PasswordHash> PKCS12_KDF_Family::tune_params(size_t output_length,
   ...
   const size_t tuning_iterations = 64;
