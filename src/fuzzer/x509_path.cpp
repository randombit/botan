...
void fuzz(std::span<const uint8_t> in) {
   ...
   try {
      const Botan::X509_Certificate subject(input);
      const Botan::X509_Certificate issuer(input);

      std::vector<Botan::Certificate_Store*> roots;
      const std::unique_ptr<Botan::Certificate_Store> root_store(new Botan::Certificate_Store_In_Memory(issuer));
