...
class HMAC_DRBG_Unit_Tests : public Test
   ...
   public:
      ...
      Test::Result test_broken_entropy_input()
         ...
         Botan::HMAC_DRBG rng_with_broken_rng(std::move(mac), broken_entropy_input_rng, 16384);
         ...
         Botan::HMAC_DRBG rng_with_broken_es(std::move(mac), broken_entropy_sources, 16384);
         ...
         mac = Botan::MessageAuthenticationCode::create("HMAC(SHA-256)");
         Botan::HMAC_DRBG rng_with_insufficient_es(std::move(mac), insufficient_entropy_sources, 16384);
         ...
         Botan::HMAC_DRBG rng_with_broken_rng_and_es(std::move(mac), broken_entropy_input_rng,
               Botan::Entropy_Sources::global_sources(), 16384);
         ...
         mac = Botan::MessageAuthenticationCode::create("HMAC(SHA-256)");
         Botan::HMAC_DRBG rng_with_rng_and_broken_es(std::move(mac), Test::rng(), broken_entropy_sources, 16384);
         ...
         mac = Botan::MessageAuthenticationCode::create("HMAC(SHA-256)");
         Botan::HMAC_DRBG rng_with_broken_rng_and_broken_es(std::move(mac), broken_entropy_input_rng, broken_entropy_sources, 16384);
      ...
      Test::Result test_fork_safety()
         ...
         Request_Counting_RNG counting_rng;
         Botan::HMAC_DRBG rng(std::move(mac), counting_rng, 16384);
         ...
         if ( pid == -1 )
            ...
            read(fd[0], &child_bytes[0], parent_bytes.size());
