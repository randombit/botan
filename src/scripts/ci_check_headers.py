...
def main(args=None):
    ...
    with open(os.path.join(args[1]), encoding="utf8") as f:
        ...
        for header in build_config["public_headers"]:
            contents = open(
                os.path.join(public_include_dir, header), encoding="utf8"
            ).read()

            match = internal_inc.search(contents)
            if match:
                print(
                    "ERROR: Public header '%s' includes an internal header '%s'"
        ...
        for header in all_headers:
            ...
            if not match:
                print(
                    "ERROR: Header '%s' is missing an appropriate header guard"
                    % (header)
                )
                return 1
            ...
            if guard in header_guards:
                print(
                    "ERROR: Duplicate header guard (%s) in '%s' and '%s'"
                    % (guard, header, header_guards[guard])
                )
                return 1
            else:
