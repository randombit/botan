...
def verify_includes(build_config):
    ...
    expected_headers = set(
        build_config["public_headers"] + build_config["external_headers"]
    )
    found_headers = set([])
...
def verify_cmake_package(build_config):
    ...
    cmake_build_dir = os.path.join(
        build_config["abs_root_dir"], build_config["build_dir"], "cmake_test"
    )
    cmake_test_dir = os.path.join(
        build_config["abs_root_dir"], "src", "scripts", "ci", "cmake_tests"
    )

    def cmake_preset():
        if build_config["os"] == "windows":
            return (
                "windows_x86_64" if build_config["arch"] == "x86_64" else "windows_x86"
            )
        return "unix"
    ...
    disabled_module = (
        build_config["disabled_mod_list"][0]
        if build_config["disabled_mod_list"]
        else None
    )
    if not disabled_module:
        print(
            "Not a single disabled module in this build to use for testing."
    ...
    try:
        subprocess.run(
            [
                ...
                "-DBOTAN_DISABLED_MODULE=%s" % disabled_module,
            ],
            check=True,
        )
        subprocess.run(
            ["cmake", "--build", cmake_build_dir, "--config", "Release"], check=True
        )
        subprocess.run(
            [
                "cmake",
...
def main(args=None):
    ...
    has_cmake = (
        "botan_cmake_config" in build_config
        and "botan_cmake_version_config" in build_config
    )
