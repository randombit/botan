...
def cli_pkcs12_tests(tmp_dir):
    if not check_for_command("pkcs12_export") or not check_for_command("pkcs12_import"):
        logging.info("Skipping PKCS#12 CLI tests: pkcs12_export/pkcs12_import not available")
        return
    ...
    if "SHA-256 Fingerprint" not in import_info:
        logging.error("pkcs12_import info missing SHA-256 fingerprint: %s", import_info)
    ...
    info_legacy = test_cli("pkcs12_import", ["--pass=hunter2", pfx_legacy], None)
    if "ECDSA" not in info_legacy:
        logging.error("pkcs12_import (legacy cipher) missing key algorithm: %s", info_legacy)
    ...
    info_named = test_cli("pkcs12_import", ["--pass=hunter2", pfx_named], None)
    ...
    test_cli("pkcs12_import", ["--pass=hunter2", pfx_sha256mac], None)
    ...
    test_cli("pkcs12_import", ["--pass=hunter2", pfx_certenc], None)
    ...
    if not os.path.exists(chain_out):
        logging.error("pkcs12_import --chain-out did not produce a file")
    ...
    if not os.path.exists(out_key_enc):
        logging.error("pkcs12_import --out-key-pass: key file not written")
    else:
        with open(out_key_enc, encoding='utf-8') as f:
            key_pem = f.read()
        if "ENCRYPTED" not in key_pem:
            logging.error("pkcs12_import --out-key-pass: output key is not encrypted")
