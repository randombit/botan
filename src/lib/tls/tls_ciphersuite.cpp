...
bool Ciphersuite::cbc_ciphersuite() const {
   return (mac_algo() != "AEAD" && cipher_algo() != "NullCipher");
}

bool Ciphersuite::null_ciphersuite() const {
   return (cipher_algo() == "NullCipher");
}
...
bool Ciphersuite::is_usable() const {
   if(((cipher_algo() != "NullCipher") && (m_cipher_keylen == 0)) ||
      ((cipher_algo() == "NullCipher") && (m_cipher_keylen != 0))) {  // uninitialized object
      return false;
   }
   ...
   if(mac_algo() == "AEAD") {
      ...
      if(!have_cipher(cipher_algo()) && (cipher_algo() != "NullCipher")) {
         return false;
      }
