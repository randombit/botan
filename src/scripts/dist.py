...
def gpg_sign(keyid, passphrase_file, files, detached=True):
    ...
    for filename in files:
        ...
        gpg = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def parse_args(args):
    ...
    parser.add_option(
        "--verbose", action="store_true", default=False, help="Extra debug output"
    )

    parser.add_option(
        "--quiet", action="store_true", default=False, help="Only show errors"
    )
...
def rewrite_version_file(
    version_file, target_version, snapshot_branch, rev_id, rel_date
):
    if snapshot_branch:
        ...
        if not snapshot_branch:
            for req_var in ["major", "minor", "patch", "suffix"]:
                if req_var not in version_info:
                    raise Exception(
                        "Missing version field for %s in version file" % (req_var)
                    )

            marked_version = "%d.%d.%d%s" % (
                version_info["major"],
                version_info["minor"],
                version_info["patch"],
                version_info["suffix"],
            ...
            if marked_version != target_version:
                raise Exception(
                    "Release version file %s does not match tagged version %s"
...
def write_archive(
    version, output_basename, archive_type, rel_epoch, all_files, hash_file
):
    ...
    archive = tarfile.open(
        output_basename + archive_suffix,
    ...
    logging.info(
        "%s is %.2f MiB", output_archive, len(archive_contents) / (1024.0 * 1024.0)
    )
...
def main(args=None):
    ...
    logging.info(
        "Found %s at revision id %s released %d", target_version, rev_id, rel_date
    )
    ...
    def find_version_file():
        ...
        for possible_version_file in [
            "src/build-data/version.txt",
            "version.txt",
            "botan_version.py",
        ]:
    ...
    if options.pgp_key_id != "none":
        if options.write_hash_file is not None:
            output_files += gpg_sign(
                options.pgp_key_id,
                options.pgp_passphrase_file,
                [options.write_hash_file],
                False,
            )
        else:
            output_files += gpg_sign(
                options.pgp_key_id, options.pgp_passphrase_file, output_files, True
            )
