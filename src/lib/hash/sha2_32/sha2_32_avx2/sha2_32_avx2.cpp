...
BOTAN_FN_ISA_AVX2_BMI2 BOTAN_SCRUB_STACK_AFTER_RETURN void SHA_256::compress_digest_x86_avx2(
   ...
   while(blocks >= 2) {
      ...
      for(size_t r = 0; r != 48; r += 16) {
         auto w = next_w(WS) + SIMD_8x32::load_le128(&SHA256_K[r + 16]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&SHA256_K[r + 20]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&SHA256_K[r + 24]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&SHA256_K[r + 28]);
   ...
   while(blocks > 0) {
      ...
      for(size_t r = 0; r != 48; r += 16) {
         auto w = next_w(WS) + SIMD_4x32::load_le(&SHA256_K[r + 16]);
         ...
         w = next_w(WS) + SIMD_4x32::load_le(&SHA256_K[r + 20]);
         ...
         w = next_w(WS) + SIMD_4x32::load_le(&SHA256_K[r + 24]);
         ...
         w = next_w(WS) + SIMD_4x32::load_le(&SHA256_K[r + 28]);
