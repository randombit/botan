...
void EC_AffinePoint_Data_PC::serialize_x_to(std::span<uint8_t> bytes) const {
   ...
   copy_mem(bytes, std::span{m_bytes}.subspan(1, fe_bytes));
}

void EC_AffinePoint_Data_PC::serialize_y_to(std::span<uint8_t> bytes) const {
   const size_t fe_bytes = this->field_element_bytes();
   BOTAN_ARG_CHECK(bytes.size() == fe_bytes, "Invalid output size");
   copy_mem(bytes, std::span{m_bytes}.last(fe_bytes));
}
...
std::unique_ptr<EC_Scalar_Data> EC_Mul2Table_Data_PC::mul2_vartime_x_mod_order(const EC_Scalar_Data& xd,
                                                                               const EC_Scalar_Data& yd) const {
   BOTAN_ARG_CHECK(xd.group() == m_group && yd.group() == m_group, "Curve mismatch");
   ...
   if(auto s = m_group->pcurve().mul2_vartime_x_mod_order(*m_tbl, x.value(), y.value())) {
      return std::make_unique<EC_Scalar_Data_PC>(m_group, std::move(*s));
   } else {
      return nullptr;
   }
}
