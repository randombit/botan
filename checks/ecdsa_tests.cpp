...
Botan::ECDSA_PrivateKey priv_key(dom_pars);
  ...
  auto_ptr<Botan::PK_Signer> pk_signer = Botan::get_pk_signer(priv_key, format);
    Botan::SecureVector<Botan::byte> signature;
  bool sig_exc = false;
  try
  {
    signature = pk_signer->sign_message(message);
  }
  ...
  if(i==0) // makes no sense to check for sha224
    {
    auto_ptr<Botan::PK_Verifier> pk_verifier = Botan::get_pk_verifier(priv_key, format);
 ...
 // sign it with the normal EMSA1
  auto_ptr<Botan::PK_Signer> pk_signer = Botan::get_pk_signer(priv_key, "EMSA1(SHA-224)");
  Botan::SecureVector<Botan::byte> signature = pk_signer->sign_message(message);

    auto_ptr<Botan::PK_Verifier> pk_verifier = Botan::get_pk_verifier(priv_key, "EMSA1_BSI(SHA-224)");
...
BOOST_AUTO_TEST_CASE( test_message_larger_than_n)
{
  ...
  try
    {
      signature = priv_key.sign(sv_message.begin(), sv_message.size());
    }
  ...
  try
    {
      ...
      Botan::Pipe pipe(Botan::create_shared_ptr<Botan::PK_Signer_Filter>(sp_dsa_sig), Botan::create_shared_ptr<Botan::Base64_Encoder>());
  ...
  try
    {
      ...
      Botan::RSA_PrivateKey rsa_key(1024);
...
//    cout << "create_and_verify started" << endl;
    ...
    auto_ptr<Botan::PKCS8_PrivateKey> loaded_key(Botan::PKCS8::load_key("checks/temp/wo_dompar_private.pkcs8.pem"));
    ...
    auto_ptr<Botan::PKCS8_PrivateKey> loaded_key_1(Botan::PKCS8::load_key("checks/temp/rsa_private.pkcs8.pem"));
...

  {
    ...
    PointGFp p_G = OS2ECP ( sv_G_secp_comp, curve );
    {
      ...
      Botan::ECDSA_PrivateKey key(dom_params);
      ofstream priv_key("checks/temp/ec_oid_not_in_reg_private.pkcs8.pem");
      priv_key << Botan::PKCS8::PEM_encode(key);
    }

    auto_ptr<Botan::PKCS8_PrivateKey> loaded_key(Botan::PKCS8::load_key("checks/temp/ec_oid_not_in_reg_private.pkcs8.pem"));
    Botan::ECDSA_PrivateKey* loaded_ec_key = dynamic_cast<Botan::ECDSA_PrivateKey*>(loaded_key.get());
    BOOST_CHECK_MESSAGE(loaded_ec_key, "the loaded key could not be converted into an ECDSA_PrivateKey");
  }
  ...
  for (i = 0; i < oids.size(); i++)
    {
      ...
      string str_message = ("12345678901234567890abcdef12");
      Botan::SecureVector<Botan::byte> sv_message = decode_hex(str_message);
      Botan::SecureVector<Botan::byte> signature = key.sign(sv_message.begin(), sv_message.size());
  ...
  try
    {
      ...
      string str_message = ("12345678901234567890abcdef12");
      Botan::SecureVector<Botan::byte> sv_message = decode_hex(str_message);
      Botan::SecureVector<Botan::byte> signature = loaded_ec_key->sign(sv_message.begin(), sv_message.size());
      //cout << "signature = " << hex_encode(signature.begin(), signature.size()) << "\n";
      bool ver_success = loaded_ec_key->verify(sv_message.begin(), sv_message.size(), signature.begin(), signature.size());
      BOOST_CHECK_MESSAGE(ver_success, "generated signature could not be verified positively");

      auto_ptr<Botan::PKCS8_PrivateKey> loaded_key_nodp(Botan::PKCS8::load_key("checks/testdata/nodompar_private.pkcs8.pem"));
      ...
      Botan::SecureVector<Botan::byte> signature_nodp = loaded_ec_key_nodp->sign(sv_message.begin(), sv_message.size());
      //cout << "signature = " << hex_encode(signature.begin(), signature.size()) << "\n";
      ...
      try
        {
          auto_ptr<Botan::PKCS8_PrivateKey> loaded_key_withdp(Botan::PKCS8::load_key("checks/testdata/withdompar_private.pkcs8.pem"));
...
BOOST_AUTO_TEST_CASE( test_cp_and_as_ctors )
{
  ...
  string str_message = ("12345678901234567890abcdef12");
  Botan::SecureVector<Botan::byte> sv_message = decode_hex(str_message);
  Botan::SecureVector<Botan::byte> signature_1 = loaded_ec_key->sign(sv_message.begin(), sv_message.size());
  //cout << "signature = " << hex_encode(signature.begin(), signature.size()) << "\n";

  Botan::ECDSA_PrivateKey cp_priv_key(*loaded_ec_key); // priv-key, cp-ctor
  Botan::SecureVector<Botan::byte> signature_2 = cp_priv_key.sign(sv_message.begin(), sv_message.size());

  Botan::ECDSA_PrivateKey as_priv_key = *loaded_ec_key;  //priv-key, as-op
  Botan::SecureVector<Botan::byte> signature_3 = as_priv_key.sign(sv_message.begin(), sv_message.size());
...
BOOST_AUTO_TEST_CASE( test_non_init_ecdsa_keys )
{
  ...
  try
    {
      Botan::SecureVector<Botan::byte> signature_1 = empty_priv.sign(sv_message.begin(), sv_message.size());
    }
