...
class PolynomialSampler {
   public:
      ...
      KyberPolyVec sample_polynomial_vector_cbd_eta1() {
         ...
         sample_polys_cbd(vec.polynomials(), m_mode.eta1());
         return vec;
      }
      ...
      std::pair<KyberPolyVec, KyberPolyVec> sample_polynomial_vector_pair_cbd_eta1() {
         ...
         sample_polys_cbd(concat(result.first.polynomials(),  //
                                 result.second.polynomials()),
                          m_mode.eta1());
         return result;
      }
         ...
         requires std::same_as<KyberEncryptionRandomness, SeedT>
      {
         ...
         sample_polys_cbd(concat(result.first.polynomials(), std::array{&result.second}), m_mode.eta2());
         return result;
      }
   ...
   private:
      ...
      void sample_polys_cbd(std::span<KyberPoly* const> polys, KyberConstants::KyberEta eta) {
         ...
         std::vector<StrongSpan<KyberSamplingRandomness>> outputs(polys.size());
         for(size_t i = 0; i < polys.size(); ++i) {
            outputs[i] = StrongSpan(randomness).subspan(i * poly_bytes, poly_bytes);
         }

         const auto& sym = m_mode.symmetric_primitives();
         sym.PRF_batch(outputs, m_seed, m_nonce);
         m_nonce += static_cast<uint8_t>(polys.size());

         for(size_t i = 0; i < polys.size(); ++i) {
            sample_polynomial_from_cbd(*polys[i], eta, outputs[i]);
         }
      }
