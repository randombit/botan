...
class SPAKE2_RT_Tests final : public Test {
   ...
   private:
      static bool spake2_test_run(Botan::RandomNumberGenerator& rng,
         ...
         Botan::SPAKE2::Parameters a_params(group, a_secret, a_id, b_id, a_context, hash_fn, per_user_params);
         Botan::SPAKE2::Context a_ctx(Botan::SPAKE2::PeerId::PeerA, a_params, rng);
         const auto a_msg = a_ctx.generate_message();

         Botan::SPAKE2::Parameters b_params(group, b_secret, a_id, b_id, b_context, hash_fn, per_user_params);
         Botan::SPAKE2::Context b_ctx(Botan::SPAKE2::PeerId::PeerB, b_params, rng);
         const auto b_msg = b_ctx.generate_message();
