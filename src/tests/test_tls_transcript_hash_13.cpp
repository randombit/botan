...
std::vector<Test::Result> transcript_hash()
   ...
   const auto sha256_truncated_ch = Botan::hex_decode(
      "63224b2e4573f2d3454ca84b9d009a04f6be9e05711a8396473aefa01e924a14");
   const auto sha256_full_ch = Botan::hex_decode(
   ...
   return
      ...
      Botan_Tests::CHECK("update without an algorithm", [](Test::Result& result)
         ...
         result.test_no_throw("update is successful", [&] { h.update(std::vector<uint8_t>{0xba, 0xad, 0xbe, 0xef}); });
         result.test_throws<Botan::Invalid_State>("previous throws invalid state exception",
               [&] { h.previous(); });
         result.test_throws<Botan::Invalid_State>("current throws invalid state exception",
               [&] { h.current(); });
      ...
      Botan_Tests::CHECK("update and result retrieval (algorithm is set)", [&](Test::Result& result)
         ...
         h.update(std::vector<uint8_t>{0xba, 0xad, 0xbe, 0xef});
         result.test_throws<Botan::Invalid_State>("previous throws invalid state exception",
               [&] { h.previous(); });
         result.test_eq("c = SHA-256(baadbeef)", h.current(), sha256("baadbeef"));

         h.update(std::vector<uint8_t>{0x60, 0x0d, 0xf0, 0x0d});
      ...
      Botan_Tests::CHECK("cloning creates independent transcript_hash instances", [&](Test::Result& result)
         ...
         h1.update(std::vector<uint8_t>{0xca, 0xfe, 0xd0, 0x0d});
      ...
      Botan_Tests::CHECK("recreation after hello retry request", [&](Test::Result& result)
         ...
         h1.update(std::vector<uint8_t>{0xc0, 0xca, 0xc0, 0x1a} /* client hello 1 */);
         h1.update(std::vector<uint8_t>{0xc0, 0x01, 0xf0, 0x0d} /* hello retry request */);
      ...
      Botan_Tests::CHECK("truncated transcript hash in client hellos with PSK", [&](Test::Result& result)
         {
         Transcript_Hash_State h1;
