...
def main(args = None):
    ...
    def gates_for_suite(info):
        ...
        if kex_algo in ('ECDH', 'ECDHE_PSK'):
            ...
            feat.append('DIFFIE_HELLMAN')
        elif kex_algo == 'STATIC_RSA':
            feat.append('RSA')
            feat.append('PKCSV15_ENCRYPTION_PADDING')
        ...
        # Cipher + mode
        if cipher_algo == 'NULL':
            feat.append('TLS_NULL')
        elif cipher_algo == 'ChaCha20Poly1305':
            feat.append('AEAD_CHACHA20_POLY1305')
        else:
            if '/' in cipher_algo:
            ...
            if base.startswith('AES-'):
                ...
                feat.append('CAMELLIA')
            elif base == '3DES':
                feat.append('DES')

            if mode == 'GCM':
                feat.append('AEAD_GCM')
            elif mode in ('CCM', 'CCM(8)'):
                feat.append('AEAD_CCM')
            elif mode.startswith('OCB'):
                feat.append('AEAD_OCB')
            elif mode == 'CBC':
                feat.append('TLS_CBC')
        ...
        if mac_algo != 'AEAD':
            feat.append(hash_macro[mac_algo])
        feat.append(hash_macro[prf_algo])
    ...
    for code in sorted(suites.keys()):
        ...
        rendered_suites.append({
            ...
            'kex_algo': info[3],
            'cipher_algo': info[4],
            'cipher_keylen': int(info[5]),
            'mac_algo': info[6],
            'mac_keylen': info[7],
            'kdf_algo': info[8].replace('-', '_'),
            'nonce_format': info[9],
            'gates_expr': ' && '.join('defined(%s)' % g for g in gates),
        })

    env = Environment(loader=FileSystemLoader('src/build-data/templates'))
    template = env.get_template('tls_suite_info.cpp.in')
    suite_info = template.render(
        script=sys.argv[0],
