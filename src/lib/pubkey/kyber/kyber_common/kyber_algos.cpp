...
template <size_t d>
   requires(d < 12)
void kyber_poly_compress_and_encode(BufferStuffer& bs, const KyberPoly& p) {
   CRYSTALS::pack<(1 << d) - 1>(p, bs, kyber_compress<d>);
}

/**
 * NIST FIPS 203 IPD, Algorithm 4 (ByteEncode) for d == 12
 */
void kyber_byte_encode(BufferStuffer& bs, const KyberPolyNTT& p) {
...
template <size_t d>
   requires(d < 12)
void kyber_poly_decode_and_decompress(KyberPoly& p, BufferSlicer& bs) {
   CRYSTALS::unpack<(1 << d) - 1>(p, bs, kyber_decompress<d>);
}

/**
 * NIST FIPS 203 IPD, Algorithm 5 (ByteDecode) for d == 12
 */
void kyber_byte_decode(KyberPolyNTT& p, BufferSlicer& bs) {
 ...
 */
void kyber_sample_ntt_uniform(KyberPolyNTT& p, XOF& xof) {
...
void kyber_encode_polynomial_vector(std::span<uint8_t> out, const KyberPolyVecNTT& vec) {
   BufferStuffer bs(out);
   for(auto& v : vec) {
      kyber_byte_encode(bs, v);
   }
...
KyberPoly kyber_polynomial_from_message(StrongSpan<const KyberMessage> msg) {
   ...
   KyberPoly r;
   BufferSlicer bs(msg);
   kyber_poly_decode_and_decompress<1>(r, bs);
   return r;
}
...
KyberMessage kyber_polynomial_to_message(const KyberPoly& p) {
   ...
   kyber_poly_compress_and_encode<1>(bs, p);
   return result;
}
...
void polyvec_compress_and_encode(BufferStuffer& sink, const KyberPolyVec& polyvec) {
   for(const auto& p : polyvec) {
      kyber_poly_compress_and_encode<d>(sink, p);
   }
}
...
void compress_poly(std::span<uint8_t> out, const KyberPoly& p, const KyberConstants& mode) {
   ...
   switch(mode.d_v()) {
      case KyberConstants::KyberDv::_4:
         kyber_poly_compress_and_encode<4>(bs, p);
         BOTAN_ASSERT_NOMSG(bs.full());
         return;
      case KyberConstants::KyberDv::_5:
         kyber_poly_compress_and_encode<5>(bs, p);
         BOTAN_ASSERT_NOMSG(bs.full());
         return;
   }
...
void polyvec_decode_and_decompress(KyberPolyVec& polyvec, BufferSlicer& source) {
   for(auto& p : polyvec) {
      kyber_poly_decode_and_decompress<d>(p, source);
   }
}
...
KyberPoly kyber_decompress_polynomial(std::span<const uint8_t> buffer, const KyberConstants& mode) {
   ...
   switch(mode.d_v()) {
      case KyberConstants::KyberDv::_4:
         kyber_poly_decode_and_decompress<4>(r, bs);
         BOTAN_ASSERT_NOMSG(bs.empty());
         return r;
      case KyberConstants::KyberDv::_5:
         kyber_poly_decode_and_decompress<5>(r, bs);
         BOTAN_ASSERT_NOMSG(bs.empty());
         return r;
   }
...
std::pair<KyberPolyVec, KyberPoly> kyber_decompress_ciphertext(StrongSpan<const KyberCompressedCiphertext> ct,
   ...
   return {decompress_polynomial_vector(pv, mode), kyber_decompress_polynomial(p, mode)};
}
...
KyberPolyMat kyber_sample_matrix(StrongSpan<const KyberSeedRho> seed, bool transposed, const KyberConstants& mode) {
   ...
   for(uint8_t i = 0; i < mode.k(); ++i) {
      for(uint8_t j = 0; j < mode.k(); ++j) {
         ...
         kyber_sample_ntt_uniform(mat[i][j], mode.symmetric_primitives().XOF(seed, pos));
      }
   }
