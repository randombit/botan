...
class BSD_SocketUDP final : public OS::SocketUDP {
   public:
      BSD_SocketUDP(std::string_view hostname, std::string_view service, std::chrono::microseconds timeout) :
            m_timeout(timeout), m_socket(Botan::OS::Socket_Platform::invalid_socket()) {
         addrinfo hints;
         clear_mem(&hints, 1);
      ...
      size_t read(uint8_t buf[], size_t len) override {
         ...
         const ssize_t got = recv(m_socket, reinterpret_cast<char*>(buf), static_cast<sendrecv_len_type>(len), 0);
         if(got < 0) {
