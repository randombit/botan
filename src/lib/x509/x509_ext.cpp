...
IPAddressBlocks::IPAddressFamily IPAddressBlocks::merge(std::vector<IPAddressFamily>& blocks) {
   ...
   if(!all_inherit && !none_inherit) {
      throw Exception("Only one of \"inherit\" / \"don't inherit\" is allowed per family");
   }
...
std::optional<std::vector<T>> sort_and_merge_ranges(std::optional<std::span<const T>> ranges) {
   ...
   // This process is repeated until there are no further changes, because multiple adjacent elements may need to be combinded one by one.
   ...
   while(changed) {
      ...
      for(size_t i = 0; i < current.size() - 1; i++) {
         ...
         // they are either adjacent or overlap
...
template <typename T>
bool validate_subject_in_issuer(const std::vector<T>& subject, const std::vector<T>& issuer) {
   ...
   for(auto subj = subject.begin(), issu = issuer.begin(); subj != subject.end();) {
      // the smallest subject range is already outside the smallest issuer range, return immediately
      if(subj->min() < issu->min()) {
         return false;
      }
      // the issuer range is smaller than the subject range, step to the next issuer range
      ...
      // we are at the closest issuer range, but it does not enclose the subject range
      if(subj->min() < issu->min() || subj->max() > issu->max()) {
         return false;
      }
      subj++;
   }
...
void IPAddressBlocks::IPAddressOrRange<V>::decode_from(Botan::BER_Decoder& from) {
   ...
   if(next_tag == ASN1_Type::BitString) {
      ...
      const uint8_t unused = addr_max.front();
...
void IPAddressBlocks::validate(const X509_Certificate& /* unused */,
   ...
   if(pos == cert_path.size() - 1) {
      ...
      auto valiate_root_cert_ext = [&](const auto& map) {
      ...
      if(valiate_root_cert_ext(v4_needs_check) || valiate_root_cert_ext(v6_needs_check)) {
         cert_status.at(pos).insert(Certificate_Status_Code::IPADDR_BLOCK_ERROR);
      }
      return;
   }
      ...
      auto validate_against_issuer = [&](auto& subject_map, const auto& issuer_map) {
         for(auto map_it = subject_map.begin(); map_it != subject_map.end(); map_it++) {
            ...
            if(needs_check && issuer_has_value) {
               if(!validate_subject_in_issuer(*subject_value, *issuer_value)) {
                  cert_status.at(pos).insert(Certificate_Status_Code::IPADDR_BLOCK_ERROR);
                  return false;
               }
               needs_check = false;
...
void ASBlocks::validate(const X509_Certificate& /* unused */,
   ...
   if(pos == cert_path.size() - 1) {
      ...
      if((asnum_present && !asnum_needs_check) || (rdi_present && !rdi_needs_check)) {
         cert_status.at(pos).insert(Certificate_Status_Code::ASBLOCKS_ERROR);
      }
      return;
   }
   ...
   for(auto it = cert_path.begin() + pos + 1; it != cert_path.end(); it++) {
      ...
      if(asnum_needs_check && as_identifiers.asnum().value().ranges().has_value()) {
         ...
         if(!validate_subject_in_issuer(subject_asnums, issuer_asnums)) {
            cert_status.at(pos).insert(Certificate_Status_Code::ASBLOCKS_ERROR);
            return;
         }
      ...
      if(rdi_needs_check && as_identifiers.rdi().value().ranges().has_value()) {
         ...
         if(!validate_subject_in_issuer(subject_rdis, issuer_rdis)) {
            cert_status.at(pos).insert(Certificate_Status_Code::ASBLOCKS_ERROR);
            return;
         }
