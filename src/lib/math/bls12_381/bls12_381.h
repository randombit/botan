...
*/
class BOTAN_PUBLIC_API(3, 14) Scalar final {
   public:
      /** The bit length of the group order r */
      static constexpr size_t BITS = 255;

      /** The byte length of the canonical scalar encoding */
      static constexpr size_t BYTES = (BITS + 7) / 8;

      /** The word length of the internal representation */
      static constexpr size_t N = (BYTES + sizeof(word) - 1) / sizeof(word);

      static_assert(BYTES == N * sizeof(word));

      /** Default constructed as zero */
      ...
      */
      ...
      /** Move assignment; the moved-from scalar is zeroized */
      ...
      /** Return the scalar equal to the small integer v */
      ...
      */
      ...
      /**
      * Reduce a 64 byte big-endian integer modulo r
      *
      * Suitable for deriving a uniformly distributed scalar from an
      * RNG or hash output
      */
      ...
      */
      ...
      /** Return the multiplicative inverse of this scalar, or zero for zero */
      Scalar invert() const;

      /** Return the square of this scalar */
      Scalar square() const;
...
/**
* An element of the group G1, in affine form
*
* Group arithmetic is performed on G1Projective; the affine form is
* used for serialization, pairings, and mixed addition
*/
class BOTAN_PUBLIC_API(3, 14) G1Affine final {
   public:
      /** The byte length of the compressed point encoding */
      ...
      *
      * This rejects points not in the prime order subgroup. The canonical
      * encoding of the identity element is accepted; callers for which the
      * identity is not a legal value (such as BLS public keys) must check
      * is_identity() themselves.
      */
...
/**
* An element of the group G1, in projective form
*/
class BOTAN_PUBLIC_API(3, 14) G1Projective final {
   public:
      /**
      ...
      */
      ...
      /** Convert to affine form, at the cost of a field inversion */
      ...
      /** Return the sum of this point and an affine point */
      G1Projective add_mixed(const G1Affine& other) const;

      /**
      * Scalar multiplication
      *
      * Constant time; safe for secret scalars
      */
   ...
   private:
      ...
      G1Projective add_mixed(const G1Affine& other, bool negate_other) const;

      void conditional_negate(uint32_t negate);
...
/**
* An element of the group G2, in affine form
*
* Group arithmetic is performed on G2Projective; the affine form is
* used for serialization, pairings, and mixed addition
*/
class BOTAN_PUBLIC_API(3, 14) G2Affine final {
   public:
      /** The byte length of the compressed point encoding */
      ...
      * This rejects points not in the prime order subgroup. The canonical
      * encoding of the identity element is accepted; callers for which the
      * identity is not a legal value (such as BLS signatures) must check
      * is_identity() themselves.
      */
...
/**
* An element of the group G2, in projective form
*/
class BOTAN_PUBLIC_API(3, 14) G2Projective final {
   public:
      /**
      * Default constructed as the identity element
      */
   ...
   private:
      ...
      G2Projective add_mixed(const G2Affine& other, bool negate_other) const;
...
*/
class BOTAN_PUBLIC_API(3, 14) Gt final {
   public:
      /** The byte length of the Gt serialization */
      ...
      *
      * More efficient than computing each pairing individually, since the
      * final exponentiation is shared. The empty product yields the identity.
      *
