...
def curve_info(src):
    ...
    for line in src:
        ...
        if key == "N":
            ...
            current["OIDExpr"] = [
                "OID{%s}" % (oid.replace(".", ", ")) for oid in current["OID"]
            ]
            yield current
            current = {}
...
def main():
    ...
    # write ec_named.cpp
    with open(
        "./src/lib/pubkey/ec_group/ec_named.cpp", encoding="utf8", mode="w"
    ) as ec_named:
    ...
    with open(
        "./src/lib/math/pcurves/pcurves_instance.h", encoding="utf8", mode="w"
    ) as pcurves_h:
    ...
    # write pcurves.cpp
    with open(
        "./src/lib/math/pcurves/pcurves.cpp", encoding="utf8", mode="w"
    ) as pcurves_cpp:
        template = env.get_template("pcurves.cpp.in")
        pcurves_cpp.write(
            template.render(script=this_script, date=date, pcurves=pcurves)
        )
        pcurves_cpp.write("\n")
    ...
    for pcurve in pcurves:
        ...
        addchain_fe_sqrt = (
            addchain_code((pcurve["P"] + 1) // 4, 0) if pcurve["P"] % 4 == 3 else None
        )
        ...
        if not re.match("^[0-9A-Za-z_]{3,30}$", module_define):
            raise ValueError(
                f"Invalid preprocessor define name ({module_define}) for new pcurve module"
            )
        ...
        with open(impl_path, "w", encoding="utf8") as src_file:
            ...
            src_file.write(
                template.render(
                    ...
                    addchain_fe_sqrt=(
                        indent(addchain_fe_sqrt, 9 * " ", OmitFirstLine())
                        if addchain_fe_sqrt
                        else None
                    ),
