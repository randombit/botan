...
Kyber_PublicKeyInternal::Kyber_PublicKeyInternal(KyberConstants mode, KyberPolyVecNTT t, KyberSeedRho rho) :
      ...
      m_t(std::move(t)),
      m_rho(std::move(rho)),
      m_public_key_bits_raw(concat(kyber_encode_polynomial_vector<std::vector<uint8_t>>(m_t, m_mode), m_rho)),
...
void Kyber_PublicKeyInternal::indcpa_encrypt(StrongSpan<KyberCompressedCiphertext> out_ct,
   ...
   KyberPolynomialSampler ps(r, m_mode);
   ...
   const auto mu = kyber_polynomial_from_message(m);
   auto v = inverse_ntt(m_t * rv);
   v += e2;
   v += mu;
   v.reduce();

   kyber_compress_ciphertext(out_ct, u, v, m_mode);
}
...
KyberMessage Kyber_PrivateKeyInternal::indcpa_decrypt(StrongSpan<const KyberCompressedCiphertext> ct) const {
   auto [u, v] = kyber_decompress_ciphertext(ct, m_mode);
   v -= inverse_ntt(m_s * ntt(std::move(u)));
   v.reduce();
   return kyber_polynomial_to_message(v);
}
