...
int main() {
   ...
   const Botan::ECDH_PrivateKey priv_key_sw(rng, Botan::EC_Group::from_name("secp256r1"));
   ...
   // import to card
   const Botan::PKCS11::PKCS11_ECDH_PrivateKey priv_key(session, priv_import_props);
   ...
   // import
   const Botan::PKCS11::PKCS11_ECDH_PublicKey pub_key(session, pub_import_props);

   /************ export ECDH private key *************/
   const Botan::ECDH_PublicKey exported_pub = pub_key.export_key();
   ...
   const Botan::PKCS11::PKCS11_ECDH_PrivateKey priv_key2(
   ...
   const Botan::PKCS11::PKCS11_ECDH_KeyPair key_pair =
      Botan::PKCS11::generate_ecdh_keypair(session, pub_generate_props, priv_generate_props);

   /************ ECDH derive *************/

   const Botan::PKCS11::PKCS11_ECDH_KeyPair key_pair_other =
      Botan::PKCS11::generate_ecdh_keypair(session, pub_generate_props, priv_generate_props);

   const Botan::PK_Key_Agreement ka(key_pair.second, rng, "Raw", "pkcs11");
   const Botan::PK_Key_Agreement kb(key_pair_other.second, rng, "Raw", "pkcs11");

   const Botan::SymmetricKey alice_key = ka.derive_key(32, key_pair_other.first.raw_public_key_bits());

   const Botan::SymmetricKey bob_key = kb.derive_key(32, key_pair.first.raw_public_key_bits());
