...
secure_vector<uint8_t> pkcs12_password_to_bytes(std::string_view password) {
   ...
   // Convert UTF-8 password to UCS-2 (big-endian) per RFC 7292
   const std::vector<uint8_t> ucs2 = Botan::utf8_to_ucs2(std::string(password));
...
*/
void copy_repeat(uint8_t* buf, size_t n, const uint8_t* data, size_t data_len) {
   if(data_len == 0) {
      clear_mem(buf, n);
      return;
   }
   ...
   size_t pos = 0;
   while(pos < n) {
      const size_t to_copy = std::min(data_len, n - pos);
      copy_mem(buf + pos, data, to_copy);
      pos += to_copy;
   }
}
...
void pkcs12_kdf(uint8_t out[],
   ...
   if(S_len > 0) {
      copy_repeat(I.data(), S_len, salt, salt_len);
   }
   if(P_len > 0) {
      copy_repeat(I.data() + S_len, P_len, pwd_bytes.data(), pwd_len);
   }
