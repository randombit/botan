...
Record_Layer::Record_Layer(Connection_Side side)
   ...
   , m_first_sent_record(true)
   , m_first_rcvd_record(true) {}
...
std::vector<uint8_t> Record_Layer::prepare_records(const Record_Type type,
   ...
   do
      ...
      const auto use_compatibility_version = m_side == Connection_Side::CLIENT && m_first_sent_record;
...
Record_Layer::ReadResult<Record> Record_Layer::next_record(Cipher_State* cipher_state)
   ...
   // The first received record is likely a client or server hello. To be able to
   // perform protocol downgrades we need must be less vigorous with the record's
   // legacy version. Hence, `check_tls13_version` is `false` for the first record.
   TLSPlaintext_Header plaintext_header({header_begin, header_end}, !m_first_rcvd_record);
