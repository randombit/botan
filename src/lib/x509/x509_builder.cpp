...
class CertificateParametersBuilder::State final {
   public:
      ...
      Extensions finalize_extensions(const Public_Key& key) const {
         ...
         const auto san_oid = OID::from_name("X509v3.SubjectAlternativeName");
         BOTAN_ASSERT_NOMSG(san_oid.has_value());
         const bool critical_san = extensions.critical_extension_set(*san_oid);

         extensions.replace(setup_alt_name(extensions), critical_san);

         // If the extension was set explicitly via add_extension then this is ignored:
         extensions.add_new(std::make_unique<Cert_Extension::Basic_Constraints>(m_is_ca_request, m_path_limit), true);
         ...
         if(!usage.empty()) {
            ...
            if(extensions.extension_set(Cert_Extension::Key_Usage::static_oid())) {
               throw Invalid_Argument("Key usage extension can be set via add_allowed_usage or extension but not both");
            }
         ...
         if(!m_ext_usage.empty()) {
            if(extensions.extension_set(Cert_Extension::Extended_Key_Usage::static_oid())) {
               throw Invalid_Argument(
                  "Extended key usage can be set via add_allowed_extended_usage or extension but not both");
            }
            // If the extension was set explicitly via add_extension then this is ignored:
...
X509_Certificate CertificateParametersBuilder::into_self_signed_cert(const ASN1_Time& not_before,
...
X509_Certificate CertificateParametersBuilder::into_cert(const ASN1_Time& not_before,
                                                         const ASN1_Time& not_after,
   ...
   Extensions extensions = m_state->finalize_extensions(pubkey);

   const std::vector<uint8_t> pub_key = pubkey.subject_public_key();
   auto skid = std::make_unique<Cert_Extension::Subject_Key_ID>(pubkey);
...
CertificateParametersBuilder& CertificateParametersBuilder::add_email(std::string_view email) {
   if(auto parsed = EmailAddress::from_string(email)) {
      m_state->add_email(parsed.value());
      return (*this);
   } else {
      throw Invalid_Argument("Could not parse email address string");
   }
}
...
CertificateParametersBuilder& CertificateParametersBuilder::add_uri(std::string_view uri) {
   if(auto parsed = URI::from_string(uri)) {
      m_state->add_uri(parsed.value());
      return (*this);
   } else {
      throw Invalid_Argument("Could not parse URI string");
   }
}
...
CertificateParametersBuilder& CertificateParametersBuilder::add_dns(std::string_view name) {
   if(auto parsed = DNSName::from_string(name)) {
      m_state->add_dns(parsed.value());
      return (*this);
   } else {
      throw Invalid_Argument("Could not parse DNS name");
   }
}
