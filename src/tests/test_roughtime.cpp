   ...
   public:
      ...
      Test::Result run_one_test(const std::string& type, const VarMap& vars) override {
         ...
         result.test_eq(
            "encode",
            type == "Valid",
            request == Botan::typecast_copy<std::array<uint8_t, 1024>>(std::span<const uint8_t>{request_v}));
...
class Roughtime final : public Test {
   private:
      static Test::Result test_nonce(Botan::RandomNumberGenerator& rng) {
         ...
         result.confirm(
            "nonce from vector",
            nonce_v.get_nonce() == Botan::typecast_copy<std::array<uint8_t, 64>>(std::span<const uint8_t>{rand64}));
         Botan::Roughtime::Nonce nonce_a(
            Botan::typecast_copy<std::array<uint8_t, 64>>(std::span<const uint8_t>{rand64}));
         result.confirm(
            "nonce from array",
            nonce_v.get_nonce() == Botan::typecast_copy<std::array<uint8_t, 64>>(std::span<const uint8_t>{rand64}));
         rand64.push_back(10);
      ...
      static Test::Result test_chain(Botan::RandomNumberGenerator& rng) {
         ...
         result.confirm("empty chain nonce is blind",
                        c1.next_nonce(nonce_v).get_nonce() ==
                           Botan::typecast_copy<std::array<uint8_t, 64>>(std::span<const uint8_t>{rand64}));
