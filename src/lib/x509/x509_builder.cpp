...
class CertificateParametersBuilder::State final {
   public:
      ...
      void add_common_name(std::string_view cn) { add_subject_dn("X520.CommonName", cn); }

      void add_country(std::string_view country) { add_subject_dn("X520.Country", country); }

      void add_state(std::string_view state) { add_subject_dn("X520.State", state); }

      void add_locality(std::string_view locality) { add_subject_dn("X520.Locality", locality); }

      void add_serial_number(std::string_view sn) { add_subject_dn("X520.SerialNumber", sn); }

      void add_org(std::string_view org) { add_subject_dn("X520.Organization", org); }

      void add_organizational_unit(std::string_view org_unit) { add_subject_dn("X520.OrganizationalUnit", org_unit); }

      void add_extension(std::unique_ptr<Certificate_Extension> extn, bool is_critical) {
         m_extensions.add_new(std::move(extn), is_critical);
      }
      ...
      void set_as_ca_certificate(std::optional<size_t> path_limit) {
         BOTAN_STATE_CHECK(!m_is_ca_request);
         m_is_ca_request = true;
         m_path_limit = path_limit;
      }

   private:
      void add_subject_dn(std::string_view attr, std::string_view value) {
         BOTAN_ARG_CHECK(!value.empty(), "Empty DN value is invalid");
         m_subject_dn.add_attribute(attr, value);
      }
...
X509_Certificate CertificateParametersBuilder::into_self_signed_cert(std::chrono::system_clock::time_point not_before,
   ...
   auto signer_p = X509_Object::choose_sig_format(key, rng, hash_fn.value_or("SHA-256"), padding.value_or(""));
...
PKCS10_Request CertificateParametersBuilder::into_pkcs10_request(const Private_Key& key,
                                                                 RandomNumberGenerator& rng,
   ...
   return PKCS10_Request::create(key,
                                 subject_dn,
                                 extensions,
                                 hash_fn.value_or("SHA-256"),
                                 rng,
                                 padding.value_or(""),
                                 challenge_password.value_or(""));
}
