...
int main() {
   ...
   { const Botan::PKCS11::Session read_only_session(slot, true); }

   // open read write session
   { const Botan::PKCS11::Session read_write_session(slot, false); }

   // open read write session by passing flags
   {
      const Botan::PKCS11::Flags flags =
         Botan::PKCS11::flags(Botan::PKCS11::Flag::SerialSession | Botan::PKCS11::Flag::RwSession);

      const Botan::PKCS11::Session read_write_session(slot, flags, nullptr, nullptr);
   }

   // move ownership of a session
   {
      Botan::PKCS11::Session session(slot, false);
      const Botan::PKCS11::SessionHandle handle = session.release();

      const Botan::PKCS11::Session session2(slot, handle);
   }

   Botan::PKCS11::Session session(slot, false);

   // get session info
   const Botan::PKCS11::SessionInfo info = session.get_info();
   ...
   // set pin
   const Botan::PKCS11::secure_string new_pin = {'6', '5', '4', '3', '2', '1'};
   ...
   const Botan::PKCS11::secure_string so_pin = {'0', '0', '0', '0', '0', '0', '0', '0'};
