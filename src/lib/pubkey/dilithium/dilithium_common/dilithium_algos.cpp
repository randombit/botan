 ...
 */
void dilithium_poly_pack_t1(const DilithiumPoly& p, BufferStuffer& stuffer) {
 ...
 */
void dilithium_poly_pack_w1(const DilithiumPoly& p, BufferStuffer& stuffer, const DilithiumConstants& mode) {
 ...
 */
void dilithium_poly_pack_gamma1(const DilithiumPoly& p, BufferStuffer& stuffer, const DilithiumConstants& mode) {
 ...
 * (for a = -eta, b = eta)
 */
void dilithium_poly_pack_eta(const DilithiumPoly& p, BufferStuffer& stuffer, const DilithiumConstants& mode) {
 ...
 */
void dilithium_poly_pack_t0(const DilithiumPoly& p, BufferStuffer& stuffer) {
 ...
 * (for a = 2^(bitlen(q-1)-d) - 1)
 */
void dilithium_poly_unpack_t1(DilithiumPoly& p, BufferSlicer& slicer) {
   constexpr auto b = (1 << (bitlen(DilithiumConstants::Q - 1) - DilithiumConstants::D)) - 1;
...
void dilithium_poly_unpack_gamma1(DilithiumPoly& p, ByteSourceT& byte_source, const DilithiumConstants& mode) {
   using Gamma1 = DilithiumConstants::DilithiumGamma1;
   switch(mode.gamma1()) {
      case Gamma1::ToThe17th:
 ...
 * (for a = -eta, b = eta)
 */
void dilithium_poly_unpack_eta(DilithiumPoly& p,
                               BufferSlicer& slicer,
 ...
 * (for a = -2^(d-1) - 1, b = 2^(d-1))
 */
void dilithium_poly_unpack_t0(DilithiumPoly& p, BufferSlicer& slicer) {
   constexpr auto TwoToTheDminus1 = 1 << (DilithiumConstants::D - 1);
 ...
 */
void dilithium_hint_pack(const DilithiumPolyVec& h, BufferStuffer& stuffer, const DilithiumConstants& mode) {
 ...
 */
std::optional<DilithiumPolyVec> dilithium_hint_unpack(BufferSlicer& slicer, const DilithiumConstants& mode) {
...
DilithiumSerializedPublicKey dilithium_encode_public_key(StrongSpan<const DilithiumSeedRho> rho,
   ...
   stuffer.append(rho);
   for(const auto& p : t1) {
      dilithium_poly_pack_t1(p, stuffer);
   }
...
std::pair<DilithiumSeedRho, DilithiumPolyVec> dilithium_decode_public_key(
   ...
   for(auto& p : t1) {
      dilithium_poly_unpack_t1(p, slicer);
   }
...
DilithiumSerializedPrivateKey dilithium_encode_private_key(StrongSpan<const DilithiumSeedRho> rho,
   ...
   for(const auto& p : s1) {
      dilithium_poly_pack_eta(p, stuffer, mode);
   }
...
dilithium_decode_private_key(StrongSpan<const DilithiumSerializedPrivateKey> sk, const DilithiumConstants& mode) {
   ...
   for(auto& p : s1) {
      dilithium_poly_unpack_eta(p, slicer, mode, true /* check decoded value range */);
   }
   ...
   for(auto& p : t0) {
      dilithium_poly_unpack_t0(p, slicer);
   }
...
DilithiumSerializedSignature dilithium_encode_signature(StrongSpan<const DilithiumCommitmentHash> c,
   ...
   for(const auto& p : response) {
      dilithium_poly_pack_gamma1(p, stuffer, mode);
   }
   dilithium_hint_pack(hint, stuffer, mode);
...
std::optional<std::tuple<DilithiumCommitmentHash, DilithiumPolyVec, DilithiumPolyVec>> dilithium_decode_signature(
   ...
   for(auto& p : response) {
      dilithium_poly_unpack_gamma1(p, slicer, mode);
   }
   BOTAN_ASSERT_NOMSG(slicer.remaining() == mode.omega() + mode.k());

   auto hint = dilithium_hint_unpack(slicer, mode);
   BOTAN_ASSERT_NOMSG(slicer.empty());
   if(!hint.has_value()) {
      return std::nullopt;
   }
...
DilithiumSerializedCommitment dilithium_encode_commitment(const DilithiumPolyVec& w1, const DilithiumConstants& mode) {
   ...
   for(const auto& p : w1) {
      dilithium_poly_pack_w1(p, stuffer, mode);
   }
 ...
 */
void dilithium_sample_ntt_uniform(StrongSpan<const DilithiumSeedRho> rho,
                                  DilithiumPolyNTT& p,
                                  uint16_t nonce,
                                  const DilithiumConstants& mode) {
   /**
 ...
 */
void dilithium_sample_uniform_eta(StrongSpan<const DilithiumSeedRhoPrime> rhoprime,
                                  DilithiumPoly& p,
                                  uint16_t nonce,
                                  const DilithiumConstants& mode) {
   using Eta = DilithiumConstants::DilithiumEta;
...
DilithiumPolyMatNTT dilithium_expand_A(StrongSpan<const DilithiumSeedRho> rho, const DilithiumConstants& mode) {
   ...
   for(uint8_t r = 0; r < mode.k(); ++r) {
      for(uint8_t s = 0; s < mode.l(); ++s) {
         ...
         dilithium_sample_ntt_uniform(rho, A[r][s], nonce, mode);
      }
   }
   return A;
}
...
std::pair<DilithiumPolyVec, DilithiumPolyVec> dilithium_expand_s(StrongSpan<const DilithiumSeedRhoPrime> rhoprime,
   ...
   uint16_t nonce = 0;
   for(auto& p : s1) {
      dilithium_sample_uniform_eta(rhoprime, p, nonce++, mode);
   }
...
DilithiumPolyVec dilithium_expand_mask(StrongSpan<const DilithiumSeedRhoPrime> rhoprime,
   ...
   for(auto& p : s) {
      auto& xof = mode.symmetric_primitives().H(rhoprime, nonce++);
      dilithium_poly_unpack_gamma1(p, xof, mode);
   }
   return s;
}
...
auto dilithium_decompose_fn(const DilithiumConstants& mode) {
   using Gamma2 = DilithiumConstants::DilithiumGamma2;
...
std::pair<DilithiumPolyVec, DilithiumPolyVec> dilithium_decompose(const DilithiumPolyVec& vec,
                                                                  const DilithiumConstants& mode) {
   auto decompose = dilithium_decompose_fn(mode);
   auto result = std::make_pair(DilithiumPolyVec(vec.size()), DilithiumPolyVec(vec.size()));
