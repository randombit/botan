 ...
 *
 * This will return an rvalue reference of the unwrapped object if the input is
 * an rvalue or an rvalue reference (perfect forwarding).
 *
...
[[nodiscard]] constexpr decltype(auto) unwrap_strong_type(T&& t) {
   if constexpr(!concepts::strong_type<std::remove_cvref_t<T>>) {
      ...
      // If the parameter type is an lvalue reference to a strong type,
      // return the wrapped object as an lvalue reference.
      return t.get();
   } else {
      // If the parameter type is an rvalue reference to a strong type,
      // i.e. the caller moved or forwarded the strong type, return the
      // wrapped object as an rvalue reference (perfect forwarding).
