...
def run_command(cmd):
    ...
    if proc.returncode != 0:
        logging.error(
            "Running command %s failed ret %d", " ".join(cmd), proc.returncode
        )
...
SIGNATURE_EVP_MAP = {"RSA": "rsa", "ECDSA": "ecdsa"}

KEY_AGREEMENT_EVP_MAP = {"DH": "ffdh", "ECDH": "ecdh"}
...
def run_openssl_bench(openssl, algo):
    ...
    if algo in EVP_MAP:
        ...
        res_header = re.compile(
            r"\+(R7|R9|R12|R14):([0-9]+):([0-9]+):([0-9]+\.[0-9]+)$"
        )
...
def run_botan_bench(botan, runtime, buf_sizes, algo):
    ...
    cmd = [
        ...
        "speed",
        "--format=json",
        "--msec=%d" % int(runtime * 1000),
        "--buf-size=%s" % (",".join([str(i) for i in buf_sizes])),
        algo,
    ]
    output = run_command(cmd)
...
def run_botan_signature_bench(botan, runtime, algo):
    ...
    for verify in output:
        for sign in output:
            if (
                sign["op"] == "sign"
                and verify["op"] == "verify"
                and verify["algo"] == sign["algo"]
            ):
                results.append(
                    {
                        "algo": algo,
                        "key_size": int(
                            re.search(r"[A-Z]+-[a-z]*([0-9]+).*", sign["algo"]).group(1)
                        ),
                        "op": "sign",
                        "ops": sign["events"],
            ...
            results.append(
                {
                    "algo": algo,
                    "key_size": int(
                        re.search(r"[A-Z]+-[a-z]*([0-9]+).*", res["algo"]).group(1)
                    ),
                    "ops": res["events"],
...
class BenchmarkResult:
    ...
    def result_string(self):
        ...
        for k, v in self.results.items():
            ...
            out += (
                "algo %s buf_size % 6d botan % 12d bps openssl % 12d bps adv %s by %.02f\n"
    ...
    def graphs(self):
        ...
        for lib in ["Botan", "OpenSSL"]:
            ...
            results = [
                math.ceil(x[lib.lower()] / (1024 * 1024)) for x in self.results.values()
            ]
    ...
    def result_string(self):
        ...
        for k, v in self.results.items():
            ...
            for op in openssl.keys():
                ...
                out += (
                    "algo %s key_size % 5d % 8s botan % 10d openssl % 10d adv %s by %.02f\n"
...
class KeyAgreementBenchmarkResult:
    ...
    def result_string(self):
        ...
        for k, v in self.results.items():
            ...
            out += (
                "algo %s key_size % 6d botan % 12d key agreements openssl % 12d key agreements adv %s by %.02f\n"
                % (self.algo, k, v["botan"], v["openssl"], winner, ratio)
            )
        return out
...
def bench_algo(openssl, botan, algo):
    ...
    runtime = (
        sum(x["runtime"] for x in openssl_results)
        / len(openssl_results)
        / len(buf_sizes)
    )
...
def bench_signature_algo(openssl, botan, algo):
    ...
    return SignatureBenchmarkResult(
        algo, sorted(kszs_ossl.intersection(kszs_botan)), openssl_results, botan_results
    )
...
def bench_key_agreement_algo(openssl, botan, algo):
    ...
    return KeyAgreementBenchmarkResult(
        algo, sorted(kszs_ossl.intersection(kszs_botan)), openssl_results, botan_results
    )
...
def gen_html_report(botan_version, openssl_version, cmdline):
    ...
    include_path = re.search(
        r"-I(.*)", run_command(["botan", "config", "cflags"]).strip()
    ).group(1)
    build_h = "N/A"

    try:
        with open(
            os.path.join(include_path, "botan/build.h"), "r", encoding="utf-8"
        ) as f:
            build_h = f.read()
...
def main(args=None):
    ...
    parser.add_option(
        "--quiet", action="store_true", default=False, help="be very quiet"
    )

    parser.add_option(
        "--html-report", action="store_true", default=False, help="create HTML report"
    )
