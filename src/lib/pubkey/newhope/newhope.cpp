...
inline void encode_b(uint8_t* r, const poly* b, const poly* c)
   ...
   for(size_t i = 0; i < PARAM_N / 4; i++)
      {
      r[NEWHOPE_POLY_BYTES + i] = c->coeffs[4 * i] | (c->coeffs[4 * i + 1] << 2) | (c->coeffs[4 * i + 2] << 4) |
                                  (c->coeffs[4 * i + 3] << 6);
      }
   }
...
inline void rec(uint8_t* key, const poly* v, const poly* c)
   ...
   for(i = 0; i < 256; i++)
      {
      tmp[0] = 16 * PARAM_Q + 8 * static_cast<int32_t>(v->coeffs[  0 + i]) - PARAM_Q * (2 * c->coeffs[  0 + i] + c->coeffs[768
               + i]);
      tmp[1] = 16 * PARAM_Q + 8 * static_cast<int32_t>(v->coeffs[256 + i]) - PARAM_Q * (2 * c->coeffs[256 + i] + c->coeffs[768
               + i]);
      tmp[2] = 16 * PARAM_Q + 8 * static_cast<int32_t>(v->coeffs[512 + i]) - PARAM_Q * (2 * c->coeffs[512 + i] + c->coeffs[768
               + i]);
