...
def sign(
    sign_internal_func, sk_bytes, m, random_bytes, n, ctx=b"", deterministic=False
):
    if len(ctx) > 255:
        raise ValueError(
...
def main(args=None):
    ...
    if algo == "slh_dsa":
        ...
        for param, alg in SLH_DSA_PARAMS.items():
            ...
            signature_rand = sign(
                alg.slh_sign_internal,
                sk,
                msg,
                drbg_instance.random_bytes,
                alg.n,
                deterministic=False,
            )
            signature_det = sign(
            ...
            if param == "SLH-DSA-SHAKE-128s":
                print(
                    "SigDet = %s"
                    % binascii.hexlify(signature_det).decode("utf-8").upper(),
                    file=output,
                )
            print(
                "HashSigDet = %s"
                % binascii.hexlify(hash_fn(signature_det)).decode("utf-8").upper(),
                file=output,
            )
            print(
                "HashSigRand = %s"
                % binascii.hexlify(hash_fn(signature_rand)).decode("utf-8").upper(),
                file=output,
            )
            print("", file=output)
        ...
        for param in ML_DSA_PARAM.keys():
            for det_str, deterministic in [
                ("Deterministic", True),
                ("Randomized", False),
            ]:
                alg = ML_DSA(param)
                output = open(
                    "src/tests/data/pubkey/%s_%s.vec"
                    % (transform_mldsa_string(param), det_str),
                    "w",
                )
                ...
                for _ in range(100):
                    ...
                    print(
                        "HashPk = %s"
                        % binascii.hexlify(hash_fn(pk)).decode("utf-8").upper(),
                        file=output,
                    )
                    print(
                        "HashSk = %s"
                        % binascii.hexlify(hash_fn(keygen_rand))
                        .decode("utf-8")
                        .upper(),
                        file=output,
                    )
