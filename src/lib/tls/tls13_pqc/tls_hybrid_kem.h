...
/**
 * Composes a pair of public keys as defined in this IETF draft:
 ...
 */
class BOTAN_TEST_API Hybrid_TLS_KEM_PublicKey : public virtual Hybrid_KEM_PublicKey {
   public:
      static std::unique_ptr<Hybrid_TLS_KEM_PublicKey> load_for_group(
         Group_Params group, std::span<const uint8_t> concatenated_public_values);

   public:
      using Hybrid_KEM_PublicKey::Hybrid_KEM_PublicKey;
...
/**
 * Composes a pair of private keys for hybrid key agreement as defined in this
 * IETF draft: https://datatracker.ietf.org/doc/html/draft-ietf-tls-hybrid-design-16
 */
class BOTAN_TEST_API Hybrid_TLS_KEM_PrivateKey final : public virtual Hybrid_TLS_KEM_PublicKey,
                                                       public virtual Hybrid_KEM_PrivateKey {
   public:
      /**
       * Generate a hybrid private key for the given TLS code point.
       */
      static std::unique_ptr<Hybrid_TLS_KEM_PrivateKey> generate_from_group(Group_Params group,
                                                                            RandomNumberGenerator& rng);

   public:
      explicit Hybrid_TLS_KEM_PrivateKey(PairOfPrivateKeys private_keys);

      std::unique_ptr<Public_Key> public_key() const override {
         return std::make_unique<Hybrid_TLS_KEM_PublicKey>(extract_public_keys(private_keys()));
      }
