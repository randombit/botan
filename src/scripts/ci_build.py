...
def build_targets(target, target_os):
    if target in ["shared", "minimized", "examples", "limbo"] or target.startswith(
        "policy-"
    ):
        yield "shared"
    elif target in [
...
def make_targets(target, target_os):
    ...
    tgts = [
        tgt if tgt not in ["static", "shared"] else "libs"
        for tgt in build_targets(target, target_os)
    ]
...
def determine_flags(
    ...
    test_prefix = []
    test_cmd = [
        os.path.join(build_dir, "botan-test"),
    ...
    if test_results_dir:
        ...
        test_cmd += [
            ...
            % ",".join(
                [
                    "%s:%s" % (sanitize_kv(k), sanitize_kv(v))
    ...
    if target.startswith("policy-"):
        ...
        flags += [
            "--module-policy=%s" % (target.replace("policy-", "")),
            "--enable-modules=tls12,tls13",
    ...
    if is_cross_target:
        if target_os == "ios":
            ...
            if ndk is None:
                raise Exception(
                    "Android CI build requires ANDROID_NDK env variable be set"
                )
            ...
            if target == "cross-android-arm32":
                cc_bin = os.path.join(
                    toolchain_dir, "armv7a-linux-androideabi%d-clang++" % (api_lvl)
                )
                flags += [
                    "--cpu=armv7",
                ...
                cc_bin = os.path.join(
                    toolchain_dir, "aarch64-linux-android%d-clang++" % (api_lvl)
                )
                flags += [
                    "--cpu=arm64",
                    "--ar-command=%s" % (os.path.join(toolchain_dir, "llvm-ar")),
        ...
        if is_running_in_github_actions() and "BOTAN_TPM2_ENABLED" in os.environ:
            ...
            if os.environ.get("BOTAN_TPM2_ENABLED") == "test":
                # run the TPM2 tests
                test_cmd += [
                    "--tpm2-tcti-name=%s" % os.getenv("BOTAN_TPM2_TCTI_NAME"),
                    "--tpm2-tcti-conf=%s" % os.getenv("BOTAN_TPM2_TCTI_CONF"),
                    "--tpm2-persistent-rsa-handle=%s"
                    % os.getenv("BOTAN_TPM2_PERSISTENT_RSA_KEY_HANDLE"),
                    "--tpm2-persistent-ecc-handle=%s"
                    % os.getenv("BOTAN_TPM2_PERSISTENT_ECC_KEY_HANDLE"),
                    "--tpm2-persistent-auth-value=%s"
                    % os.getenv("BOTAN_TPM2_PERSISTENT_KEY_AUTH_VALUE"),
                ]
...
def run_cmd(cmd, root_dir, build_dir):
    ...
    with LoggingGroup(" ".join(cmd)):
        ...
        proc = subprocess.Popen(
            cmd, cwd=cwd, close_fds=True, env=sub_env, stdout=redirect_stdout_fd
        )
        proc.communicate()
        ...
        if proc.returncode != 0:
            ...
            if redirect_stdout_fd is not None:
                ...
                last_lines = open(redirect_stdout_fsname, encoding="utf8").readlines()[
                    -100:
                ]
...
def parse_args(args):
    ...
    parser.add_option(
        "--os", default=default_os(), help="Set the target os (default %default)"
    )
    parser.add_option("--cpu", default=None, help="Specify a target CPU platform")
    parser.add_option(
        "--cc",
        default=default_cc(),
        help="Set the target compiler type (default %default)",
    )
    parser.add_option("--cc-bin", default=None, help="Set path to compiler")
    parser.add_option(
        "--root-dir",
        metavar="D",
        default=".",
...
def main(args=None):
    ...
    if options.compiler_cache not in [None, "ccache", "sccache"]:
        raise Exception(
            "Don't know about %s as a compiler cache" % (options.compiler_cache)
        )
    ...
    if not os.path.exists(build_dir):
        ...
        raise Exception(
            "Bad build dir setting %s is not a directory or not accessible"
            % (build_dir)
        )
    ...
    if target == "lint":
        ...
        pylint_flags += [
            "--ignored-modules=gdb"
        ]  # 'import gdb' is not available outside gdb...

        if is_running_in_github_actions():
            pylint_flags += [
                "--msg-template='::warning file={path},line={line},endLine={end_line}::Pylint ({category}): {msg_id} {msg} ({symbol})'"
            ]
        ...
        cmds.append(
            ["indir:%s" % root_dir, py_interp, "-m", "pylint"]
            + pylint_flags
            + py_scripts
        )
        ...
        cmds.append(
            ["indir:%s" % (root_dir), "typos", "-c", "src/configs/typos.toml", "."]
        )
    elif target == "format":
        cmds.append(
            [
        ...
        config_flags, run_test_command, make_prefix = determine_flags(
            ...
            root_dir,
            build_dir,
            options.test_results_dir,
            options.pkcs11_lib,
            options.use_gdb,
            options.disable_werror,
            options.extra_cxxflags,
            options.custom_optimization_flags,
            options.disabled_tests,
        )

        make_tool, make_opts = validate_make_tool(options.make_tool, options.build_jobs)

        cmds.append(
            [py_interp, os.path.join(root_dir, "configure.py")]
        ...
        if target in ["valgrind", "valgrind-full", "valgrind-ct", "valgrind-ct-full"]:
            ...
            cmds.append(
                [
                    os.path.join(root_dir, "src", "ct_selftest", "ct_selftest.py"),
            ...
            valgrind_script_options = [
                "--test-binary=%s" % (os.path.join(build_dir, "botan-test")),
                "--verbose",
                "--bunch",
                "--track-origins",
            ]
            ...
            if target not in ["valgrind-full", "valgrind-ct-full"]:
                ...
                slow_tests += [
                    f"dilithium_kat_{mode}_{rand}"
                    for mode in ("6x5", "8x7", "6x5_AES", "8x7_AES")
                    for rand in ("Deterministic", "Randomized")
                ]
                slow_tests += [
                    f"ml_dsa_kat_{mode}_{rand}"
                    for mode in ("6x5", "8x7")
                    for rand in ("Deterministic", "Randomized")
                ]

                valgrind_script_options.append(
                    "--skip-tests=%s" % (",".join(slow_tests))
                )
            elif (
                target == "valgrind-ct-full"
                and options.cc == "clang"
                and "-Os" in options.custom_optimization_flags
            ):
            ...
            cmds.append(
                ["indir:%s" % (root_dir), "src/scripts/run_tests_under_valgrind.py"]
                + valgrind_script_options
            )
        ...
        if target in ["coverage", "sanitizer"] and options.os != "windows":
            ...
            runner_dir = os.path.abspath(
                os.path.join(options.boringssl_dir, "ssl", "test", "runner")
            )
            ...
            cmds.append(
                [
                    ...
                    "-pipe",
                    "-num-workers",
                    str(4 * get_concurrency()),
                    "-shim-path",
                    os.path.abspath(os.path.join(build_dir, "botan_bogo_shim")),
                    "-shim-config",
                    os.path.abspath(
                        os.path.join(root_dir, "src", "bogo_shim", "config.json")
                    ),
                ]
            )
        ...
        if target in ["limbo"]:
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/run_limbo_tests.py"),
        ...
        if target in ["coverage", "fuzzers"]:
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/test_fuzzers.py"),
        ...
        if target in ["shared", "coverage", "sanitizer"]:
            botan_exe = os.path.join(
                build_dir, "botan-cli.exe" if options.os == "windows" else "botan"
            )
            ...
            for script in test_scripts:
                test_data_arg = []
                cmds.append(
                    [py_interp, os.path.join(root_dir, "src/scripts", script)]
                    + args
                    + test_data_arg
                    + [botan_exe]
                )
        ...
        if target in ["strubbing"]:
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/test_strubbed_symbols.py"),
                    "--botan-cli",
                    os.path.join(build_dir, "botan"),
                ]
            )
        ...
        if target in ["hybrid-tls13-interop-test"]:
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/test_cli.py"),
                    "--run-online-tests",
                    os.path.join(build_dir, "botan"),
                    "pqc_hybrid_tests",
                ]
        ...
        if target in ["shared", "coverage"] and not (
            options.os == "windows" and options.cpu == "x86"
        ):
        ...
        if target in ["shared", "static"]:
            ...
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/ci_check_install.py"),
                    build_config,
                ]
            )
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/ci_check_headers.py"),
                    build_config,
                ]
            )
            cmds.append(
                [
                    py_interp,
                    os.path.join(root_dir, "src/scripts/ci_report_sizes.py"),
                    build_config,
                ]
            )
        ...
        if target in ["coverage"]:
            if have_prog("coverage"):
                cmds.append(
                    [
                        ...
                        "--branch",
                        "--rcfile",
                        os.path.join(root_dir, "src/configs/coverage.rc"),
                    ]
                    + python_tests
                )
            ...
            cmds.append(
                ["lcov", "--remove", raw_cov_file, "/usr/*", "--output-file", cov_file]
            )
            cmds.append(["lcov", "--list", cov_file])
            cmds.append(
                [os.path.join(root_dir, "src/scripts/rewrite_lcov.py"), cov_file]
            )
            ...
            if have_prog("coveralls"):
                ...
                cmds.append(
                    [
                        ...
                        cov_file,
                        "--output-directory",
                        os.path.join(build_dir, "lcov-out"),
                    ]
                )
    ...
    if is_running_in_github_actions() and "BOTAN_BUILD_WITH_ESDM" in os.environ:
        ...
        esdm_is_ok = esdm_process.poll() is None
        assert esdm_is_ok, f"esdm-server did not start for target {target}"
