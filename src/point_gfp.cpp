        ...
        namespace ec
          {
            ...
            PointGFp::PointGFp(CurveGFp const& curve, gf::GFpElement const& x,
            ...
            PointGFp const& PointGFp::operator=(PointGFp const& other)
            {
              ...
              mZpow2 = gf::GFpElement(other.mZpow2);
              mZpow3 = gf::GFpElement(other.mZpow3);
              mAZpow4 = gf::GFpElement(other.mAZpow4);
              mZpow2_set = other.mZpow2_set;
              mZpow3_set = other.mZpow3_set;
              mAZpow4_set = other.mAZpow4_set;
              set_shrd_mod(mC.get_ptr_mod());
            ...
            void PointGFp::set_shrd_mod(SharedPtrConverter<GFpModulus> p_mod)
            {
              mX.set_shrd_mod(p_mod.get_shared());
              mY.set_shrd_mod(p_mod.get_shared());
              mZ.set_shrd_mod(p_mod.get_shared());
              mZpow2.set_shrd_mod(p_mod.get_shared());
              mZpow3.set_shrd_mod(p_mod.get_shared());
              mAZpow4.set_shrd_mod(p_mod.get_shared());
            }
...
#ifdef TA_COLL_T
// in this case we want to say whether randomized exponent
// should be re-used (false) or initialized with a new random
// (not threadsafe!)
...
// AADA with rand. Exp.
            ...
            */
            ...
            PointGFp const& PointGFp::set_z_to_one() const
              {
                if (!(mZ.get_value() == BigInt(1)) && !(mZ.get_value() == BigInt(0)))
                  {
                    gf::GFpElement z = gf::inverse(mZ);
                    gf::GFpElement z2 = z * z;
                    z *= z2;
                    gf::GFpElement x = mX * z2;
                    gf::GFpElement y = mY * z;
                    mZ = gf::GFpElement(mC.get_p(), BigInt(1));
                    mX = x;
                    mY = y;
                  }
                else
            ...
            gf::GFpElement const PointGFp::get_affine_x() const
              {
                ...
                //}
                //assert(mZpow2 == mZ*mZ);
                gf::GFpElement z2 = mZpow2;
                return mX * z2.inverse_in_place();
              }

            gf::GFpElement const PointGFp::get_affine_y() const
              {
                ...
                gf::GFpElement z3 = mZpow3;
                return mY * z3.inverse_in_place();
              }
            gf::GFpElement const PointGFp::get_jac_proj_x() const
              {
                return gf::GFpElement(mX);
              }
            gf::GFpElement const PointGFp::get_jac_proj_y() const
              {
                return gf::GFpElement(mY);
              }
            gf::GFpElement const PointGFp::get_jac_proj_z() const
              {
                return gf::GFpElement(mZ);
              }
            ...
            void PointGFp::check_invariants() const
              {
                ...
                const gf::GFpElement y2 = mY * mY;
                const gf::GFpElement x3 = mX * mX * mX;

                if (mZ.get_value() == BigInt(1))
                  {
                    gf::GFpElement ax = mC.get_a() * mX;
                ...
                mAZpow4_set = true;
                //}
                const gf::GFpElement aXZ4 = mAZpow4 * mX;
                const gf::GFpElement bZ6 = mC.get_b() * mZpow3 * mZpow3;
            ...
            {
              return output << "curve: " << elem.get_curve()
                     << "; X=" << elem.get_jac_proj_x() << "; Y=" << elem.get_jac_proj_y() <<
                     "; Z=" << elem.get_jac_proj_z();
            }
            ...
            PointGFp OS2ECP(MemoryRegion<byte> const& os, CurveGFp const& curve)
            {
              ...
              switch (pc)
                ...
                case 3:
                  ...
                  x = gf::GFpElement(curve.get_p(), bi_dec_x);
                  bool yMod2;
                  yMod2 = (pc & 1) == 1;
                ...
                case 4:
                  ...
                  x = gf::GFpElement(curve.get_p(), bi_dec_x);
                  y = gf::GFpElement(curve.get_p(), bi_dec_y);
                  break;
              ...
              z = gf::GFpElement(curve.get_p(), BigInt(1));
            ...
            gf::GFpElement PointGFp::decompress(bool yMod2, gf::GFpElement const& x,
                                                CurveGFp const& curve)
            {
              ...
              try
                {
                  z = ::Botan::ressol(g, curve.get_p());
                }
              catch (No_Quadratic_Residue)
                {
              ...
              return gf::GFpElement(curve.get_p(),z);
            }
            ...
            PointGFp const create_random_point(CurveGFp const& curve)
            {
              ...
              gf::GFpElement mX(1,1);
              gf::GFpElement mY(1,1);
              gf::GFpElement mZ(1,1);
              gf::GFpElement minusOne(curve.get_p(), BigInt(BigInt::Negative,1));
              mY = minusOne;
              gf::GFpElement y2(1,1);
              gf::GFpElement x(1,1);

              while (mY == minusOne)
                {
                  BigInt value = random_integer(curve.get_p().bits());
                  mX = gf::GFpElement(curve.get_p(),value);
                  y2 = curve.get_a() * mX;
                  x = mX * mX;
                  x *= mX;
                  y2 += (x + curve.get_b());
                  try
                    {
                      ...
                      mY = gf::GFpElement(curve.get_p(), value);
                    }
                  catch (No_Quadratic_Residue)
                    {
                      mY = minusOne;
              ...
              mZ = gf::GFpElement(curve.get_p(), BigInt(1));
              return PointGFp(curve, mX, mY, mZ);

            }

          } // namespace Botan::math:ec

      } // namespace Botan::math
