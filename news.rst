...
* The PKCS#12 parser now verifies each private key with ``check_key`` and
  rejects keys whose private and public components are inconsistent, as can
  result from tampering with a PKCS#12 file that has no MAC.
