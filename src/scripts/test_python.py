...
class BotanPythonTests(unittest.TestCase):
    ...
    def test_pbkdf(self):
        (salt, iterations, pbkdf) = botan.pbkdf(
            "PBKDF2(SHA-1)", "", 32, 10000, hex_decode("0001020304050607")
        )
    ...
    def test_bcrypt(self):
        ...
        self.assertTrue(
            botan.check_bcrypt(
                "test", "$2a$04$wjen1fAA.UW6UxthpKK.huyOoxvCR7ATRCVC4CBIEGVDOCtr8Oj1C"
            )
        )
    ...
    def test_rng(self):
        ...
        with self.assertRaisesRegex(
            botan.BotanException, r".*Unexpected arguments for RNG.*"
        ):
    ...
    def test_tpm2_rng(self):
        ...
        with self.assertRaisesRegex(
            botan.BotanException, r".*without a TPM2 context.*"
        ):
            botan.RandomNumberGenerator("tpm2")

        # invalid session object provided
        with self.assertRaisesRegex(
            botan.BotanException, r".*0 to 3 TPM2Session objects.*"
        ):
            botan.RandomNumberGenerator(
                "tpm2", tpm2_context=tpm2_ctx, tpm2_sessions=int(0)
            )
        ...
        with self.assertRaisesRegex(
            ...
            botan.RandomNumberGenerator(
                "tpm2", tpm2_context=tpm2_ctx, tpm2_sessions=[session, None, None, None]
            )

        # unexpected kwarg provided
        with self.assertRaisesRegex(
            botan.BotanException, r".*Unexpected arguments for TPM2.*"
        ):
            botan.RandomNumberGenerator(
                "tpm2", tpm2_context=tpm2_ctx, unexpected_arg="unexpected"
            )

        # session provided as a single session (not wrapped into an Iterable)
        tpm2_rng = botan.RandomNumberGenerator(
            "tpm2", tpm2_context=tpm2_ctx, tpm2_sessions=session
        )
    ...
    def test_cipher(self):
        for mode in [
            ...
            try:
                ...
                raise RuntimeError(
                    "Failed to create encrypting cipher for " + mode
                ) from e
            ...
            try:
                ...
                raise RuntimeError(
                    "Failed to create decrypting cipher for " + mode
                ) from e
            dec.set_key(key)
            dec.start(iv)
...
"""
    ...
    def test_key_crypto(self):
        ...
        for is_pem in [True, False]:
            ...
            pem2 = priv.export_encrypted(
                passphrase, rng, is_pem, msec=10, cipher="AES-128/SIV"
            )
            dec2 = botan.PrivateKey.load(pem2, passphrase)
            self.assertEqual(dec2.export(is_pem), ref_val)

            pem3 = priv.export_encrypted(
                passphrase, rng, is_pem, msec=10, cipher="AES-128/GCM", pbkdf="Scrypt"
            )
    ...
    def test_stateful_operations(self):
        ...
        try:
            ...
            self.assertEqual(
                str(e),
                "botan_privkey_remaining_operations failed: -3 (No value available)",
            )
    ...
    def test_check_key(self):
        ...
        try:
            ...
            self.assertEqual(
                str(e),
                "botan_pubkey_load_rsa failed: -1 (Invalid input): Invalid RSA public key parameters",
            )
    ...
    def _pksign_roundtrips(self, sk, pk, param_str):
        ...
        raw_bytes = bytes.fromhex(
            "8100112233445566778899AABBCCDDEEFF"
        )  # these bytes can't be decoded as UTF-8!
    ...
    def test_ecdsa(self):
        ...
        compressed_sec1 = BotanPythonTests._ecc_sec1_convert_to_compressed(
            uncompressed_sec1
        )
    ...
    def test_certs(self):
        cert = botan.X509Cert(
            filename=test_data("src/tests/data/x509/ecc/isrg-root-x2.pem")
        )
        ...
        self.assertEqual(
            hex_encode(cert.subject_key_id()),
            "7c4296aede4b483bfa92f89e8ccf6d8ba9723795",
        )

        self.assertEqual(cert.subject_dn("Name", 0), "ISRG Root X2")
        self.assertEqual(
            cert.subject_dn("Organization", 0), "Internet Security Research Group"
        )
        ...
        self.assertEqual(
            cert.issuer_dn("Organization", 0), "Internet Security Research Group"
        )
        ...
        self.assertEqual(
            end04.verify(
                [int04_1, int04_2],
                [],
                test_data("src/tests/data/x509/nist/"),
        ...
        self.assertEqual(
            end04.verify(
                ...
                [root],
                required_strength=80,
                hostname="User1-CP.02.01",
            ),
            0,
        )
        self.assertEqual(
            end04.verify(
                [int04_1, int04_2], [root], required_strength=80, hostname="invalid"
            ),
            4008,
        )
        self.assertEqual(
            end04.verify(
                [int04_1, int04_2], [root], required_strength=80, reference_time=1
            ),
            2000,
        )
        ...
        self.assertEqual(
            botan.X509Cert.validation_status(3000), "Certificate issuer not found"
        )
        self.assertEqual(
            botan.X509Cert.validation_status(4008),
        ...
        self.assertEqual(
            end20.verify(
                [int20], [root], required_strength=80, crls=[int20crl, rootcrl]
            ),
            5000,
        )
        self.assertEqual(
            botan.X509Cert.validation_status(5000), "Certificate is revoked"
        )
        ...
        self.assertEqual(
            end21.verify(
                [int21], [root], required_strength=80, crls=[int21crl, rootcrl]
            ),
            5000,
        )
    ...
    def test_keywrap(self):
        ...
        self.assertEqual(
            hex_encode(wrapped), "1fa68b0a8112b447aef34bd8fb5a7b829d3e862371d2cfe5"
        )
    ...
    def test_srp6(self):
        ...
        salt = rng.get(24)
        verifier = botan.srp6_generate_verifier(
            identity, password, salt, group, hash_fn
        )
        b = server.step1(verifier, hash_fn, rng)
        (a, key_c) = botan.srp6_client_agree(
            identity, password, group, hash_fn, salt, b, rng
        )
        key_s = server.step2(a)
        self.assertEqual(key_c, key_s)

    def test_kyber_raw_keys(self):
        a_pub_bits = hex_decode(
    ...
    def test_ml_kem_raw_keys(self):
        ...
        sk = botan.PrivateKey.create(
            "ML-KEM", mlkem_mode, botan.RandomNumberGenerator("user")
        )
    ...
    def test_ml_dsa_raw_keys(self):
        ...
        sk = botan.PrivateKey.create(
            "ML-DSA", mldsa_mode, botan.RandomNumberGenerator("user")
        )
        pk = sk.get_public_key()
    ...
    def test_slh_dsa_raw_keys(self):
        ...
        sk = botan.PrivateKey.create(
            "SLH-DSA", slhdsa_mode, botan.RandomNumberGenerator("user")
        )
        pk = sk.get_public_key()
    ...
    def test_frodokem_raw_keys(self):
        ...
        sk = botan.PrivateKey.create(
            "FrodoKEM", frodo_mode, botan.RandomNumberGenerator("user")
        )
        pk = sk.get_public_key()
    ...
    def test_classic_mceliece_raw_keys(self):
        cmce_mode = "348864f"
        sk = botan.PrivateKey.create(
            "ClassicMcEliece", cmce_mode, botan.RandomNumberGenerator("user")
        )
        pk = sk.get_public_key()
    ...
    def test_oids(self):
        ...
        new_oid_from_string = botan.OID.from_string(
            "random-name-that-definitely-has-no-oid"
        )
    ...
    def test_ec_group(self):
        ...
        self.assertEqual(
            group_from_name, botan.ECGroup.from_pem(group_from_name.to_pem())
        )
        self.assertEqual(
            group_from_name, botan.ECGroup.from_ber(group_from_name.to_der())
        )
        ...
        if botan.ECGroup.supports_application_specific_group():
            ...
            group_from_parameters = botan.ECGroup.from_params(
                secp256r1_new_oid, p, a, b, g_x, g_y, order
            )
        ...
        for k in range(1, 5):
            for n in range(k, k + 5):
                for x in range(128, 256, 5):
                    input_bytes = b"".join([next(random_bytes) for _ in range(x * k)])
