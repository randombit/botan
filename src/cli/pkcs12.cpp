...
class PKCS12_Export final : public Command {
   public:
      PKCS12_Export() :
            Command(
               "pkcs12_export --pass= --key-pass= --friendly-name= --use-pbes2 --no-mac --cert-cipher= --mac-digest=SHA-1 --iterations=2048 key cert *ca_certs") {
      }
      ...
      void go() override {
         ...
         if(!cert_cipher.empty()) {
            options.cert_encryption_algo = cert_cipher;
         }

         if(flag_set("no-mac")) {
            options.include_mac = false;
         }

         const std::string mac_digest = get_arg("mac-digest");
         if(!mac_digest.empty()) {
            options.mac_digest = mac_digest;
         }
