/*
* (C) 2024      Jack Lloyd
*     2024-2025 René Meusel - Rohde & Schwarz Cybersecurity
*
...
class TestOptionsBuilder final : public Botan::OptionsBuilder<TestOptionsContainer> {
   public:
   ...
   public:
      ...
      TestOptions create() { return commit<TestOptions>(); }
};
...
std::vector<Test::Result> test_default_options_builder() {
   return {
      CHECK("default builder creates empty options",
            [](Test::Result& result) {
               TestOptions options = TestOptionsBuilder().create();
