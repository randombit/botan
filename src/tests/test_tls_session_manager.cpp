...
std::vector<Test::Result> test_session_manager_in_memory()
   ...
   return
      {
      Botan_Tests::CHECK("creation", [&](auto&)
         {
         mgr.emplace(Test::rng(), 5);
         }),

      Botan_Tests::CHECK("empty cache does not obtain anything", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("clearing empty cache", [&](auto& result)
         {
         result.test_eq("does not delete anything", mgr->remove_all(), 0);
         }),

      Botan_Tests::CHECK("establish new session", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("obtain session from server info", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("obtain session from ID", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("obtain session from ID disguised as opaque handle", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("add session with ID", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("add session with ticket", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("removing by ID or opaque handle", [&](auto& result)
         {
         Botan::TLS::Session_Manager_In_Memory local_mgr(Test::rng());

         const auto new_session1 = local_mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs), default_id);
         const auto new_session2 = local_mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         result.require("saving worked", new_session1.has_value() && new_session1->id().has_value() && !new_session1->ticket().has_value());
         result.require("saving worked", new_session2.has_value() && new_session2->id().has_value() && !new_session2->ticket().has_value());
         ...
         result.confirm("cannot obtain via default ID anymore", !local_mgr.retrieve(default_id, cbs, plcy).has_value());
         result.test_is_eq("can find less via server info", local_mgr.find(server_info, cbs, plcy).size(), size_t(1));

         result.test_is_eq("last one was deleted", local_mgr.remove(Botan::TLS::Opaque_Session_Handle(new_session2->id().value())), size_t(1));
         result.confirm("cannot obtain via ID anymore", !local_mgr.retrieve(new_session2->id().value(), cbs, plcy).has_value());
         result.confirm("cannot find via server info", local_mgr.find(server_info, cbs, plcy).empty());
         }),

      Botan_Tests::CHECK("removing by ticket or opaque handle", [&](auto& result)
         {
         Botan::TLS::Session_Manager_In_Memory local_mgr(Test::rng());
         ...
         result.test_is_eq("can find them via server info ", local_mgr.find(server_info, cbs, plcy).size(), size_t(3));
         ...
         result.test_is_eq("remove one session by opaque handle", local_mgr.remove(Botan::TLS::Opaque_Session_Handle(ticket3.get())), size_t(1));
         result.test_is_eq("can find only one via server info", local_mgr.find(server_info, cbs, plcy).size(), size_t(1));
         }),

      Botan_Tests::CHECK("session purging", [&](auto& result)
         {
         ...
         for(size_t i = 0; i < mgr->capacity(); ++i)
            {
            handles.push_back(mgr->establish(default_session(Botan::TLS::Connection_Side::Server, cbs), random_id()).value());
            }
...
std::vector<Test::Result> test_session_manager_choose_ticket()
   ...
   return
      {
      CHECK("empty manager has nothing to choose from", [&](auto& result)
         {
         Botan::TLS::Session_Manager_In_Memory mgr(Test::rng());

         Botan::TLS::Session_Ticket random_session_ticket = random_ticket();

         result.confirm("empty ticket list, no session", !mgr.choose_from_offered_tickets({}, "SHA-256", cbs, plcy).has_value());
         result.confirm("empty session manager, no session", !mgr.choose_from_offered_tickets(std::vector{ticket(random_session_ticket)}, "SHA-256", cbs, plcy).has_value());
         }),

      CHECK("choose ticket by ID", [&](auto& result)
         {
         Botan::TLS::Session_Manager_In_Memory mgr(Test::rng());
         std::vector<Botan::TLS::Session_Handle> handles;
         ...
         auto session1 = mgr.choose_from_offered_tickets(std::vector{ticket(handles[0].id().value())}, "SHA-256", cbs, plcy);
         ...
         auto session2 = mgr.choose_from_offered_tickets(std::vector{ticket(handles[1].id().value())}, "SHA-256", cbs, plcy);
         ...
         auto session3 = mgr.choose_from_offered_tickets(std::vector{ticket(random_ticket()), ticket(handles[1].id().value())}, "SHA-256", cbs, plcy);
      ...
      CHECK("choose ticket by ticket", [&](auto& result)
         {
         ...
         // choose from a list of tickets that contains only handles[0]
         auto session1 = mgr.choose_from_offered_tickets(std::vector{ticket(handles[0].ticket().value())}, "SHA-256", cbs, plcy);
         result.require("ticket was chosen and produced a session (1)", session1.has_value());
         result.test_is_eq("chosen offset", session1->second, uint16_t(0));

         // choose from a list of tickets that contains only handles[1]
         auto session2 = mgr.choose_from_offered_tickets(std::vector{ticket(handles[1].ticket().value())}, "SHA-256", cbs, plcy);
         result.require("ticket was chosen and produced a session (2)", session2.has_value());
         result.test_is_eq("chosen offset", session2->second, uint16_t(0));

         // choose from a list of tickets that contains a random ticket and handles[1]
         auto session3 = mgr.choose_from_offered_tickets(std::vector{ticket(random_ticket()), ticket(handles[1].ticket().value())}, "SHA-256", cbs, plcy);
         result.require("ticket was chosen and produced a session (3)", session3.has_value());
         result.test_is_eq("chosen second offset", session3->second, uint16_t(1));
         }),

      CHECK("choose ticket based on requested hash function", [&](auto& result)
         {
         Test_Credentials_Manager creds;
         Botan::TLS::Session_Manager_Stateless mgr(creds, Test::rng());
         std::vector<Botan::TLS::Session_Handle> handles;
...
std::vector<Test::Result> test_session_manager_stateless()
   ...
   return
      {
      Botan_Tests::CHECK("establish with default parameters", [&](auto& result)
         {
      ...
      Botan_Tests::CHECK("establish without ticket key in credentials manager", [&](auto& result)
         {
         Empty_Credentials_Manager empty_creds;
         Botan::TLS::Session_Manager_Stateless local_mgr(empty_creds, Test::rng());
      ...
      Botan_Tests::CHECK("retrieve via ticket", [&](auto& result)
         ...
         Botan::TLS::Session_Manager_Stateless local_mgr(creds, Test::rng());
      ...
      Botan_Tests::CHECK("retrieve via ID does not work", [&](auto& result)
         {
         auto ticket1 = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         result.require("tickets created successfully", ticket1.has_value() && ticket1.has_value());

         result.confirm("retrieval by ID does not work", !mgr.retrieve(random_id(), cbs, plcy).has_value());
         }),

      Botan_Tests::CHECK("retrieve via opaque handle does work", [&](auto& result)
         {
         auto ticket1 = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         result.require("tickets created successfully", ticket1.has_value() && ticket1.has_value());

         result.confirm("retrieval by opaque handle", mgr.retrieve(ticket1->opaque_handle(), cbs, plcy).has_value());
         }),

      Botan_Tests::CHECK("no retrieve without or with wrong ticket key", [&](auto& result)
         {
         auto ticket1 = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         result.require("tickets created successfully", ticket1.has_value() && ticket1.has_value());

         Empty_Credentials_Manager empty_creds;
         Botan::TLS::Session_Manager_Stateless local_mgr1(empty_creds, Test::rng());

         Other_Test_Credentials_Manager incompatible_creds;
         Botan::TLS::Session_Manager_Stateless local_mgr2(incompatible_creds, Test::rng());

         result.confirm("no successful retrieval (without key)", !local_mgr1.retrieve(ticket1.value(), cbs, plcy).has_value());
         result.confirm("no successful retrieval (with wrong key)", !local_mgr2.retrieve(ticket1.value(), cbs, plcy).has_value());
         result.confirm("successful retrieval", mgr.retrieve(ticket1.value(), cbs, plcy).has_value());
         }),

      Botan_Tests::CHECK("Clients cannot be stateless", [&](auto& result)
         {
         ...
         result.test_throws("::store() does not work with opaque handle", [&]
            { mgr.store(default_session(Botan::TLS::Connection_Side::Client, cbs), random_opaque_handle()); });
      ...
      Botan_Tests::CHECK("remove is a NOOP", [&](auto& result)
         {
         auto ticket1 = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         result.require("tickets created successfully", ticket1.has_value() && ticket1.has_value());
      ...
      Botan_Tests::CHECK("retrieval via ticket reconstructs the start_time stamp", [&](auto& result)
         {
         ...
         result.test_is_eq("timestamps match", std::chrono::duration_cast<std::chrono::seconds>(session_before.start_time().time_since_epoch()).count(),
                                               std::chrono::duration_cast<std::chrono::seconds>(session_after->start_time().time_since_epoch()).count());
         }),
      };
   }
...
std::vector<Test::Result> test_session_manager_hybrid()
   ...
   auto CHECK_all = [&](std::string name, auto lambda) -> std::vector<Test::Result>
      ...
      std::vector<std::pair<std::string, std::function<std::unique_ptr<Botan::TLS::Session_Manager>()>>>
         stateful_manager_factories =
            {
            { "In Memory", []() -> std::unique_ptr<Botan::TLS::Session_Manager> { return std::make_unique<Botan::TLS::Session_Manager_In_Memory>(Test::rng(), 10); }},
#if defined(BOTAN_HAS_TLS_SQLITE3_SESSION_MANAGER)
            { "SQLite", []() -> std::unique_ptr<Botan::TLS::Session_Manager> { return std::make_unique<Botan::TLS::Session_Manager_SQLite>("secure_pw", Test::rng(), Test::temp_file_name("tls_session_manager_sqlite"), 10); }},
#endif
            };
      ...
      for(auto& factory_and_name : stateful_manager_factories)
         ...
         auto make_manager = [stateful_manager_factory, &creds](bool prefer_tickets)
            {
            return Botan::TLS::Session_Manager_Hybrid(stateful_manager_factory(),
                                                      creds,
                                                      Test::rng(),
                                                      prefer_tickets);
            };

         results.push_back(Botan_Tests::CHECK(std::string(name + " (" + stateful_manager_name + ")").c_str(), std::bind(lambda, make_manager, _1)));
         }
      return results;
      };
   ...
   return Test::flatten_result_lists(
      {
      CHECK_all("ticket vs ID preference in establishment", [&](auto make_manager, auto& result)
         {
         auto mgr_prefers_tickets = make_manager(true);
         auto ticket1 = mgr_prefers_tickets.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
      ...
      CHECK_all("ticket vs ID preference in retrieval", [&](auto make_manager, auto& result)
         {
         auto mgr_prefers_tickets = make_manager(true);
         auto mgr_prefers_ids = make_manager(false);

         auto id1 = mgr_prefers_tickets.underlying_stateful_manager()->establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
         auto id2 = mgr_prefers_ids.underlying_stateful_manager()->establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
      ...
      CHECK_all("no session tickets if hybrid manager cannot create them", [&](auto make_manager, auto& result)
         {
         Empty_Credentials_Manager empty_creds;
         Botan::TLS::Session_Manager_Hybrid empty_mgr(
            std::make_unique<Botan::TLS::Session_Manager_In_Memory>(Test::rng(), 10), empty_creds, Test::rng());
         auto mgr_prefers_tickets = make_manager(true);
         auto mgr_prefers_ids = make_manager(false);
...
class Temporary_Database_File
   ...
   public:
      Temporary_Database_File(const std::string& db_file)
...
std::vector<Test::Result> test_session_manager_sqlite()
   ...
   return
      {
      Botan_Tests::CHECK("migrate session database scheme (purges database)", [&](auto& result)
         {
         ...
         Botan::TLS::Session_Manager_SQLite legacy_db("thetruthisoutthere", Test::rng(), dbfile.get());

         result.confirm("Session_ID for randombit.net is gone", !legacy_db.retrieve(Botan::TLS::Session_ID(Botan::hex_decode("63C137030387E4A6CDAD303CCB1F53884944FDE5B4EDD91E6FCF74DCB033DCEB")), cbs, plcy).has_value());
         result.confirm("Session_ID for cloudflare.com is gone", !legacy_db.retrieve(Botan::TLS::Session_ID(Botan::hex_decode("63C136FAD49F05A184F910FD6568A3884164216C11E41CEBFDCD149AF66C1714")), cbs, plcy).has_value());
         result.confirm("no more session for randombit.net", legacy_db.find(Botan::TLS::Server_Information("randombit.net", 443), cbs, plcy).empty());
         result.confirm("no more session for cloudflare.com", legacy_db.find(Botan::TLS::Server_Information("cloudflare.com", 443), cbs, plcy).empty());

         result.test_is_eq("empty database won't get more empty", legacy_db.remove_all(), size_t(0));
         }),

      Botan_Tests::CHECK("clearing empty database", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("empty.sqlite"));
         result.test_eq("does not delete anything", mgr.remove_all(), 0);
         }),

      Botan_Tests::CHECK("establish new session", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("new_session.sqlite"));
         auto some_random_id = random_id();
         auto some_random_handle = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs), some_random_id);
      ...
      Botan_Tests::CHECK("retrieve session by ID", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("retrieve_by_id.sqlite"));
         auto some_random_id = random_id();
         auto some_random_handle = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs), some_random_id);
         auto some_virtual_handle = mgr.establish(default_session(Botan::TLS::Connection_Side::Server, cbs));
      ...
      Botan_Tests::CHECK("retrieval via ticket creates empty result", [&](auto& result)
        {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("retrieve_by_ticket.sqlite"));
      ...
      Botan_Tests::CHECK("storing sessions and finding them by server info", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("store_and_find.sqlite"));
         auto id = random_id();
      ...
      Botan_Tests::CHECK("removing sessions", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("remove.sqlite"));
         auto id = random_id();
         auto ticket = random_ticket();
         mgr.store(default_session(Botan::TLS::Connection_Side::Client, cbs), id);
         mgr.store(default_session(Botan::TLS::Connection_Side::Client, cbs), ticket);
      ...
      Botan_Tests::CHECK("old sessions are purged when needed", [&](auto& result)
         {
         Botan::TLS::Session_Manager_SQLite mgr("thetruthisoutthere", Test::rng(), Test::temp_file_name("purging.sqlite"), 1);
...
std::vector<Test::Result> tls_session_manager_expiry()
   ...
   auto CHECK_all = [&](std::string name, auto lambda) -> std::vector<Test::Result>
      ...
      std::vector<std::pair<std::string, std::function<std::unique_ptr<Botan::TLS::Session_Manager>()>>>
         stateful_manager_factories =
            {
            { "In Memory", []() -> std::unique_ptr<Botan::TLS::Session_Manager> { return std::make_unique<Botan::TLS::Session_Manager_In_Memory>(Test::rng(), 10); }},
            { "Stateless", [&]() -> std::unique_ptr<Botan::TLS::Session_Manager> { return std::make_unique<Botan::TLS::Session_Manager_Stateless>(creds, Test::rng()); }},
#if defined(BOTAN_HAS_TLS_SQLITE3_SESSION_MANAGER)
            { "SQLite", []() -> std::unique_ptr<Botan::TLS::Session_Manager> { return std::make_unique<Botan::TLS::Session_Manager_SQLite>("secure_pw", Test::rng(), Test::temp_file_name("tls_session_manager_sqlite"), 10); }},
#endif
            };
      ...
      for(auto& [sub_name, factory] : stateful_manager_factories)
         {
         results.push_back(Botan_Tests::CHECK(std::string(name + " (" + sub_name + ")").c_str(), std::bind(lambda, sub_name, factory, _1)));
         }
      return results;
      };
   ...
   return Test::flatten_result_lists(
      {
      CHECK_all("sessions expire", [&](auto, auto factory, auto& result)
         {
         auto mgr = factory();
      ...
      CHECK_all("expired tickets are not selected for PSK resumption", [&](auto, auto factory, auto& result)
         ...
         auto nothing = mgr->choose_from_offered_tickets(std::vector{ticket(new_handle.value()), ticket(old_handle.value())}, "SHA-256", cbs, plcy);
...
BOTAN_REGISTER_TEST_FN("tls", "tls_session_manager", test_session_manager_in_memory, test_session_manager_choose_ticket, test_session_manager_stateless, test_session_manager_hybrid, test_session_manager_sqlite, tls_session_manager_expiry);
