...
size_t buffer_insert(std::vector<T, Alloc>& buf,
   ...
   copy_mem(buf.data() + buf_offset, input, to_copy);
   return to_copy;
   }
...
size_t buffer_insert(std::vector<T, Alloc>& buf,
   ...
   copy_mem(buf.data() + buf_offset, input.data(), to_copy);
   return to_copy;
   }
...
operator+=(std::vector<T, Alloc>& out,
   ...
   copy_mem(out.data() + copy_offset, in.data(), in.size());
   return out;
   }
...
std::vector<T, Alloc>& operator+=(std::vector<T, Alloc>& out,
   ...
   copy_mem(out.data() + copy_offset, in.first, in.second);
   return out;
   }
