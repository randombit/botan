...
#include <botan/math/gf/gfp_modulus.h>
#include <iosfwd>

using namespace Botan::math;
using namespace Botan::math::gf;

/*BOOST_AUTO_TEST_CASE(test_bi_comp)
{
Botan::InitializerOptions init_options("");
Botan::LibraryInitializer init(init_options);
BigInt minus_one = BigInt("-1");
//BOOST_CHECK_MESSAGE(minus_one == -1, "actually, BigInt(\"-1\") is " << minus_one);
BOOST_CHECK(minus_one == BigInt(-1));
BOOST_CHECK_MESSAGE((minus_one <= -1) && (minus_one <= 1), "error in comparison with minus one");
}*/
...
BOOST_AUTO_TEST_CASE(test_rand_int)
{
    ...
    {
		cout << "." << flush;
        BigInt x = Botan::random_integer(1,3);
