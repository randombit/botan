...
void BOTAN_FN_ISA_SIMD_2X64 Argon2::blamka_simd64(uint64_t N[128], uint64_t T[128]) {
   ...
   for(size_t i = 0; i != 128 / 4; ++i) {
      ...
      const SIMD_2x64 t0 = SIMD_2x64::load_le(&T[4 * i]);
      const SIMD_2x64 t1 = SIMD_2x64::load_le(&T[4 * i + 2]);
