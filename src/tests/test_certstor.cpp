...
Test::Result test_certstor_crl_test(
   ...
   store.revoke_cert(certs[3].first, Botan::CA_COMPROMISE);
      ...
      result.test_eq("Can't revoke certificate", crls[0].is_revoked(certs[0].first) ^ crls[1].is_revoked(certs[0].first),
                     true);
      result.test_eq("Can't revoke certificate", crls[0].is_revoked(certs[3].first) ^ crls[1].is_revoked(certs[3].first),
                     true);
      }
