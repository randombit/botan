...
std::string resolve_mac_hash(const OID& oid) {
   ...
   static const OID sha224_oid = OID::from_string("SHA-224");
   ...
   static const OID sha512_256_oid = OID::from_string("SHA-512-256");
   if(oid == sha1_oid) {
      return "SHA-1";
   }
   if(oid == sha224_oid) {
...
/*
* Verify PKCS#12 MAC using PKCS12_KDF class (RFC 7292 password encoding).
*/
...
* handled; other attributes are silently skipped (RFC 7292 §4.2).
*/
...
void parse_safe_contents(BER_Decoder& decoder,
   ...
   static const OID safe_contents_bag_oid = OID::from_string("PKCS12.SafeContentsBag");
   ...
   while(decoder.more_items()) {
      ...
      if(bag_type == cert_bag_oid) {
         ...
         parse_safe_contents(nested_sc, password, cert_entries, key_entries, unknown_bag_types, depth + 1);
...
void parse_authenticated_safe(std::span<const uint8_t> data,
   ...
   while(seq.more_items()) {
      ...
      if(content_type == pkcs7_data_oid) {
         ...
         parse_safe_contents(sc_seq, password, cert_entries, key_entries, unknown_bag_types);
         sc_seq.verify_end();
...
PKCS12::PKCS12(std::span<const uint8_t> data, std::string_view password) {
   ...
   if(pfx_seq.more_items()) {
      ...
      verify_mac(auth_safe_content, mac_value, mac_salt, iterations, hash_name, password);
   }

   parse_authenticated_safe(auth_safe_content, password, cert_entries, key_entries, m_unknown_bag_types);
   ...
   pfx_seq.end_cons();
   if(!src.end_of_data()) {
      throw Decoding_Error("PKCS#12: trailing data after PFX");
   }
}
