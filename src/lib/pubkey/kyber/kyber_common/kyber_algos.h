...
T kyber_encode_polynomial_vector(const KyberPolyVecNTT& vec, const KyberConstants& mode) {
   T r(mode.polynomial_vector_bytes());
   kyber_encode_polynomial_vector(r, vec);
   return r;
}
...
class KyberPolynomialSampler {
   public:
      KyberPolynomialSampler(StrongSpan<const SeedT> seed, const KyberConstants& mode) :
   ...
   private:
      ...
      void sample_poly_cbd(KyberPoly& poly, KyberConstants::KyberEta eta) {
         ...
         kyber_sample_polynomial_from_cbd(poly, eta, randomness);
      }
...
template <typename T>
KyberPolynomialSampler(T, const KyberConstants&) -> KyberPolynomialSampler<T>;
