...
struct BOTAN_PUBLIC_API(3, 7) PKCS12_Options {
      ...
      /**
       * Digest algorithm for the PKCS#12 MAC (default: "SHA-1").
       * Use "SHA-256" for compatibility with modern OpenSSL 3.x.
       * Supported: "SHA-1", "SHA-256"
       */
      std::string mac_digest = "SHA-1";
};
...
class BOTAN_PUBLIC_API(3, 7) PKCS12_Data {
   public:
      ...
      /**
       * @return true if this contains any CA certificates
       */
      bool has_ca_certificates() const { return !m_ca_certs.empty(); }

      /**
       * @return true if this contains a friendly name
       */
      bool has_friendly_name() const { return !m_friendly_name.empty(); }
