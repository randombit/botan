...
def configure_build(botan_dir, build_dir):
    run_and_check(
        [
            os.path.join(botan_dir, "configure.py"),
            "--with-doxygen",
            "--with-sphinx",
...
def run_sphinx(botan_dir, tmp_dir, output_dir):
    ...
    shutil.copyfile(
        os.path.join(botan_dir, "readme.rst"), os.path.join(sphinx_dir, "index.rst")
    )
...
"""
    ...
    run_and_check(
        ["sphinx-build", "-c", sphinx_config, "-b", "latex", handbook_dir, latex_output]
    )
    ...
    shutil.copy(
        os.path.join(latex_output, "botan.pdf"), os.path.join(output_dir, "handbook")
    )
