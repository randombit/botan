...
const std::string eac_cvc_emsa("EMSA1_BSI");
  ...
  *************************************************/
  ...
  class EAC1_1_obj : public Signed_Object // CRTP is used enable the call sequence:
    ...
    protected:
      ...
      static SecureVector<byte> make_signature(std::auto_ptr<PK_Signer> const signer,
...
template<typename Derived> SecureVector<byte> EAC1_1_obj<Derived>::make_signature(std::auto_ptr<PK_Signer> const signer,
          const MemoryRegion<byte>& tbs_bits)
      {
        SecureVector<byte> seq_sig = signer->sign_message(tbs_bits); // this is the signature as a der sequence
        ECDSA_Signature sig(ecdsa::decode_seq(seq_sig));
