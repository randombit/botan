...
def to_ciphersuite_info(code, name):
    ...
    if mode not in ["CBC", "GCM", "CCM(8)", "CCM", "OCB"]:
        print(
            "#warning Unknown mode '%s' for ciphersuite %s (0x%d)"
            % (" ".join(cipher), name, code)
        )
...
def process_command_line(args):
    ...
    parser.add_option(
        "--with-ocb", action="store_true", default=True, help="enable OCB AEAD suites"
    )
    parser.add_option(
        "--without-ocb",
        action="store_false",
...
def main(args=None):
    ...
    ciphersuite_re = re.compile(
        " +0x([0-9a-fA-F][0-9a-fA-F]),0x([0-9a-fA-F][0-9a-fA-F]) + TLS_([A-Za-z_0-9]+) "
    )
...
"""
    ...
    for code in sorted(suites.keys()):
        ...
        suite_expr = (
            'Ciphersuite(0x%s, "%s", Auth_Method::%s, Kex_Algo::%s, "%s", %d, "%s", %d, KDF_Algo::%s, Nonce_Format::%s)'
            % (
                code,
                info[0],
                info[2],
