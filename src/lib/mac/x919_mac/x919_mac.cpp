...
void ANSI_X919_MAC::add_data(std::span<const uint8_t> input) {
   ...
   xor_buf(std::span(m_state.data() + m_position, to_be_xored.size()), to_be_xored);
   ...
   while(in.remaining() >= 8) {
      xor_buf(std::span(m_state.data(), 8), std::span<const uint8_t>(in.take(8)));
      m_des1->encrypt(m_state);
   }

   const auto remaining = in.take(in.remaining());
   xor_buf(std::span(m_state.data(), remaining.size()), remaining);
