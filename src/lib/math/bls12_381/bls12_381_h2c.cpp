...
// Z = -(2 + u), where c1 = 3 since 8 exactly divides p^2 - 1, and
...
std::pair<CT::Choice, FieldElement2> sswu_sqrt_ratio(const FieldElement2& u, const FieldElement2& v) {
   ...
   auto tv5 = pow_p2_minus_9_over_16(u * tv3);
   tv5 = tv5 * tv2;
   tv2 = tv5 * v;
   tv3 = tv5 * u;
   auto tv4 = tv3 * tv2;
...
std::pair<FE, FE> map_to_curve_sswu(const FE& u, const FE& A, const FE& B, const FE& Z) {
   ...
   // requires, though tv4 is never zero here. Possible optimization:
   // return the fraction (x : tv4) and evaluate the isogeny on it
   // homogeneously (Wahby-Boneh), avoiding this inversion entirely
   x = x * tv4.invert();
...
G1Projective clear_cofactor_g1(const G1Projective& pt) {
   static_assert(G1_H_EFF == BLS_Z_ABS + 1);
   return PointMul<G1Projective>::mul_by_z_abs(pt).add(pt);
}
