...
_MAC_ALGORITHMS = {
    ...
    "AES-GMAC": "GMAC(AES-256)",
    "KMAC128": "KMAC-128",
    "KMAC256": "KMAC-256",
}

def _mac_algorithm(algorithm: str, key_size_bits: int | None, tag_size_bits: int | None) -> str:
...
class TestMAC(WycheproofTests, unittest.TestCase):
    ...
    def run_test(self, data: dict, group: dict, test: dict) -> None:
        ...
        final_mac = mac.final()
        actual_mac = final_mac[:tag_size_bits // 8] if tag_size_bits is not None else final_mac
