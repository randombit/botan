/*
* (C) 2024      Jack Lloyd
* (C) 2024-2025 René Meusel - Rohde & Schwarz Cybersecurity
*
...
class Option {
   ...
   private:
      ...
      const std::optional<T>& peek() const { return m_value; }
...
class OptionsBuilder {
   public:
      static_assert(detail::OptionsContainer<ContainerT>);

   protected:
      template <detail::ConcreteOptions OptionsT>
      OptionsT commit() {
         return {std::exchange(m_options, {}), std::exchange(m_product_name, detail::unknown_product)};
      }

      ContainerT& options() { return m_options; }

      template <detail::BuilderOption OptionT>
      bool is_set(const OptionT& option) const {
...
class Options {
   ...
   protected:
      ...
      /**
       * Consume an option value for processing
       * @returns an OptionValue object containing the option value while
       *          providing convenience functionality to implement expectations
       *          on the existence of option values for runtime error handling.
       */
      ...
      /**
       * Access an option value without consuming it.
       * Note: This should be used in exceptional cases only, prefer to take()
       *       option values for processing to ensure proper error handling.
       * @returns a reference to the option value stored by the builder
       */
      [[nodiscard]] decltype(auto) peek(const detail::BuilderOption auto& o) { return o.peek(); }
 ...
 *
 *   auto all_options() const { return std::tie(context, label, hash); }
 * };
 *
 ...
 *
 * class KDF_Builder : public OptionsBuilder<KDF_OptionsContainer> {
 *    public:
 ...
 *          auto opts = this->commit<KDF_Options>();
 *          KDF kdf(opts);
