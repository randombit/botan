...
/*
* AggregateVerify: check e(pk_1, H(msg_1)) * ... * e(pk_n, H(msg_n)) == e(g1, sig)
* via one product of n+1 pairings.
*
* Section 3.1 of the draft: "In a basic scheme, rogue key attacks are
* handled by requiring all messages signed by an aggregate signature
* to be distinct."
*/
bool bls_aggregate_verify(std::span<const std::vector<uint8_t>> pks,
   ...
   // "If any two input messages are equal, return INVALID."
   std::vector<std::string_view> sorted_msgs(msgs.begin(), msgs.end());
   std::ranges::sort(sorted_msgs);
   if(std::ranges::adjacent_find(sorted_msgs) != sorted_msgs.end()) {
      return false;
   }
...
int main() {
   ...
   // Even a correctly signed aggregate over repeated messages must be
   // rejected in the basic scheme
   const std::vector<std::string_view> dup_msgs{"same message", "same message"};
   std::vector<std::vector<uint8_t>> dup_pks;
   std::vector<std::array<uint8_t, G2Affine::BYTES>> dup_sigs;
   for(const auto msg : dup_msgs) {
      const auto k = BlsPrivateKey::generate(rng);
      const auto k_pk = k.public_key();
      dup_pks.emplace_back(k_pk.begin(), k_pk.end());
      dup_sigs.push_back(k.sign(msg));
   }

   const auto dup_agg = bls_aggregate(dup_sigs);
   if(!dup_agg) {
      std::cout << "Aggregation failed\n";
      return 1;
   }

   check("Aggregate with repeated messages", bls_aggregate_verify(dup_pks, dup_msgs, *dup_agg), false);
