 ...
 **/
class BOTAN_PUBLIC_API(3, 13) XMSSMT_Parameters final {
   public:
      ...
      /**
       * @returns the XMSS^MT algorithm id encoded in the first 4 bytes of @p bytes.
       */
      static xmssmt_algorithm_t parse_oid(std::span<const uint8_t> bytes);

      static XMSSMT_Parameters from_name(std::string_view algo_name);

      static XMSSMT_Parameters from_id(xmssmt_algorithm_t id);

      XMSSMT_Parameters(const XMSSMT_Parameters& other) = default;
      XMSSMT_Parameters(XMSSMT_Parameters&& other) noexcept = default;
      XMSSMT_Parameters& operator=(const XMSSMT_Parameters& other) = default;
      XMSSMT_Parameters& operator=(XMSSMT_Parameters&& other) noexcept = default;
      ~XMSSMT_Parameters() = default;
