    ...
    namespace
    {
        ...
        public:
            Asio_Socket(const std::string &hostname,
                        ...
                        std::chrono::milliseconds timeout) : m_timeout(timeout), m_timer(m_io), m_tcp(m_io)
            {
            ...
            void write(const uint8_t buf[], size_t len) override
            {
                ...
                m_tcp.async_send(boost::asio::buffer(buf, len),
                                 [&ec](boost::system::error_code e, size_t)
                                 { ec = e; });
    ...
    namespace
    {
        ...
        public:
            Asio_SocketUDP(const std::string &hostname,
                           const std::string &service,
                           std::chrono::microseconds timeout) : m_timeout(timeout), m_timer(m_io), m_udp(m_io)
            {
                m_timer.expires_from_now(m_timeout);
                check_timeout();
