...
def _aead_algorithm(algorithm: str, key_size_bits: int, tag_size_bits: int | None, nonce_len: int) -> str:
...
def _decrypt(aead: botan.SymmetricCipher, iv: bytes, aad: bytes, ct_tag: bytes) -> bytes:
    aead.set_assoc_data(aad)
    aead.start(iv)
