...
class FFI_CRL_Test final : public FFI_Test {
   public:
      ...
      void ffi_test(Test::Result& result, botan_rng_t rng) override {
         ...
         size_t count;
         TEST_FFI_OK(botan_x509_crl_get_count, (empty_crl, &count));
         result.confirm("crl has correct number of entries", count == 0);

         botan_x509_crl_t new_crl;
         botan_x509_cert_t certs[1] = {sub2_cert};
         TEST_FFI_OK(botan_x509_crl_update,
                     (&new_crl, empty_crl, rng, ca_cert, ca_key, now, 86400, certs, 1, 1, nullptr, nullptr));
         ...
         result.confirm("new crl has correct number of entries", count == 1);
         ...
         result.confirm("expire time is correct", now - 20 <= expire_time && expire_time <= now + 20);
         result.confirm("reason is correct", reason == 1);

         ViewStringSink crl_string_sink;
         TEST_FFI_OK(botan_x509_crl_view_pem, (new_crl, crl_string_sink.delegate(), crl_string_sink.callback()));
         ViewBytesSink crl_bytes_sink;
         TEST_FFI_OK(botan_x509_crl_view_der, (new_crl, crl_bytes_sink.delegate(), crl_bytes_sink.callback()));

         TEST_FFI_OK(botan_x509_crl_verify_signature, (new_crl, ca_pubkey, &rc));
         result.confirm("crl signature is valid", rc == 1);
