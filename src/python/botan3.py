...
class SymmetricCipher:
    ...
    def update(self, txt: str | bytes):
        ...
        Input text must be of update_granularity() length.
    ...
    def finish(self, txt: str | bytes | None = None):
        """Finish processing (with an optional final input).
        May throw if message authentication checks fail, in which case all plaintext
        previously processed must be discarded.
...
def kdf(algo: str, secret: bytes, out_len: int, salt: bytes, label: bytes) -> bytes:
    ...
    the provided secret and salt values. Returns a value of the specified length."""
    out_buf = create_string_buffer(out_len)
...
class PublicKey: # pylint: disable=invalid-name
    ...
    def load_ecdh(cls, curve: str, pub_x: MPILike, pub_y: MPILike) -> PublicKey:
        ...
        as a pair ofintegers giving the affine coordinates."""
        pub = PublicKey()
        pub_x = MPI(pub_x)
        pub_y = MPI(pub_y)
    ...
    def export(self, pem: bool = False) -> str | bytes:
        ...
        If ``pem`` is True, the result is a PEM encoded string. Otherwise it is a binary DER value."""
        if pem:
            return self.to_pem()
        else:
            return self.to_der()
...
class PrivateKey:
    ...
    def check_key(self, rng_obj: RandomNumberGenerator, strong: bool = True) -> bool:
        """Test the key for consistency.
        If ``strong`` is ``True`` then more expensive tests are performed."""
        flags = 1 if strong else 0
...
class MPI:
    """Initialize an MPI object with specified value, left as zero otherwise.
    The ``initial_value`` should be an ``int``, ``str``, or ``MPI``.
...
class ECGroup:
    ...
    def supports_application_specific_group(cls) -> bool:
        """Returns true if in this build configuration it is possible to register an
        application specific elliptic curve"""
        r = c_int(0)
