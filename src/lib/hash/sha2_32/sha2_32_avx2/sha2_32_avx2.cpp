...
BOTAN_FN_ISA_AVX2_BMI2 BOTAN_SCRUB_STACK_AFTER_RETURN void SHA_256::compress_digest_x86_avx2(
   ...
   while(blocks >= 2) {
      ...
      for(size_t i = 0; i < 4; i++) {
         ...
         auto WK = WS[i] + SIMD_8x32::load_le128(&K[4 * i]);
      ...
      for(size_t r = 0; r != 48; r += 16) {
         auto w = next_w(WS) + SIMD_8x32::load_le128(&K[r + 16]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&K[r + 20]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&K[r + 24]);
         ...
         w = next_w(WS) + SIMD_8x32::load_le128(&K[r + 28]);
