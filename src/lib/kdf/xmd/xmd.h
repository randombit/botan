...
inline void expand_message_xmd(std::string_view hash_fn,
   ...
   const std::span<const uint8_t> input(reinterpret_cast<const uint8_t*>(input_str.data()), input_str.size());

   const std::span<const uint8_t> domain_sep(reinterpret_cast<const uint8_t*>(domain_sep_str.data()),
                                             domain_sep_str.size());
