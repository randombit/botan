...
OID EC_Group::EC_group_identity_from_order(const BigInt& order)
   {
   if(order == BigInt("0xFFFFFFFF00000000FFFFFFFFFFFFFFFFBCE6FAADA7179E84F3B9CAC2FC632551"))
      return OID{1,2,840,10045,3,1,7};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFC7634D81F4372DDF581A0DB248B0A77AECEC196ACCC52973"))
      return OID{1,3,132,0,34};

   if(order == BigInt("0x1FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFA51868783BF2F966B7FCC0148F709A5D03BB5C9B8899C47AEBB6FB71E91386409"))
      return OID{1,3,132,0,35};

   if(order == BigInt("0xE95E4A5F737059DC60DF5991D45029409E60FC09"))
      return OID{1,3,36,3,3,2,8,1,1,1};

   if(order == BigInt("0xC302F41D932A36CDA7A3462F9E9E916B5BE8F1029AC4ACC1"))
      return OID{1,3,36,3,3,2,8,1,1,3};

   if(order == BigInt("0xD7C134AA264366862A18302575D0FB98D116BC4B6DDEBCA3A5A7939F"))
      return OID{1,3,36,3,3,2,8,1,1,5};

   if(order == BigInt("0xA9FB57DBA1EEA9BC3E660A909D838D718C397AA3B561A6F7901E0E82974856A7"))
      return OID{1,3,36,3,3,2,8,1,1,7};

   if(order == BigInt("0xD35E472036BC4FB7E13C785ED201E065F98FCFA5B68F12A32D482EC7EE8658E98691555B44C59311"))
      return OID{1,3,36,3,3,2,8,1,1,9};

   if(order == BigInt("0x8CB91E82A3386D280F5D6F7E50E641DF152F7109ED5456B31F166E6CAC0425A7CF3AB6AF6B7FC3103B883202E9046565"))
      return OID{1,3,36,3,3,2,8,1,1,11};

   if(order == BigInt("0xAADD9DB8DBE9C48B3FD4E6AE33C9FC07CB308DB3B3C9D20ED6639CCA70330870553E5C414CA92619418661197FAC10471DB1D381085DDADDB58796829CA90069"))
      return OID{1,3,36,3,3,2,8,1,1,13};

   if(order == BigInt("0xF1FD178C0B3AD58F10126DE8CE42435B53DC67E140D2BF941FFDD459C6D655E1"))
      return OID{1,2,250,1,223,101,256,1};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF6C611070995AD10045841B09B761B893"))
      return OID{1,2,643,7,1,2,1,1,1};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF27E69532F48D89116FF22B8D4E0560609B4B38ABFAD2B85DCACDB1411F10B275"))
      return OID{1,2,643,7,1,2,1,2,1};

   if(order == BigInt("0x100000000000000000001B8FA16DFAB9ACA16B6B3"))
      return OID{1,3,132,0,9};

   if(order == BigInt("0x100000000000000000001F4C8F927AED3CA752257"))
      return OID{1,3,132,0,8};

   if(order == BigInt("0x100000000000000000000351EE786A818F3A1A16B"))
      return OID{1,3,132,0,30};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFE26F2FC170F69466A74DEFD8D"))
      return OID{1,3,132,0,31};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFF99DEF836146BC9B1B4D22831"))
      return OID{1,2,840,10045,3,1,1};

   if(order == BigInt("0x10000000000000000000000000001DCE8D2EC6184CAF0A971769FB1F7"))
      return OID{1,3,132,0,32};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFF16A2E0B8F03E13DD29455C5C2A3D"))
      return OID{1,3,132,0,33};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"))
      return OID{1,3,132,0,10};

   if(order == BigInt("0xFFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123"))
      return OID{1,2,156,10197,1,301};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFE5FB1A724DC80418648D8DD31"))
      return OID{1,2,840,10045,3,1,2};

   if(order == BigInt("0xFFFFFFFFFFFFFFFFFFFFFFFF7A62D031C83F4294F640EC13"))
      return OID{1,2,840,10045,3,1,3};

   if(order == BigInt("0x7FFFFFFFFFFFFFFFFFFFFFFF7FFFFF9E5E9A9F5D9071FBD1522688909D0B"))
      return OID{1,2,840,10045,3,1,4};

   if(order == BigInt("0x7FFFFFFFFFFFFFFFFFFFFFFF800000CFA7E8594377D414C03821BC582063"))
      return OID{1,2,840,10045,3,1,5};

   if(order == BigInt("0x7FFFFFFFFFFFFFFFFFFFFFFF7FFFFF975DEB41B3A6057C3C432146526551"))
