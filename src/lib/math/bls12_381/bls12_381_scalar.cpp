...
std::array<word, Scalar::N> scalar_redc(const std::array<word, 2 * Scalar::N>& v) {
   std::array<word, Scalar::N> r{};
   std::array<word, Scalar::N> ws{};
   bigint_monty_redc(r.data(), v.data(), SCALAR_P.data(), SCALAR_P.size(), SCALAR_P_DASH, ws.data(), ws.size());
   return r;
}
...
std::optional<Scalar> Scalar::deserialize(std::span<const uint8_t> bytes) {
   ...
   // Reject the zero scalar; a zero secret key, challenge, or nonce is
   // essentially always a mistake. This matches EC_Scalar::deserialize.
   if(CT::all_zeros(words.data(), N).as_bool()) {
      return {};
   }
...
void Scalar::serialize_to(std::span<uint8_t, Scalar::BYTES> bytes) const {
   ...
   store_be(bytes, v);
   // The scalar may be a secret; don't leave an extra copy on the stack
   secure_scrub_memory(v);
}

Scalar Scalar::add(const Scalar& other) const {
   std::array<word, Scalar::N> t{};
   word carry = 0;
   for(size_t i = 0; i != Scalar::N; ++i) {
...
Scalar Scalar::sub(const Scalar& other) const {
   std::array<word, Scalar::N> r;  // NOLINT(*-member-init)
   word borrow = 0;
   for(size_t i = 0; i != Scalar::N; ++i) {
   ...
   word carry = 0;
   for(size_t i = 0; i != Scalar::N; ++i) {
      r[i] = word_add(r[i], SCALAR_P[i] & mask, &carry);
   }
