...
class TLS_Null_Tests final : public Text_Based_Test {
   public:
      ...
      void invalid_ad_length_test(Test::Result& result,
         ...
         result.test_throws(
            "TLS_NULL_HMAC_AEAD_Decryption::set_associated_data()",
            "TLS 1.2 NULL/HMAC: invalid TLS AEAD associated data length in set_associated_data_n:src/lib/tls/tls12/tls_null/tls_null.cpp",
            [&]() { tls_null_decrypt.set_associated_data(associated_data); });
         return;
      }
