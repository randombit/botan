...
void TLS_CBC_HMAC_AEAD_Encryption::cbc_encrypt_record(std::span<uint8_t> buffer, size_t padding_length) {
   ...
   BOTAN_DEBUG_ASSERT(buffer.size() % block_size() == 0);

   const uint8_t padding_val = static_cast<uint8_t>(padding_length - 1);

   auto poison = CT::scoped_poison(padding_val, padding_length, buffer);

   const auto last_block = buffer.last(block_size());
...
size_t TLS_CBC_HMAC_AEAD_Encryption::finish_msg(std::span<uint8_t> buffer, size_t input_bytes) {
   ...
   const size_t input_size = full_msg.size() + 1 + (use_encrypt_then_mac() ? 0 : tag_size());
   ...
   if(use_encrypt_then_mac()) {
      BOTAN_ASSERT_NOMSG(buffer.size() == enc_size + tag_size());
      const auto payload_and_padding = buffer.first(enc_size);
      const auto tag = buffer.last(tag_size());
      ...
      const auto tag = buffer.subspan(full_msg.size(), tag_size());
...
void TLS_CBC_HMAC_AEAD_Decryption::cbc_decrypt_record(std::span<uint8_t> record) {
   if(record.empty() || record.size() % block_size() != 0) {
      throw Decoding_Error("Received TLS CBC ciphertext with invalid length");
   }

   const auto final_block = record.last(block_size());
...
size_t TLS_CBC_HMAC_AEAD_Decryption::finish_msg(std::span<uint8_t> buffer, size_t input_bytes) {
   ...
   if(record.size() < tag_size() || (record.size() - (use_encrypt_then_mac() ? tag_size() : 0)) % block_size() != 0) {
      throw TLS_Exception(Alert::BadRecordMac, "Message authentication failure");
   }

   if(use_encrypt_then_mac()) {
      const auto payload_and_padding = record.first(record.size() - tag_size());
      const auto tag = record.last(tag_size());
      ...
      BOTAN_DEBUG_ASSERT(mac_buf.size() == tag_size());

      const auto mac_ok = CT::is_equal(tag.data(), mac_buf.data(), tag_size());
      ...
      */
      ...
      const auto size_ok_mask =
         CT::Mask<uint16_t>::is_lte(static_cast<uint16_t>(tag_size() + pad_size), static_cast<uint16_t>(record.size()));
      ...
      */
      ...
      BOTAN_DEBUG_ASSERT(record.size() >= tag_size() + pad_size);
      const auto payload = record.first(record.size() - tag_size() - pad_size);
      const auto tag = record.subspan(payload.size(), tag_size());

      mac().update(assoc_data_with_len(payload.size()));
      mac().update(payload);
