...
std::vector<Test::Result> finished_message_handling()
   ...
   return
      {
      Botan_Tests::CHECK("Client sends and receives Finished messages", [&](auto& result)
         {
         ...
         std::reference_wrapper<Botan::TLS::Finished_13> client_fin = state.sending(std::move(client_finished));
...
std::vector<Test::Result> handshake_message_filtering()
   ...
   return
      {
      Botan_Tests::CHECK("Client with client hello", [&](auto& result)
         {
         Client_Handshake_State_13 state;
         ...
         [[maybe_unused]] // just making sure that the return type of .sending is correct
         std::reference_wrapper<Client_Hello_13> filtered = state.sending(std::move(client_hello));
