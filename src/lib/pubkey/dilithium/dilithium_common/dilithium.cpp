...
class Dilithium_PublicKeyInternal {
   public:
      static std::shared_ptr<Dilithium_PublicKeyInternal> decode(
         ...
         auto [rho, t1] = dilithium_decode_public_key(raw_pk, mode);
   ...
   public:
      DilithiumSerializedPublicKey raw_pk() const { return dilithium_encode_public_key(m_rho, m_t1, m_mode); }
...
class Dilithium_PrivateKeyInternal {
   public:
      static std::shared_ptr<Dilithium_PrivateKeyInternal> decode(DilithiumConstants mode,
         ...
         auto [rho, signing_seed, tr, s1, s2, t0] = dilithium_decode_private_key(sk, mode);
   ...
   public:
      DilithiumSerializedPrivateKey raw_sk() const {
         return dilithium_encode_private_key(m_rho, m_tr, m_signing_seed, m_s1, m_s2, m_t0, m_mode);
      }
...
std::pair<DilithiumPolyVec, DilithiumPolyVec> dilithium_compute_t1_and_t0(const DilithiumPolyMatNTT& A,
   ...
   return dilithium_power2round(t);
}
...
Dilithium_PrivateKey::Dilithium_PrivateKey(RandomNumberGenerator& rng, DilithiumMode m) {
   ...
   const auto A = dilithium_expand_A(rho, mode);
   auto [s1, s2] = dilithium_expand_s(rhoprime, mode);
   auto [t1, t0] = dilithium_compute_t1_and_t0(A, s1, s2);
...
Dilithium_PrivateKey::Dilithium_PrivateKey(std::span<const uint8_t> sk, DilithiumMode m) {
   ...
   const auto A = dilithium_expand_A(m_private->rho(), m_private->mode());
   auto [t1, _] = dilithium_compute_t1_and_t0(A, m_private->s1(), m_private->s2());
