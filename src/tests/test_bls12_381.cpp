...
class BLS12_381_Scalar_Tests final : public Test {
   public:
      std::vector<Test::Result> run() override {
         ...
         results.push_back(scalar_booth_recode());
   #endif
   ...
   private:
   ...
   #if defined(BOTAN_HAS_NUMBERTHEORY)
      Test::Result scalar_booth_recode() const {
         using Botan::BLS12_381::Scalar;

         Test::Result result("Scalar Booth recoding");
         const std::array<Scalar, 8> scalars = {Scalar::zero(),
                                                Scalar::one(),
                                                Scalar::from_u32(7),
                                                Scalar::from_u32(8),
                                                Scalar::from_u32(15),
                                                Scalar::from_u32(16),
                                                Scalar::from_u32(0xFFFFFFFF),
                                                Scalar::one().negate()};

         for(const auto& scalar : scalars) {
            const auto bytes = scalar.serialize();
            for(size_t window_bits = 3; window_bits <= 16; ++window_bits) {
               const size_t windows = (Scalar::BITS + window_bits) / window_bits;
               Botan::BigInt reconstructed;

               for(size_t window = 0; window != windows; ++window) {
                  const auto [digit, negate] = Botan::BLS12_381::scalar_booth_window(bytes, window, window_bits);
                  Botan::BigInt term(digit);
                  term <<= window * window_bits;
                  reconstructed += negate.as_bool() ? -term : term;
               }

               result.test_bn_eq("Reconstruct scalar with window " + std::to_string(window_bits),
                                 reconstructed,
                                 Botan::BigInt::from_bytes(bytes));
            }
         }
      ...
      Test::Result scalar_serde() const {
         ...
         // Oversized input is rejected even if it starts with a valid encoding
         std::array<uint8_t, Botan::BLS12_381::Scalar::BYTES + 1> long_buf{};
         long_buf[Botan::BLS12_381::Scalar::BYTES - 1] = 1;
         result.test_is_true("Oversized input rejected", !Botan::BLS12_381::Scalar::deserialize(long_buf).has_value());
   ...
   private:
      Test::Result fe_sqrt() const {
         ...
         for(size_t i = 0; i != 128; ++i) {
            ...
            if(result.test_is_true("sqrt of a square exists", s.has_value().as_bool())) {
               result.test_is_true("sqrt returns +-x", (s.value() == x || s.value() == x.negate()).as_bool());
            }

            // -1 is not a square since p == 3 (mod 4), so -x^2 has no root for x != 0
            if(!x.is_zero().as_bool()) {
               result.test_is_true("sqrt of a nonsquare fails", !x2.negate().sqrt().has_value().as_bool());
            }
         }

         const auto zero_sqrt = Botan::BLS12_381::FieldElement::zero().sqrt();
         if(result.test_is_true("sqrt of zero exists", zero_sqrt.has_value().as_bool())) {
            result.test_is_true("sqrt of zero is zero", zero_sqrt.value().is_zero().as_bool());
         }
      ...
      Test::Result scalar_serde() const {
         ...
         // Oversized input is rejected even if it starts with a valid encoding
         std::array<uint8_t, Botan::BLS12_381::FieldElement::BYTES + 1> long_buf{};
         long_buf[Botan::BLS12_381::FieldElement::BYTES - 1] = 1;
         result.test_is_true("Oversized input rejected",
                             !Botan::BLS12_381::FieldElement::deserialize(long_buf).has_value());
...
class BLS12_381_Fp2_Tests final : public Test {
   ...
   private:
      Test::Result fp2_serde() const {
         ...
         // A coefficient equal to p is non-canonical; c1 is encoded first
         const char* p_hex =
            "1A0111EA397FE69A4B1BA7B6434BACD764774B84F38512BF6730D2A0F6B0F6241EABFFFEB153FFFFB9FEFFFFFFFFAAAB";
         ...
         std::array<uint8_t, FieldElement2::BYTES + 1> long_buf{};
         result.test_is_true("Oversized input rejected", !FieldElement2::deserialize(long_buf).has_value());
      ...
      Test::Result fp2_sqrt() const {
         ...
         for(size_t i = 0; i != 64; ++i) {
            ...
            if(!x.is_zero().as_bool()) {
               result.test_is_true("sqrt of a nonsquare fails", !x2.mul_by_nonresidue().sqrt().has_value().as_bool());
            }
         }
   ...
   private:
      ...
      Test::Result fp12_ops() const {
         ...
         for(size_t i = 0; i != 64; ++i) {
            ...
            result.test_is_true("squaring matches multiplication", (x.square() == x * x).as_bool());
            result.test_is_true("negation", (x - x).is_zero().as_bool());  // NOLINT(*-redundant-expression)
            result.test_is_true("x plus negation is zero", (x + x.negate()).is_zero().as_bool());
...
class BLS12_381_G1_Tests final : public Test {
   ...
   private:
      Test::Result g1_mul2() const {
         ...
         for(const uint32_t v : {0, 1, 7, 8, 9, 15, 16, 17, 31, 32, 33}) {
            auto expected = G1Projective::identity();
            for(size_t i = 0; i != v; ++i) {
               expected = expected.add(g);
            }

            const auto scalar = Scalar::from_u32(v);
            result.test_bin_eq("mul handles signed-window boundary " + std::to_string(v),
                               g.mul(scalar).to_affine().serialize(),
                               expected.to_affine().serialize());
            result.test_bin_eq("mul2_vartime handles signed-window boundary " + std::to_string(v),
                               G1Projective::mul2_vartime(g, scalar, g, Scalar::zero()).to_affine().serialize(),
                               expected.to_affine().serialize());
         }
      ...
      Test::Result g1_msm() const {
         ...
         {
            const std::array<uint32_t, 16> values = {0, 1, 3, 4, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64, 255};
            ...
            for(const uint32_t v : values) {
               points.push_back(point.to_affine());
               scalars.push_back(Scalar::from_u32(v));
               for(size_t i = 0; i != v; ++i) {
                  expected = expected.add(point);
               }
               point = point.add(g);
            }

            result.test_bin_eq("Signed Pippenger boundary digits",
                               G1Projective::msm_vartime(points, scalars).to_affine().serialize(),
                               expected.to_affine().serialize());
         }
      ...
      Test::Result g1_group_law() const {
         ...
         result.test_bin_eq(
            "(r-1)*G is -G", g.mul(Scalar::one().negate()).to_affine().serialize(), g.negate().to_affine().serialize());
   ...
   private:
      Test::Result g2_mul2() const {
         ...
         for(size_t i = 0; i != 4; ++i) {
            ...
            result.test_bin_eq("mul2 with zero scalar",
                               G2Projective::mul2(p, Scalar::zero(), q, b).to_affine().serialize(),
                               q.mul(b).to_affine().serialize());
            result.test_bin_eq("mul2_vartime with zero scalar",
                               G2Projective::mul2_vartime(p, a, q, Scalar::zero()).to_affine().serialize(),
                               p.mul(a).to_affine().serialize());
         ...
         result.test_is_true("mul2_vartime of zeros is the identity",
                             G2Projective::mul2_vartime(g, Scalar::zero(), g, Scalar::zero()).is_identity());
      ...
      Test::Result g2_msm() const {
         ...
         result.test_throws("Mismatched span lengths throw", [&]() {
            const std::array<G2Affine, 1> p{G2Affine::generator()};
            G2Projective::msm_vartime(p, {});
         });
   ...
   private:
      Test::Result pairing_properties() const {
         ...
         for(size_t i = 0; i != 4; ++i) {
            ...
            // Identity points within a product contribute nothing
            const std::array<G1Affine, 2> id_ps{G1Affine::identity(), a_g1};
            const std::array<G2Affine, 2> id_qs{a_g2, b_g2};
            result.test_is_true("Identity G1 term is skipped",
                                Gt::multi_pairing(id_ps, id_qs) == Gt::pairing(a_g1, b_g2));

            const std::array<G1Affine, 2> ps2{a_g1, b_g1};
            const std::array<G2Affine, 2> id_qs2{b_g2, G2Affine::identity()};
            result.test_is_true("Identity G2 term is skipped",
                                Gt::multi_pairing(ps2, id_qs2) == Gt::pairing(a_g1, b_g2));
         }

         // A product consisting only of identity terms
         const std::array<G1Affine, 2> all_id_ps{G1Affine::identity(), G1Affine::identity()};
         const std::array<G2Affine, 2> all_id_qs{G2Affine::generator(), G2Affine::identity()};
         result.test_is_true("All-identity product is the identity",
                             Gt::multi_pairing(all_id_ps, all_id_qs).is_identity());
...
class BLS12_381_Multi_Pairing_KAT_Tests final : public Text_Based_Test {
   public:
      ...
      Test::Result run_one_test(const std::string& /*header*/, const VarMap& vars) override {
         ...
         if(!a1.has_value() || !b1.has_value() || !a2.has_value() || !b2.has_value()) {
            return result;
         }
...
class BLS12_381_H2C_DST_Tests final : public Test {
   ...
   private:
      Test::Result dst_handling() const {
         ...
         // DSTs longer than 255 bytes are prehashed with the H2C-OVERSIZE-DST-
         // prefix (RFC 9380 5.3.3); check every entry point against a manual prehash
         ...
         result.test_bin_eq("G2 NU long DST hashed",
                            G2Projective::hash_to_curve_nu(msg, long_dst).to_affine().serialize(),
                            G2Projective::hash_to_curve_nu(msg, hashed_dst).to_affine().serialize());
