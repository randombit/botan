...
Test::Result test_x509_ip_addr_blocks_path_validation_success() {
   ...
   */
   ...
   for(size_t i = 0; i < 4; i++) {
      ...
      auto dyn_ipv4_choice = IPAddressBlocks::IPAddressChoice<IPv4>(
         include_v4 ? dyn_ipv4_ranges : std::vector<IPAddressBlocks::IPAddressOrRange<IPv4>>{});
      auto dyn_ipv6_choice = IPAddressBlocks::IPAddressChoice<IPv6>(
         include_v6 ? dyn_ipv6_ranges : std::vector<IPAddressBlocks::IPAddressOrRange<IPv6>>{});
      ...
      std::vector<Botan::X509_Certificate> certs = {dyn_cert, inherit_cert};
