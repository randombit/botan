...
#define BOTAN_FFI_DO(T, obj, param, block)                \
    {                                                     \
      try                                                 \
        {                                                 \
        return apply_fn(obj, BOTAN_CURRENT_FUNCTION, [=](T& param) -> int { do { block } while(0); return 0; });\
        }                                                 \
      catch(std::exception& e)                            \
        {                                                 \
...
int botan_rng_get(botan_rng_t rng, uint8_t* out, size_t out_len)
   {
   BOTAN_FFI_DO(Botan::RandomNumberGenerator, rng, r, { r.randomize(out, out_len); });
   }

int botan_rng_reseed(botan_rng_t rng, size_t bits)
   {
   BOTAN_FFI_DO(Botan::RandomNumberGenerator, rng, r, { r.reseed_from_rng(Botan::system_rng(), bits); });
   }
...
int botan_mp_clear(botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { bn.clear(); });
   }

int botan_mp_set_from_int(botan_mp_t mp, int initial_value)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, {
      if(initial_value >= 0)
         {
...
int botan_mp_set_from_str(botan_mp_t mp, const char* str)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { bn = Botan::BigInt(str); });
   }
...
int botan_mp_set_from_mp(botan_mp_t dest, const botan_mp_t source)
   {
   BOTAN_FFI_DO(Botan::BigInt, dest, bn, { bn = safe_get(source); });
   }

int botan_mp_is_negative(const botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { return bn.is_negative() ? 1 : 0; });
   }

int botan_mp_is_positive(const botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { return bn.is_positive() ? 1 : 0; });
   }

int botan_mp_flip_sign(botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { bn.flip_sign(); });
   }

int botan_mp_from_bin(botan_mp_t mp, const uint8_t bin[], size_t bin_len)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { bn.binary_decode(bin, bin_len); });
   }
...
int botan_mp_to_bin(const botan_mp_t mp, uint8_t vec[])
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { bn.binary_encode(vec); });
   }
...
int botan_mp_to_uint32(const botan_mp_t mp, uint32_t* val)
   ...
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { *val = bn.to_u32bit(); });
   }
...
int botan_mp_add(botan_mp_t result, const botan_mp_t x, const botan_mp_t y)
   {
   BOTAN_FFI_DO(Botan::BigInt, result, res, { res = safe_get(x) + safe_get(y); });
   }
...
int botan_mp_div(botan_mp_t quotient,
   ...
   {
   BOTAN_FFI_DO(Botan::BigInt, quotient, q, {
      Botan::BigInt r;
...
int botan_mp_equal(const botan_mp_t x_w, const botan_mp_t y_w)
   {
   BOTAN_FFI_DO(Botan::BigInt, x_w, x, { return x == safe_get(y_w); });
   }

int botan_mp_is_zero(const botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { return bn.is_zero(); });
   }

int botan_mp_is_odd(const botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { return bn.is_odd(); });
   }

int botan_mp_is_even(const botan_mp_t mp)
   {
   BOTAN_FFI_DO(Botan::BigInt, mp, bn, { return bn.is_even(); });
   }

int botan_mp_cmp(int* result, const botan_mp_t x_w, const botan_mp_t y_w)
   {
   BOTAN_FFI_DO(Botan::BigInt, x_w, x, { *result = x.cmp(safe_get(y_w)); });
   }

int botan_mp_swap(botan_mp_t x_w, botan_mp_t y_w)
   {
   BOTAN_FFI_DO(Botan::BigInt, x_w, x, { x.swap(safe_get(y_w)); });
   }

// Return (base^exponent) % modulus
int botan_mp_powmod(botan_mp_t out, const botan_mp_t base, const botan_mp_t exponent, const botan_mp_t modulus)
   {
   BOTAN_FFI_DO(Botan::BigInt, out, o,
                       { o = Botan::power_mod(safe_get(base), safe_get(exponent), safe_get(modulus)); });
   }

int botan_mp_lshift(botan_mp_t out, const botan_mp_t in, size_t shift)
   {
   BOTAN_FFI_DO(Botan::BigInt, out, o, { o = safe_get(in) << shift; });
   }
...
int botan_mp_mod_inverse(botan_mp_t out, const botan_mp_t in, const botan_mp_t modulus)
   {
   BOTAN_FFI_DO(Botan::BigInt, out, o, { o = Botan::inverse_mod(safe_get(in), safe_get(modulus)); });
   }
...
int botan_mp_rand_bits(botan_mp_t rand_out, botan_rng_t rng, size_t bits)
   {
   BOTAN_FFI_DO(Botan::RandomNumberGenerator, rng, r, {
...
int botan_block_cipher_clear(botan_block_cipher_t bc)
   {
   BOTAN_FFI_DO(Botan::BlockCipher, bc, b, { b.clear(); });
   }
...
int botan_block_cipher_set_key(botan_block_cipher_t bc,
   ...
   {
   BOTAN_FFI_DO(Botan::BlockCipher, bc, b, { b.set_key(key, len); });
   }
...
int botan_block_cipher_block_size(botan_block_cipher_t bc)
   {
   BOTAN_FFI_DO(Botan::BlockCipher, bc, b, { return b.block_size(); });
   }
...
int botan_block_cipher_encrypt_blocks(botan_block_cipher_t bc,
                                      ...
                                      size_t blocks)
   {
   BOTAN_FFI_DO(Botan::BlockCipher, bc, b, { b.encrypt_n(in, out, blocks); });
   }
...
int botan_block_cipher_decrypt_blocks(botan_block_cipher_t bc,
                                      ...
                                      size_t blocks)
   {
   BOTAN_FFI_DO(Botan::BlockCipher, bc, b, { b.decrypt_n(in, out, blocks); });
   }
...
int botan_hash_output_length(botan_hash_t hash, size_t* out)
   {
   BOTAN_FFI_DO(Botan::HashFunction, hash, h, { *out = h.output_length(); });
   }

int botan_hash_block_size(botan_hash_t hash, size_t* out)
   {
   BOTAN_FFI_DO(Botan::HashFunction, hash, h, { *out = h.hash_block_size(); });
   }

int botan_hash_clear(botan_hash_t hash)
   {
   BOTAN_FFI_DO(Botan::HashFunction, hash, h, { h.clear(); });
   }

int botan_hash_update(botan_hash_t hash, const uint8_t* buf, size_t len)
   {
   BOTAN_FFI_DO(Botan::HashFunction, hash, h, { h.update(buf, len); });
   }

int botan_hash_final(botan_hash_t hash, uint8_t out[])
   {
   BOTAN_FFI_DO(Botan::HashFunction, hash, h, { h.final(out); });
   }

int botan_hash_copy_state(botan_hash_t* dest, const botan_hash_t source)
   {
   BOTAN_FFI_DO(Botan::HashFunction, source, src, {
...
int botan_mac_set_key(botan_mac_t mac, const uint8_t* key, size_t key_len)
   {
   BOTAN_FFI_DO(Botan::MessageAuthenticationCode, mac, m, { m.set_key(key, key_len); });
   }

int botan_mac_output_length(botan_mac_t mac, size_t* out)
   {
   BOTAN_FFI_DO(Botan::MessageAuthenticationCode, mac, m, { *out = m.output_length(); });
   }

int botan_mac_clear(botan_mac_t mac)
   {
   BOTAN_FFI_DO(Botan::MessageAuthenticationCode, mac, m, { m.clear(); });
   }

int botan_mac_update(botan_mac_t mac, const uint8_t* buf, size_t len)
   {
   BOTAN_FFI_DO(Botan::MessageAuthenticationCode, mac, m, { m.update(buf, len); });
   }

int botan_mac_final(botan_mac_t mac, uint8_t out[])
   {
   BOTAN_FFI_DO(Botan::MessageAuthenticationCode, mac, m, { m.final(out); });
   }
...
int botan_cipher_clear(botan_cipher_t cipher)
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { c.clear(); });
   }
...
int botan_cipher_query_keylen(botan_cipher_t cipher,
   ...
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, {
...
int botan_cipher_set_key(botan_cipher_t cipher,
   ...
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { c.set_key(key, key_len); });
   }
...
int botan_cipher_valid_nonce_length(botan_cipher_t cipher, size_t nl)
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { return c.valid_nonce_length(nl) ? 1 : 0; });
   }

int botan_cipher_get_default_nonce_length(botan_cipher_t cipher, size_t* nl)
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { *nl = c.default_nonce_length(); });
   }

int botan_cipher_get_update_granularity(botan_cipher_t cipher, size_t* ug)
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { *ug = c.update_granularity(); });
   }

int botan_cipher_get_tag_length(botan_cipher_t cipher, size_t* tl)
   {
   BOTAN_FFI_DO(Botan::Cipher_Mode, cipher, c, { *tl = c.tag_size(); });
   }
...
int botan_privkey_ed25519_get_privkey(botan_privkey_t key,
   ...
   {
#if defined(BOTAN_HAS_ED25519)
   BOTAN_FFI_DO(Botan::Private_Key, key, k, {
...
int botan_pubkey_ed25519_get_pubkey(botan_pubkey_t key,
   ...
   {
#if defined(BOTAN_HAS_ED25519)
   BOTAN_FFI_DO(Botan::Public_Key, key, k, {
...
int botan_pubkey_algo_name(botan_pubkey_t key, char out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::Public_Key, key, k, { return write_str_output(out, out_len, k.algo_name()); });
   }
...
int botan_pubkey_estimated_strength(botan_pubkey_t key, size_t* estimate)
   {
   BOTAN_FFI_DO(Botan::Public_Key, key, k, { *estimate = k.estimated_strength(); });
   }
...
int botan_pk_op_encrypt(botan_pk_op_encrypt_t op,
   ...
   {
   BOTAN_FFI_DO(Botan::PK_Encryptor, op, o, {
...
int botan_pk_op_decrypt(botan_pk_op_decrypt_t op,
   ...
   {
   BOTAN_FFI_DO(Botan::PK_Decryptor, op, o, {
...
int botan_pk_op_verify_update(botan_pk_op_verify_t op, const uint8_t in[], size_t in_len)
   {
   BOTAN_FFI_DO(Botan::PK_Verifier, op, o, { o.update(in, in_len); });
   }

int botan_pk_op_verify_finish(botan_pk_op_verify_t op, const uint8_t sig[], size_t sig_len)
   {
   BOTAN_FFI_DO(Botan::PK_Verifier, op, o, {
...
int botan_pk_op_key_agreement(botan_pk_op_ka_t op,
                              ...
                              const uint8_t salt[], size_t salt_len)
   {
   BOTAN_FFI_DO(Botan::PK_Key_Agreement, op, o, {
...
int botan_x509_cert_get_time_starts(botan_x509_cert_t cert, char out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.start_time()); });
   }

int botan_x509_cert_get_time_expires(botan_x509_cert_t cert, char out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.end_time()); });
   }

int botan_x509_cert_get_serial_number(botan_x509_cert_t cert, uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_vec_output(out, out_len, c.serial_number()); });
   }

int botan_x509_cert_get_fingerprint(botan_x509_cert_t cert, const char* hash, uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.fingerprint(hash)); });
   }

int botan_x509_cert_get_authority_key_id(botan_x509_cert_t cert, uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_vec_output(out, out_len, c.authority_key_id()); });
   }

int botan_x509_cert_get_subject_key_id(botan_x509_cert_t cert, uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_vec_output(out, out_len, c.subject_key_id()); });
   }

int botan_x509_cert_get_public_key_bits(botan_x509_cert_t cert, uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_vec_output(out, out_len, c.subject_public_key_bits()); });
   }
...
int botan_x509_cert_get_issuer_dn(botan_x509_cert_t cert,
                                  ...
                                  uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.issuer_info(key).at(index)); });
   }
...
int botan_x509_cert_get_subject_dn(botan_x509_cert_t cert,
                                   ...
                                   uint8_t out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.subject_info(key).at(index)); });
   }

int botan_x509_cert_to_string(botan_x509_cert_t cert, char out[], size_t* out_len)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, { return write_str_output(out, out_len, c.to_string()); });
   }

int botan_x509_cert_allowed_usage(botan_x509_cert_t cert, unsigned int key_usage)
   {
   BOTAN_FFI_DO(Botan::X509_Certificate, cert, c, {
...
int botan_mceies_decrypt(botan_privkey_t mce_key_obj,
   ...
   try
      ...
      {
      return ffi_error_exception_thrown("unknown");
      }
   }
