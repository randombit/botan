...
/// Maximum allowed SafeContentsBag nesting depth during parsing (anti-DoS).
constexpr size_t PKCS12_MAX_NESTING = 10;
...
void validate_options(const PKCS12_Export_Options& opts) {
   ...
   // An empty password is permitted for both encryption and the MAC: PKCS#12
   // defines an encoding for it (RFC 7292) and such files are common in the
   // wild (e.g. some Java keystores). It offers no real protection, but there
   // is no technical reason to reject it.
   if(opts.include_mac()) {
