...
class AsyncTestProcess:
    ...
    async def _read_stdout_until(self, needle):
        ...
        try:
            self._stdout += await asyncio.wait_for(
                self._proc.stdout.readuntil(needle), timeout=ASYNC_TIMEOUT
            )
    ...
    async def _close_stdin_read_stdout_to_eof_and_wait_for_termination(
        self,
        ...
        try:
            self._stdout += await asyncio.wait_for(
                self._proc.stdout.read(), timeout=ASYNC_TIMEOUT
            )
        except asyncio.TimeoutError:
    ...
    async def _finalize(self):
        ...
        try:
            ...
            logging.debug(
                "%s finished with return code: %d", self._name, self._proc.returncode
            )
            logging.debug(
                "%s said (stdout): %s", self._name, self._stdout.decode("utf-8")
            )
            if final_stderr:
                logging.log(
                    logging.ERROR if not self._all_clear else logging.DEBUG,
                    "%s said (stderr): %s",
                    self._name,
...
class ServerCertificateSuite:
    ...
    def __init__(self, tmp_dir, ecdsa_algo, hash_algo):
        ...
        test_cli(
            "keygen",
            ["--algo=ECDSA", f"--params={ecdsa_algo}", f"--output={self.private_key}"],
            "",
        )
        test_cli(
            "gen_self_signed",
        ...
        test_cli(
            "gen_pkcs10", [f"{self.private_key}", "localhost", f"--output={crt_req}"]
        )
        test_cli(
            "sign_cert",
            [self.ca_cert, self.private_key, crt_req, f"--output={self.cert}"],
        )
...
def test_cli(
    ...
    try:
        if cmd_input is None:
            ...
            proc = subprocess.Popen(
                cmdline, stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=proc_env
            )
    ...
    if stderr:
        if expected_stderr is None:
            logging.error(
                "Got output on stderr %s (stdout was %s) for command %s",
                stderr,
                stdout,
                cmdline,
                stack_info=True,
            ...
            if stderr != expected_stderr:
                logging.error(
                    "Got output on stderr %s which did not match expected value %s",
                    stderr,
                    expected_stderr,
                    stack_info=True,
                )
    ...
    if expected_output is not None:
        if stdout != expected_output:
            logging.error(
                "Got unexpected output running cmd %s %s",
                cmd,
                cmd_options,
                stack_info=True,
            )
            logging.info(
                "Output lengths %d vs expected %d", len(stdout), len(expected_output)
            )
...
def cli_version_tests(_tmp_dir):
    ...
    version_full_re = re.compile(
        r"Botan [0-9]\.[0-9]+\.[0-9](\-[a-z]+[0-9]+)?( UNSAFE .* BUILD)? \(.*\)$"
    )
...
def cli_is_prime_tests(_tmp_dir):
    ...
    test_cli(
        "is_prime", "548950623407687320763", "548950623407687320763 is probably prime"
    )
...
def cli_entropy_tests(_tmp_dir):
    ...
    status_re = re.compile(
        "Polling [a-z0-9_]+ gathered [0-9]+ bytes in [0-9]+ outputs with estimated entropy [0-9]+"
    )
    unavail_re = re.compile("Source [a-z0-9_]+ is unavailable")
    comp_re = re.compile(
        "Sample from [a-z0-9_]+ was .* compressed from [0-9]+ bytes to [0-9]+ bytes"
    )
...
def cli_factor_tests(_tmp_dir):
    ...
    test_cli(
        "factor", "12019502040659149507", "12019502040659149507: 3298628633 3643787579"
    )
...
def cli_hash_tests(_tmp_dir):
    ...
    test_cli(
        "hash",
        ["--algo=SHA-224", "--format=base58", "--no-fsname"],
        "MuGc8HkSVyJjfMjPM5UQikPToBTzNucEghcGLe",
        "abc",
    )

    test_cli(
        "hash",
        ["--algo=SHA-224", "--format=base58check", "--no-fsname"],
        "3MmfMqgrhemdVa9bDAGfooukbviWtKMBx2xauL2RsyAe",
        "abc",
    )
...
def cli_hmac_tests(tmp_dir):
    ...
    test_cli(
        "hmac",
        ["--no-fsname", "--hash=SHA-384", key_file, key_file],
...
def cli_bcrypt_tests(_tmp_dir):
    test_cli(
        "gen_bcrypt",
        "--work-factor=4 s3kr1t",
        "$2a$04$0.8G7o08XYwvBBWA3l0WUujtwoGZgGDzVSN8fNkNqXikcK4A3lHPS",
    )

    test_cli(
        "check_bcrypt",
        "s3kr1t $2a$04$gHX4Qg7pDSJuXiPXnmt8leyb.FFzX1Bv4rXwIj2cPSakJ8zNnhIka",
        "Password is valid",
    )

    test_cli(
        "check_bcrypt",
        "santa $2a$04$gHX4Qg7pDSJuXiPXnmt8leyb.FFzX1Bv4rXwIj2cPSakJ8zNnhIka",
...
-----END PRIVATE KEY-----"""
    ...
    test_cli(
        "keygen", ["--algo=ECDSA", "--params=secp256r1", "--output=" + priv_key], ""
    )

    test_cli("pkcs8", "--pub-out --output=%s %s" % (pub_key, priv_key), "")
    test_cli(
        "pkcs8", "--pub-out --der-out --output=%s %s" % (pub_der_key, priv_key), ""
    )

    test_cli(
        "pkcs8",
        "--pass-out=foof --cipher=AES-128/CBC --der-out --output=%s %s"
        % (enc_der, priv_key),
        "",
    )
    test_cli(
        "pkcs8",
        "--pass-out=foof --pbkdf=Scrypt --output=%s %s" % (enc_pem, priv_key),
        "",
    )
    ...
    test_cli(
        "fingerprint",
        ["--no-fsname", pub_der_key],
        "83:FC:67:87:30:C7:0C:9C:54:9A:E7:A1:FA:25:83:4C:77:A4:43:16:33:6D:47:3C:CE:4B:91:62:30:97:62:D4",
    )

    test_cli(
        "fingerprint",
        ["--no-fsname", pub_key, pub_der_key],
        "83:FC:67:87:30:C7:0C:9C:54:9A:E7:A1:FA:25:83:4C:77:A4:43:16:33:6D:47:3C:CE:4B:91:62:30:97:62:D4\n"
        "83:FC:67:87:30:C7:0C:9C:54:9A:E7:A1:FA:25:83:4C:77:A4:43:16:33:6D:47:3C:CE:4B:91:62:30:97:62:D4",
    )
    ...
    test_cli("verify", [pub_key, pub_key, "-"], "Signature is valid", valid_sig)
    ...
    test_cli(
        "cert_verify", ca_cert, "Certificate did not validate - Cannot establish trust"
    )
    ...
    if (
        cert_info.find(
            "Subject keyid: 69DD911C9EEE3400C67CBC3F3056CBE711BD56AF9495013F"
        )
        < 0
    ):
        logging.error("Unexpected output for cert_info command %s", cert_info)
    ...
    test_cli(
        "sign_cert", "%s %s %s --output=%s" % (ca_cert, priv_key, crt_req, user_cert)
    )

    test_cli(
        "cert_verify", [user_cert, ca_cert], "Certificate passes validation checks"
    )
...
def cli_xmss_sign_tests(tmp_dir):
    ...
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "1F040283F0D7D2156B06B7BE03FA5861035FF3BCC059671DB288162C04A94CED",
    )
    ...
    test_cli(
        "hash",
        ["--no-fsname", sig1],
        "9DEBA79CE9FDC4966D7BA7B05ABEC54E3C11BB1C2C2732F7658820F2CAE47646",
    )
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "A71507087530C85E9CF971CF3A305890B07B51519C405A2B3D0037C64D5802B1",
    )
    ...
    test_cli(
        "hash",
        ["--no-fsname", sig2],
        "803EC5D6BECDFB9DC676EE2EDFEFE3D71EE924343A2ED9D2D7BFF0A9D97D704E",
    )
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "D581F5BFDA65669A825165C7A9CF17D6D5C5DF349004BCB7416DCD1A5C0349A0",
    )
    ...
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "ACFD94CDF5D0674EE5489039CF70850A1FFF95480A94E8C6C6FD2BF006909D07",
    )
    ...
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "BE6F8F868DB495D95F73B50A370A218225253048E2F1C7C3E286568FDE203700",
    )

    # verify that key is updated after issuing a certificate
    test_cli(
        "sign_cert", "%s %s %s --output=%s" % (root_crt, priv_key, int_csr, int_crt)
    )
    test_cli(
        "hash",
        ["--no-fsname", priv_key],
        "8D3B736D8A708C342F9263163E0E3BAFE4132F74AE53A8EDF78074422CF80496",
    )

    test_cli(
        "cert_verify",
        "%s %s" % (int_crt, root_crt),
...
def cli_pbkdf_tune_tests(_tmp_dir):
    ...
    expected = re.compile(
        r"For (default|[1-9][0-9]*) ms selected Scrypt\([0-9]+,[0-9]+,[0-9]+\) using [0-9]+ MiB"
    )

    output = test_cli(
        "pbkdf_tune", ["--tune-msec=1", "--check", "1", "10", "50", "default"], None
    ).split("\n")
    ...
    expected_pbkdf2 = re.compile(
        r"For (default|[1-9][0-9]*) ms selected PBKDF2\(HMAC\(SHA-256\),[0-9]+\)"
    )

    output = test_cli(
        "pbkdf_tune",
        ["--algo=PBKDF2(SHA-256)", "--check", "1", "10", "50", "default"],
        None,
    ).split("\n")
    ...
    expected_argon2 = re.compile(
        r"For (default|[1-9][0-9]*) ms selected Argon2id\([0-9]+,[0-9]+,[0-9]+\)"
    )

    output = test_cli(
        "pbkdf_tune", ["--algo=Argon2id", "--check", "1", "10", "50", "default"], None
    ).split("\n")
...
1: UTC 2019-08-04T13:38:17 (+-1000000us)
  ...
  5: UTC 2019-08-04T13:38:18 (+-1000000us)""",
    ...
    test_cli(
        "roughtime_check",
        [chain, "--raw-time"],
        "1: UTC 1564925897781286 (+-1000000us)",
    )

    with open(chain, "w", encoding="utf8") as f:
        f.write(
            "ed25519 cbT+RPS7zKX6w71ssPibzmwWqU9ffRV5oj2OresSmhE= eu9yhsJfVfguVSqGZdE8WKIxaBBM0ZG3Vmuc+IyZmG2YVmrIktUByDdwIFw6F4rZqmSFsBO85ljoVPz5bVPCOw== "
        )
    test_cli(
        "roughtime_check",
        chain,
        expected_stderr="Error: Roughtime: Invalid signature or public key",
    )
...
""",
    ...
    test_cli(
        "roughtime",
        [],
        expected_stderr="Please specify either --servers-file or --host and --pubkey",
    )

    with open(ecosystem, "w", encoding="utf8") as f:
        f.write(
            "Cloudflare-Roughtime ed25519 gD63hSj4ScS+wuOeGrubXlq35N1c5Lby/S+T7MNTjxo= udp 127.0.0.1:"
            + str(server_port)
        )

    test_cli(
        "roughtime",
        ["--check-local-clock=0", "--chain-file=", "--servers-file=" + ecosystem],
        expected_stderr="ERROR: Public key does not match!",
    )

    with open(ecosystem, "w", encoding="utf8") as f:
        f.write(
            "Cloudflare-Roughtime ed25519 gD63hSj3ScS+wuOeGrubXlq35N1c5Lby/S+T7MNTjxo= udp 127.0.0.1:"
            + str(server_port)
        )

    test_cli(
        "roughtime",
        ["--chain-file=", "--servers-file=" + ecosystem],
...
def cli_cert_issuance_tests(tmp_dir, algos=None):
    ...
    test_cli(
        "keygen",
        ["--algo=%s" % algos[1][0], "--params=%s" % algos[1][1], "--output=" + int_key],
        "",
    )
    test_cli(
        "keygen",
    ...
    test_cli(
        "sign_cert", "%s %s %s --output=%s" % (root_crt, root_key, int_csr, int_crt)
    )

    test_cli("gen_pkcs10", "%s Leaf --output=%s" % (leaf_key, leaf_csr))
    test_cli(
        "sign_cert", "%s %s %s --output=%s" % (int_crt, int_key, leaf_csr, leaf_crt)
    )

    test_cli(
        "cert_verify",
        "%s %s %s" % (leaf_crt, int_crt, root_crt),
        "Certificate passes validation checks",
    )
...
def cli_cert_issuance_alternative_algos_tests(tmp_dir):
    for i, algo in enumerate(
        [
            [
                ("Dilithium", "Dilithium-8x7-AES-r3"),
                ("Dilithium", "Dilithium-8x7-AES-r3"),
                ("Dilithium", "Dilithium-8x7-AES-r3"),
            ],
            [("ECDSA", "secp256r1"), ("ECDSA", "secp384r1"), ("ECDSA", "secp256r1")],
            [
                ("Dilithium", "Dilithium-6x5-r3"),
                ("ECDSA", "secp256r1"),
                ("RSA", "2048"),
            ],
        ]
...
def cli_tls_ciphersuite_tests(_tmp_dir):
    ...
    for policy in policies:
        for version in versions:
            ...
            output = test_cli(
                "tls_ciphers", ["--version=" + version, "--policy=" + policy], None
            ).split("\n")
...
"""
  ...
  d= 1, l=   4:  BIT STRING                                 FFFFFF"""
    ...
    test_cli(
        "oid_info",
        "RSA",
        "The string 'RSA' is associated with OID 1.2.840.113549.1.1.1",
    )
    test_cli(
        "oid_info",
        "1.2.840.113549.1.1.1",
        "OID 1.2.840.113549.1.1.1 is associated with RSA",
    )
...
def cli_tls_socket_tests(tmp_dir):
    if (
        not run_socket_tests()
        or not check_for_command("tls_client")
        or not check_for_command("tls_server")
    ):
        return
    ...
    configs = [
        ...
        TestConfig(
            ...
            stdout_regex="Alert: handshake_failure",
            expect_error=True,
        ),
    ...
    class TestServer(AsyncTestProcess):
        def __init__(self, tmp_dir, port, psk, psk_identity, psk_prf, clients=0):
            ...
            with open(self.policy, "w", encoding="utf8") as f:
                ...
                f.write(
                    "key_exchange_groups = x25519 x448 secp256r1 ffdhe/ietf/2048 ML-KEM-768 x25519/ML-KEM-768"
                )
        ...
        async def __aenter__(self):
            server_cmd = [
                CLI_PATH,
                "tls_server",
                f"--max-clients={self.clients}",
                f"--port={self.port}",
                f"--policy={self.policy}",
                f"--psk={self.psk}",
                f"--psk-identity={self.psk_identity}",
                f"--psk-prf={self.psk_prf}",
                self.cert_suite.cert,
                self.cert_suite.private_key,
            ]
    ...
    class TestClient(AsyncTestProcess):
        client_message = (
            b"Client message %d with extra stuff to test record_size_limit: %s\n"
        ...
        async def perform_message_ping_pong(self):
            ...
            retcode = (
                await self._close_stdin_read_stdout_to_eof_and_wait_for_termination()
            )

            if retcode != 0:
                raise Exception(
                    f"Client failed with error ({self.config.name}): {retcode}"
                )
        ...
        async def expect_handshake_error(self):
            ...
            if retcode == 0:
                raise Exception(
                    f"Expected an error, but tls_client finished with success ({self.config.name})"
                )
            self._check_stdout_regex()
        ...
        def _check_stdout_regex(self):
            if self.config.stdout_regex:
                ...
                if not match:
                    raise Exception(
                        f"Client log did not match expected regex ({self.config.name}): {self.config.stdout_regex}"
                    )
        ...
        async def __aenter__(self):
            client_cmd = [
                ...
                "localhost",
                f"--port={self.port}",
                f"--trusted-cas={self.ca}",
                f"--tls-version={self.config.protocol_version}",
                f"--policy={self.policy}",
            ]
            if self.config.psk:
                client_cmd += [
                    f"--psk={self.config.psk}",
                    f"--psk-identity={self.config.psk_identity}",
                ]
            ...
            await self._launch(
                client_cmd,
                b"Handshake complete" if not self.config.expect_error else None,
            )
    ...
    async def run_async_test():
        async with TestServer(
            tmp_dir, port_for("tls_server"), psk, psk_identity, psk_prf, len(configs)
        ) as server:
            errors = 0
            for tls_config in configs:
                logging.debug(
                    "Running test for %s in TLS %s mode",
                    tls_config.name,
                    tls_config.protocol_version,
                )
                async with TestClient(
                    tmp_dir, server.port, server.ca_cert, tls_config
                ) as client:
                    try:
...
def cli_tls_online_pqc_hybrid_tests(tmp_dir):
    ...
    class TestConfig:
        ...
        def setup(self, tmp_dir):
            self.policy_file = tempfile.NamedTemporaryFile(
                dir=tmp_dir, mode="w+", encoding="utf-8"
            )
    ...
    if oqsp and oqs_test_ca:
        ...
        test_cfg += [
            TestConfig(
                ...
                "x25519/ML-KEM-768",
                port=oqsp["X25519MLKEM768"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp256r1/ML-KEM-768",
                port=oqsp["SecP256r1MLKEM768"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
            ...
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-640-SHAKE",
                port=oqsp["frodo640shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-976-SHAKE",
                port=oqsp["frodo976shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-1344-SHAKE",
                port=oqsp["frodo1344shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-640-AES",
                port=oqsp["frodo640aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-976-AES",
                port=oqsp["frodo976aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "eFrodoKEM-1344-AES",
                port=oqsp["frodo1344aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "x25519/eFrodoKEM-640-SHAKE",
                port=oqsp["x25519_frodo640shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "x25519/eFrodoKEM-640-AES",
                port=oqsp["x25519_frodo640aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "x448/eFrodoKEM-976-SHAKE",
                port=oqsp["x448_frodo976shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "x448/eFrodoKEM-976-AES",
                port=oqsp["x448_frodo976aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp256r1/eFrodoKEM-640-SHAKE",
                port=oqsp["p256_frodo640shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp256r1/eFrodoKEM-640-AES",
                port=oqsp["p256_frodo640aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp384r1/eFrodoKEM-976-SHAKE",
                port=oqsp["p384_frodo976shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp384r1/eFrodoKEM-976-AES",
                port=oqsp["p384_frodo976aes"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp521r1/eFrodoKEM-1344-SHAKE",
                port=oqsp["p521_frodo1344shake"],
                ca=oqs_test_ca,
            ),
            TestConfig(
                "test.openquantumsafe.org",
                "secp521r1/eFrodoKEM-1344-AES",
                port=oqsp["p521_frodo1344aes"],
                ca=oqs_test_ca,
            ),
        ]
    else:
    ...
    for cfg in test_cfg:
        ...
        if "Handshake complete" not in stdout:
            logging.error(
                "Failed to complete handshake (%s with %s): %s",
                cfg.host,
                cfg.kex_algo,
                stdout,
            )
...
def cli_tls_proxy_tests(tmp_dir):
    ...
    if (
        ...
        or not check_for_command("tls_proxy")
    ):
        return
        ...
        async with Proxy(tmp_dir, server_port, proxy_port, max_clients) as tls_proxy:
            ...
            for i in range(max_clients):
                ...
                conn = HTTPSConnection(
                    "localhost", port=proxy_port, context=context, timeout=20
                )
                conn.request("GET", "/")
                resp = conn.getresponse()
...
def cli_tss_tests(tmp_dir):
    ...
    test_cli(
        "tss_split",
        [str(m), str(n), data_file, "--share-prefix=%s/split" % (tmp_dir)],
        "",
    )
...
def cli_pk_encrypt_tests(tmp_dir):
    ...
    test_cli(
        "keygen", ["--algo=RSA", "--params=2048", "--output=%s" % (rsa_priv_key)], ""
    )
    ...
    test_cli(
        "pkcs8",
        ["--pub-out", "%s/rsa.priv" % (tmp_dir), "--output=%s" % (rsa_pub_key)],
        "",
    )
    ...
    test_cli(
        "pk_decrypt", [rsa_priv_key, ctext_file, "--output=%s" % (recovered_file)], ""
    )
...
def cli_tls_client_hello_tests(_tmp_dir):
    chellos = [
        # TLS 1.2
        (
            "6073536D3FA201A37C1F3944F6DCDD5A83FAA67DF4B1C9CBE4FA4399FDE7673C",
...
def cli_speed_pk_tests(_tmp_dir):
    ...
    format_re = re.compile(
        r"^.* [0-9]+ ([A-Za-z0-9 ]+)/sec; [0-9]+\.[0-9]+ ms/op .*\([0-9]+ (op|ops) in [0-9\.]+ ms\)"
    )
    for line in output:
...
def cli_speed_pbkdf_tests(_tmp_dir):
    ...
    format_re = re.compile(
        r"^.* [0-9]+ /sec; [0-9]+\.[0-9]+ ms/op .*\([0-9]+ (op|ops) in [0-9]+(\.[0-9]+)? ms\)"
    )
    for op in pbkdf_ops:
...
def cli_speed_table_tests(_tmp_dir):
    ...
    format_re = re.compile(
        r"^.* buffer size [0-9]+ bytes: [0-9]+\.[0-9]+ MiB\/sec .*\([0-9]+\.[0-9]+ MiB in [0-9]+\.[0-9]+ ms\)"
    )
    tbl_hdr_re = re.compile(r"^algo +operation +1024 bytes$")
    tbl_val_re = re.compile(r"^.* +(encrypt|decrypt) +[0-9]+(\.[0-9]{2})$")

    output = test_cli(
        "speed", ["--format=table", "--msec=%d" % (msec), "AES-128"], None
    ).split("\n")
    ...
    if (
        output[10].find("results are the number of 1000s bytes processed per second")
        < 0
    ):
...
def cli_speed_invalid_option_tests(_tmp_dir):
    ...
    test_cli(
        "speed",
        ["--buf-size=90000000", "--msec=1", "AES-128"],
    ...
    test_cli(
        "speed",
        ["--clear-cpuid=goku", "--msec=1", "AES-128"],
...
def cli_speed_tests(_tmp_dir):
    ...
    output = test_cli(
        "speed", ["--msec=%d" % (msec), "--buf-size=64,512", "AES-128"], None
    ).split("\n")
    ...
    output = test_cli(
        "speed", ["--msec=%d" % (msec), "ChaCha20", "SHA-256", "HMAC(SHA-256)"], None
    ).split("\n")
    ...
    format_re_ks = re.compile(
        r"^AES-128/GCM\(16\).* [0-9]+ key schedule/sec; [0-9]+\.[0-9]+ ms/op .*\([0-9]+ (op|ops) in [0-9\.]+ ms\)"
    )
    format_re_cipher = re.compile(
        r"^AES-128/GCM\(16\) .* buffer size [0-9]+ bytes: [0-9]+\.[0-9]+ MiB\/sec .*\([0-9]+\.[0-9]+ MiB in [0-9]+\.[0-9]+ ms\)"
    )
    for line in output:
    ...
    format_re = re.compile(
        r"^scrypt-[0-9]+-[0-9]+-[0-9]+ \([0-9]+ MiB\) [0-9]+ /sec; [0-9]+\.[0-9]+ ms/op .*\([0-9]+ (op|ops) in [0-9\.]+ ms\)"
    )
    ...
    format_re = re.compile(
        r"^.* generate buffer size [0-9]+ bytes: [0-9]+\.[0-9]+ MiB/sec .*\([0-9]+\.[0-9]+ MiB in [0-9]+\.[0-9]+ ms"
    )
    for line in output:
        if format_re.match(line) is None:
            logging.error("Unexpected line %s", line)
    ...
    format_re = re.compile(
        r"^zfec [0-9]+/[0-9]+ (encode|decode) buffer size [0-9]+ bytes: [0-9]+\.[0-9]+ MiB/sec .*\([0-9]+\.[0-9]+ MiB in [0-9]+\.[0-9]+ ms"
    )
    for line in output:
        if format_re.match(line) is None:
            logging.error("Unexpected line %s", line)
