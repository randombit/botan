...
def _set_prototypes(dll):
    ...
    ffi_api(dll.botan_x509_ext_ip_addr_blocks_get_family,
            [c_void_p, c_size_t, c_int, c_size_t, POINTER(c_int), POINTER(c_uint8), POINTER(c_int), POINTER(c_size_t)])
    ffi_api(dll.botan_x509_ext_ip_addr_blocks_get_address,
            [c_void_p, c_size_t, c_int, c_size_t, c_size_t, c_char_p, c_char_p, POINTER(c_size_t)])
    ...
    def ext_ip_addr_blocks(self) -> tuple[
        ...
        for (ipv6, stop) in ((0, v4_count), (1, v6_count)):
            for i in range(stop):
                ...
                present = c_int(0)
                count = c_size_t(0)
                _DLL.botan_x509_ext_ip_addr_blocks_get_family(self.__obj, c_size_t(v4_count), c_int(ipv6), c_size_t(i), byref(has_safi), byref(safi), byref(present), byref(count))
                ranges = None
                if present.value == 1:
                    ranges = []
                    for entry in range(count.value):
                        min_, max_ = _call_fn_returning_vec_pair(
                            size, size, lambda mi, _, ma, out_len, v4_count=v4_count, ipv6=ipv6, i=i, entry=entry: _DLL.botan_x509_ext_ip_addr_blocks_get_address(
                                self.__obj,
                                c_size_t(v4_count),
                                c_int(ipv6),
                                c_size_t(i),
